 ...
 */
async function runBrowserCheck(filePath: string): Promise<BrowserCheckResult> {
  ...
  try {
    ...
    page.on('console', msg => {
      ...
      const thresholdLevel = severityLevels[SEVERITY_THRESHOLD as keyof typeof severityLevels] || 1;
 ...
 */
function formatGuardianTodo(result: BrowserCheckResult): string {
 ...
 */
async function createGuardianTodo(result: BrowserCheckResult): Promise<string> {
