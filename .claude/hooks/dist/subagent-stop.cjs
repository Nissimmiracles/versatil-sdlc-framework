...
if (filesChanged.length > 0) {
    ...
    const sourceFiles = filesChanged.filter(f => f.match(/\.(ts|tsx|js|jsx|py|rb|go|java)$/) && !f.match(/\.(test|spec)\./));
    ...
    if (sourceFiles.length > 0) {
        ...
        const packageJsonPath = (0, path_1.join)(workingDirectory, 'package.json');
