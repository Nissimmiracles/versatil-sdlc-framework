...
exports.projectVisionManager = exports.ProjectVisionManager = void 0;
const events_1 = require("events");
const fs_1 = require("fs");
const path_1 = require("path");
const os_1 = require("os");
// ==================== PROJECT VISION MANAGER ====================
class ProjectVisionManager extends events_1.EventEmitter {
    constructor() {
        super();
        this.versatilHome = (0, path_1.join)((0, os_1.homedir)(), '.versatil');
        this.projectsPath = (0, path_1.join)(this.versatilHome, 'projects');
    }
    /**
     * Initialize project vision storage directory
     */
    async ensureProjectDir(projectId) {
        const projectDir = (0, path_1.join)(this.projectsPath, projectId);
    ...
    async storeVision(projectId, vision) {
        try {
            ...
            const visionPath = (0, path_1.join)(projectDir, 'vision.json');
            // Load existing vision or create new
            let existingVision = null;
            try {
                const data = await fs_1.promises.readFile(visionPath, 'utf-8');
            ...
            await fs_1.promises.writeFile(visionPath, JSON.stringify(updatedVision, null, 2));
    ...
    async updateMarketContext(projectId, market) {
        try {
            const projectDir = await this.ensureProjectDir(projectId);
            const marketPath = (0, path_1.join)(projectDir, 'market-context.json');
            const marketContext = {
                ...market,
                updatedAt: Date.now()
            };
            await fs_1.promises.writeFile(marketPath, JSON.stringify(marketContext, null, 2));
    ...
    async getMarketContext(projectId) {
        try {
            const marketPath = (0, path_1.join)(this.projectsPath, projectId, 'market-context.json');
            const data = await fs_1.promises.readFile(marketPath, 'utf-8');
            return JSON.parse(data);
        }
        catch {
            return null;
    ...
    async trackEvent(projectId, event) {
        try {
            const projectDir = await this.ensureProjectDir(projectId);
            const historyPath = (0, path_1.join)(projectDir, 'history.jsonl');
            const fullEvent = {
            ...
            await fs_1.promises.appendFile(historyPath, line, 'utf-8');
    ...
    async trackMilestone(projectId, milestone) {
        try {
            const projectDir = await this.ensureProjectDir(projectId);
            const milestonesPath = (0, path_1.join)(projectDir, 'milestones.json');
            // Load existing milestones
            let milestones = [];
            try {
                const data = await fs_1.promises.readFile(milestonesPath, 'utf-8');
            ...
            await fs_1.promises.writeFile(milestonesPath, JSON.stringify(milestones, null, 2));
    ...
    async storeDecision(projectId, decision) {
        try {
            const projectDir = await this.ensureProjectDir(projectId);
            const decisionsPath = (0, path_1.join)(projectDir, 'decisions.jsonl');
            const fullDecision = {
            ...
            await fs_1.promises.appendFile(decisionsPath, line, 'utf-8');
    ...
    async getProjectHistory(projectId, limit) {
        try {
            ...
            const events = [];
            try {
                const historyData = await fs_1.promises.readFile((0, path_1.join)(projectDir, 'history.jsonl'), 'utf-8');
            ...
            let milestones = [];
            try {
                const milestonesData = await fs_1.promises.readFile((0, path_1.join)(projectDir, 'milestones.json'), 'utf-8');
            ...
            const decisions = [];
            try {
                const decisionsData = await fs_1.promises.readFile((0, path_1.join)(projectDir, 'decisions.jsonl'), 'utf-8');
    ...
    async hasVision(projectId) {
        try {
            const visionPath = (0, path_1.join)(this.projectsPath, projectId, 'vision.json');
            await fs_1.promises.access(visionPath);
            return true;
        }
        catch {
            return false;
    ...
    async deleteProjectData(projectId) {
        try {
            const projectDir = (0, path_1.join)(this.projectsPath, projectId);
            await fs_1.promises.rm(projectDir, { recursive: true, force: true });
...
exports.ProjectVisionManager = ProjectVisionManager;
// Export singleton instance
exports.projectVisionManager = new ProjectVisionManager();
