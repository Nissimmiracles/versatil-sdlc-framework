...
exports.verifyContextIssue = verifyContextIssue;
const child_process_1 = require("child_process");
const fs_1 = require("fs");
const path_1 = require("path");
const os_1 = require("os");
const semantic_similarity_service_js_1 = require("./semantic-similarity-service.js");
/**
 ...
 */
async function verifyContextIssue(issue, workingDir, userId, teamId, projectId, resolvedContext) {
    const verifications = [];
...
function loadUserPreferences(userId) {
    if (!userId)
        return null;
    const prefsPath = (0, path_1.join)((0, os_1.homedir)(), '.versatil', 'users', userId, 'preferences.json');
    if (!(0, fs_1.existsSync)(prefsPath)) {
        return null;
    }
    try {
        return JSON.parse((0, fs_1.readFileSync)(prefsPath, 'utf-8'));
    }
    catch {
        return null;
    }
...
function loadTeamConventions(teamId) {
    if (!teamId)
        return null;
    const conventionsPath = (0, path_1.join)((0, os_1.homedir)(), '.versatil', 'teams', teamId, 'conventions.json');
    if (!(0, fs_1.existsSync)(conventionsPath)) {
        return null;
    }
    try {
        return JSON.parse((0, fs_1.readFileSync)(conventionsPath, 'utf-8'));
    }
    catch {
        return null;
    }
...
function loadProjectVision(projectId) {
    if (!projectId)
        return null;
    const visionPath = (0, path_1.join)((0, os_1.homedir)(), '.versatil', 'projects', projectId, 'vision.json');
    if (!(0, fs_1.existsSync)(visionPath)) {
        return null;
    }
    try {
        return JSON.parse((0, fs_1.readFileSync)(visionPath, 'utf-8'));
    }
    catch {
        return null;
    }
...
async function verifyIndentationStyle(claim, workingDir, userId, resolvedContext) {
    ...
    const filePath = (0, path_1.join)(workingDir, claim.file);
...
async function verifyCommitFormat(claim, workingDir, teamId, resolvedContext) {
    ...
    // Get recent commits
    try {
        const commits = (0, child_process_1.execSync)('git log --format="%s" -n 10', {
            cwd: workingDir,
            encoding: 'utf-8'
        }).split('\n');
...
async function verifyVisionAlignment(claim, workingDir, projectId) {
    ...
    try {
        ...
        const semanticService = new semantic_similarity_service_js_1.SemanticSimilarityService();
...
async function getResponsibleAgent(file, workingDir) {
    try {
        ...
        const blame = (0, child_process_1.execSync)(`git log -1 --format="%an" -- ${file}`, {
            cwd: workingDir,
            encoding: 'utf-8'
        }).trim();
