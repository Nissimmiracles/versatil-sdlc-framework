...
exports.detectAndVerifyIssues = detectAndVerifyIssues;
exports.createVerifiedTodos = createVerifiedTodos;
exports.generateVerificationSummary = generateVerificationSummary;
 ...
 */
const GUARDIAN_ACTIVE_SESSIONS = new Set();
/**
...
const layer_classifier_js_1 = require("./layer-classifier.js");
const framework_verifier_js_1 = require("./framework-verifier.js");
const project_verifier_js_1 = require("./project-verifier.js");
const context_verifier_js_1 = require("./context-verifier.js");
/**
 ...
 */
async function detectAndVerifyIssues(healthCheckResult, workingDir, contextIdentity, userId, teamId, projectId) {
    ...
    if (GUARDIAN_ACTIVE_SESSIONS.size >= MAX_RECURSION_DEPTH) {
        console.error(`🛑 [Guardian Recursion Guard] Max recursion depth reached (${MAX_RECURSION_DEPTH}). ` +
    ...
    const classifications = (0, layer_classifier_js_1.classifyIssues)(healthCheckResult.issues);
    const layerStats = (0, layer_classifier_js_1.getLayerStatistics)(classifications);
    console.log(`📊 [Layer Classification] ${layerStats.total} issues: ` +
    ...
    for (const [issue, classification] of classifications.entries()) {
        console.log(`  🔍 Verifying: ${issue.component} (${classification.layer} layer, ${classification.confidence}% classification confidence)`);
        let verificationResult;
        try {
            // Route to appropriate verifier based on layer
            switch (classification.layer) {
                case 'framework':
                    verificationResult = await (0, framework_verifier_js_1.verifyFrameworkIssue)(issue, workingDir);
                    break;
                case 'project':
                    verificationResult = await (0, project_verifier_js_1.verifyProjectIssue)(issue, workingDir);
                    break;
                case 'context':
                    // Extract resolved context from health check result (v7.8.0)
                    const resolvedContext = healthCheckResult.resolvedContext;
                    verificationResult = await (0, context_verifier_js_1.verifyContextIssue)(issue, workingDir, userId, teamId, projectId, resolvedContext);
                    break;
            }
            // Step 3: Assign agent based on layer + issue type
            const assignedAgent = assignAgentByLayer(issue, classification.layer, verificationResult, contextIdentity);
            ...
            if (verificationResult.verified) {
                ...
                console.log(`    ✅ VERIFIED (${verificationResult.confidence}% confidence) → ${assignedAgent} ${autoApply ? '[AUTO-APPLY]' : '[MANUAL]'}`);
            }
            else {
 ...
 */
function assignAgentByLayer(issue, layer, verificationResult, contextIdentity) {
    // Get issue category for routing
    const category = categorizeIssue(issue);
    // Get routing rule for layer
    const layerRouting = layer_classifier_js_1.LAYER_AGENT_ROUTING[layer];
    ...
    if (layer === 'context' && agent === 'git-blame') {
        const contextResult = verificationResult;
...
function categorizeIssue(issue) {
    ...
    if (component.includes('build') || description.includes('build'))
    ...
    if (component.includes('hook'))
        return 'hook-error';
    if (component.includes('mcp'))
        return 'mcp-error';
    if (component.includes('rag'))
        return 'rag-health';
    // Project categories
    if (description.includes('test') && description.includes('fail'))
        return 'test-failure';
    if (description.includes('coverage'))
        return 'test-coverage';
    if (description.includes('vulnerabilit'))
        return 'security-vulnerability';
    if (description.includes('quality') || description.includes('eslint'))
        return 'code-quality';
    if (description.includes('accessibilit'))
    ...
    if (description.includes('convention'))
        return 'convention-violation';
    if (description.includes('vision'))
        return 'vision-misalignment';
...
function todoAlreadyExists(verifiedIssue, todosDir) {
    try {
        ...
        for (const file of files) {
            ...
            const filePath = (0, path_1.join)(todosDir, file);
 ...
 */
function groupVerifiedIssues(verifiedIssues, groupBy = 'agent', maxIssuesPerGroup = 10) {
 ...
 */
async function createVerifiedTodos(verifiedIssues, outputDir) {
    ...
    const todosDir = outputDir || (0, path_1.join)(process.cwd(), 'todos');
    const createdFiles = [];
    // Check if grouping is enabled (default: true)
    const groupTodos = process.env.GUARDIAN_GROUP_TODOS !== 'false';
    const groupBy = process.env.GUARDIAN_GROUP_BY || 'agent';
    ...
    if (groupTodos) {
        ...
        for (const group of groups) {
            ...
            const todoPath = (0, path_1.join)(todosDir, todoFilename);
            try {
                (0, fs_1.writeFileSync)(todoPath, todoContent, 'utf-8');
 ...
 */
function formatVerifiedTodoMarkdown(verifiedIssue) {
    const layerEmoji = {
        framework: '🏗️',
        project: '📦',
        context: '👤'
    ...
    const evidenceLines = verification.verifications
        .map((v, i) => `${i + 1}. ${v.verified ? '✓' : '❌'} ${v.claim}\n` +
        `   - Method: \`${v.method}\`\n` +
...
${verification.priority_violation ? `
### ⚠️ Priority Violation Detected

**What Went Wrong**: ${verification.priority_violation.explanation}

**Comparison Table**:

| Layer | Expected | Actual | Status |
|-------|----------|--------|--------|
| **User** | ${verification.priority_violation.expected_priority === 'User' ? `\`${verification.priority_violation.expected_value}\` ✅` : '-'} | ${verification.priority_violation.actual_priority === 'User' ? `\`${verification.priority_violation.actual_value}\`` : '-'} | ${verification.priority_violation.expected_priority === 'User' ? '❌ Ignored' : '-'} |
| **Team** | ${verification.priority_violation.expected_priority === 'Team' ? `\`${verification.priority_violation.expected_value}\` ✅` : '-'} | ${verification.priority_violation.actual_priority === 'Team' ? `\`${verification.priority_violation.actual_value}\`` : '-'} | ${verification.priority_violation.expected_priority === 'Team' ? '❌ Ignored' : '-'} |
| **Project** | ${verification.priority_violation.expected_priority === 'Project' ? `\`${verification.priority_violation.expected_value}\` ✅` : '-'} | ${verification.priority_violation.actual_priority === 'Project' ? `\`${verification.priority_violation.actual_value}\`` : '-'} | ${verification.priority_violation.expected_priority === 'Project' ? '❌ Ignored' : '-'} |
| **Framework** | ${verification.priority_violation.expected_priority === 'Framework' ? `\`${verification.priority_violation.expected_value}\` ✅` : '-'} | ${verification.priority_violation.actual_priority === 'Framework' ? `\`${verification.priority_violation.actual_value}\`` : '-'} | ${verification.priority_violation.actual_priority === 'Framework' ? '✅ Applied' : '-'} |

**Severity**: \`${verification.priority_violation.severity.toUpperCase()}\`

**Explanation**: ${verification.priority_violation.expected_priority} preference (\`${verification.priority_violation.expected_value}\`) should have been applied, but ${verification.priority_violation.actual_priority} default (\`${verification.priority_violation.actual_value}\`) was used instead.
...
${(() => {
        ...
        if (actualValue && typeof actualValue === 'object' && actualValue.violations && Array.isArray(actualValue.violations)) {
            ...
            if (violations.length > 0) {
                ...
                const violationRows = topViolations.map((v) => `| ${v.identifier} | ${v.line}:${v.column} | ${v.type} | \`${v.expected}\` | \`${v.actual}\` | \`${v.suggestion || v.identifier}\` |`).join('\n');
                return `
...
${(() => {
        ...
        if (actualValue && typeof actualValue === 'object' && actualValue.semantic_matches && Array.isArray(actualValue.semantic_matches)) {
            ...
            if (overallAlignment < 70) {
                const matchRows = semanticMatches.slice(0, 5).map((match) => `| ${match.text.slice(0, 60)}${match.text.length > 60 ? '...' : ''} | ${Math.round(match.similarity * 100)}% | ${match.isMatch ? '✅ Aligned' : '❌ Not aligned'} |`).join('\n');
                return `
 ...
 */
function formatCombinedTodoMarkdown(group) {
    const layerEmoji = {
        framework: '🏗️',
        project: '📦',
        context: '👤'
    ...
    const avgConfidence = Math.round(group.issues.reduce((sum, issue) => sum + issue.confidence, 0) / group.issues.length);
 ...
 */
function generateVerificationSummary(result) {
    const layers = ['framework', 'project', 'context'];
...
${result.verified_issues
        ...
        .slice(0, 5)
        .map((issue, i) => `${i + 1}. [${issue.priority.toUpperCase()}] ${issue.original_issue.component}: ` +
