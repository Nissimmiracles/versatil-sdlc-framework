...
exports.verifyFrameworkIssue = verifyFrameworkIssue;
 ...
 */
async function verifyFrameworkIssue(issue, workingDir) {
    const verifications = [];
...
async function verifyTypeScriptError(claim, workingDir) {
    try {
        ...
        const output = (0, child_process_1.execSync)('npx tsc --noEmit', {
            cwd: workingDir,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
        });
        ...
        return {
            ...
            evidence: {
                ...
                file_exists: claim.file ? (0, fs_1.existsSync)((0, path_1.join)(workingDir, claim.file)) : undefined
            }
        };
    }
}
...
async function verifyAgentDefinition(claim, workingDir) {
    ...
    const agentPath = (0, path_1.join)(workingDir, '.claude', 'agents', `${agentName}.md`);
    ...
    const content = (0, fs_1.readFileSync)(agentPath, 'utf-8');
...
async function verifyHookRegistration(claim, workingDir) {
    ...
    const settingsPath = (0, path_1.join)(workingDir, '.claude', 'settings.json');
...
async function verifyMCPServer(claim, workingDir) {
    ...
    const mcpPath = (0, path_1.join)(workingDir, 'src', 'mcp', 'versatil-mcp-server-v2.ts');
    ...
    try {
        (0, child_process_1.execSync)(`npx tsc --noEmit ${mcpPath}`, {
            cwd: workingDir,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
        });
...
async function verifyRAGHealth(claim, workingDir) {
    ...
    const ragRouterPath = (0, path_1.join)(workingDir, 'src', 'rag', 'rag-router.ts');
    if (!(0, fs_1.existsSync)(ragRouterPath)) {
        return {
    ...
    const distPath = (0, path_1.join)(workingDir, 'dist', 'rag', 'rag-router.js');
