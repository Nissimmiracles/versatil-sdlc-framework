...
exports.EnhancementDetector = void 0;
const guardian_logger_js_1 = require("./guardian-logger.js");
const guardian_learning_store_js_1 = require("./guardian-learning-store.js");
/**
...
class EnhancementDetector {
     ...
     */
    async detectEnhancements(patterns) {
        ...
        const enhancementCandidates = patterns.filter(p => p.enhancement_candidate &&
     ...
     */
    async generateEnhancementSuggestion(pattern) {
        ...
        const { autoApplicable, requiresManualReview } = this.determineAutoApplicability(pattern, evidence.verification_confidence);
        // Step 9: Determine approval tier (v7.12.0+)
        const { approvalTier, approvalRequired, approvalRequiredReason } = this.determineApprovalTier(pattern, evidence.verification_confidence, evidence.success_rate_if_implemented, roi, estimatedEffortHours);
        const suggestion = {
     ...
     */
    generateTitleAndDescription(pattern, category) {
        const descLower = pattern.issue_description.toLowerCase();
     ...
     */
    generateImplementationSteps(pattern, category) {
        const descLower = pattern.issue_description.toLowerCase();
        const steps = [];
     ...
     */
    async estimateEffort(pattern, implementationSteps) {
        ...
        if (this.config.enable_rag_lookup) {
            try {
                const similarFixes = await (0, guardian_learning_store_js_1.searchGuardianLearnings)(pattern.issue_description, 3);
     ...
     */
    calculateROI(pattern, estimatedEffortHours) {
    ...
    async gatherEvidence(pattern) {
        ...
        if (this.config.enable_rag_lookup) {
            try {
                const similarFixes = await (0, guardian_learning_store_js_1.searchGuardianLearnings)(pattern.issue_description, 5);
                evidence.similar_historical_fixes = similarFixes.map(fix => `${fix.pattern.slice(0, 80)} (${fix.success_rate}% success, ${fix.times_used} times used)`);
     ...
     */
    determineAutoApplicability(pattern, verificationConfidence) {
        // Auto-applicable if:
        // 1. Confidence ≥ 90%
        // 2. Manual fix exists (proven solution)
        // 3. Success rate ≥ 90%
        const autoApplicable = verificationConfidence >= 90 &&
        ...
        const requiresManualReview = pattern.severity === 'critical' ||
     ...
     */
    determineApprovalTier(pattern, verificationConfidence, successRate, roi, estimatedEffortHours) {
        ...
        if (verificationConfidence >= tier1Threshold &&
            successRate >= 95 &&
        ...
        const reasons = [`Medium confidence (${verificationConfidence}%)`];
        if (roi.roi_ratio >= 5) {
...
exports.EnhancementDetector = EnhancementDetector;
