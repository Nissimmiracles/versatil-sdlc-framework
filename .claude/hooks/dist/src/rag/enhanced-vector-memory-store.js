...
const gcp_vector_store_js_1 = require("../lib/gcp-vector-store.js");
...
class EnhancedVectorMemoryStore extends events_1.EventEmitter {
    ...
    async initializeGraphRAG() {
        try {
            ...
            if (gcpProject) {
                await graphrag_store_js_1.graphRAGStore.initialize();
    ...
    async initializeGCP() {
        try {
            const gcpProject = process.env.GOOGLE_CLOUD_PROJECT;
            if (gcpProject) {
                await gcp_vector_store_js_1.gcpVectorStore.initialize();
    ...
    async storeInGCP(memory) {
        if (!this.isGCPEnabled)
            return;
        try {
            await gcp_vector_store_js_1.gcpVectorStore.storePattern({
    ...
    async graphRAGSearch(query) {
        ...
        try {
            const results = await graphrag_store_js_1.graphRAGStore.query({
                query: query.query,
                limit: query.topK || 20,
                minRelevance: 0.3,
                agent: query.agentId,
    ...
    async gcpVectorSearch(query) {
        ...
        try {
            const results = await gcp_vector_store_js_1.gcpVectorStore.searchSimilar({
                text: query.query,
                limit: query.topK || 20,
                threshold: 0.7,
                agent: query.agentId,
