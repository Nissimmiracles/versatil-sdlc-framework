...
const context_identity_js_1 = require("../../src/isolation/context-identity.js");
 ...
 */
function generateEnforcementContext(identity) {
 ...
 */
function shouldRunFullHealthCheck() {
...
async function main() {
    ...
    try {
        ...
        try {
            contextIdentity = await (0, context_identity_js_1.detectContextIdentity)(workingDir);
        ...
        try {
            const { checkGuardianHealth } = await Promise.resolve().then(() => __importStar(require('../../src/agents/guardian/guardian-health-check.js')));
            const healthStatus = await checkGuardianHealth(workingDir, contextIdentity?.role || 'user', runFullHealthCheck // Pass throttle flag to health check function
            );
