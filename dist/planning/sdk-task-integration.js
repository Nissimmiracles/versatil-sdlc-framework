...
export class SDKTaskIntegration extends EventEmitter {
    constructor(config) {
        ...
        this.agentTaskToPlan = new Map();
        /** Map of plan IDs to agent tasks */
        this.planToAgentTasks = new Map();
    ...
    setupEventListeners() {
        ...
        this.taskPlanManager.on('plan-created', (plan) => {
     ...
     */
    async captureAgentTask(agentTask) {
        const activity = {
     ...
     */
    async generatePlanFromAgentTask(agentTask) {
        try {
     ...
     */
    async generateTaskBreakdown(agentTask) {
        // Create root task
        const rootTask = {
     ...
     */
    generateSubtasksForAgent(agentTask, parentId) {
        const subtasks = [];
    ...
    async reconstructHistoricalPlans(timeRange) {
        ...
        if (timeRange) {
            activities = activities.filter(a => a.startTime &&
     ...
     */
    groupActivitiesByTask(activities) {
        const groups = [];
        let currentGroup = [];
     ...
     */
    async reconstructTaskGroup(group) {
        if (group.length === 0)
            return null;
        ...
        const tasks = group.map((activity, index) => ({
     ...
     */
    async syncAgentTaskProgress(agentTask) {
     ...
     */
    findTaskByAgentTaskId(plan, agentTaskId) {
     ...
     */
    calculateAgentTaskProgress(agentTask) {
        if (agentTask.status === 'completed')
            return 100;
        if (agentTask.status === 'failed')
            return 0;
        if (agentTask.status === 'in_progress')
            return 50;
        return 0;
    }
    /**
     ...
     */
    estimateTaskDuration(agentTask) {
...
//# sourceMappingURL=sdk-task-integration.js.map