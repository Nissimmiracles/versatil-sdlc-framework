...
export class IntelligentContextCache extends EventEmitter {
    ...
    async analyzeProjectStructure(projectPath) {
        ...
        try {
            ...
            return {
                ...
                hasTests: await fs.access(join(projectPath, 'test')).then(() => true).catch(() => fs.access(join(projectPath, '__tests__')).then(() => true).catch(() => false))
            };
        }
        catch (error) {
            return {
    ...
    async analyzeCodeMetrics(projectPath) {
        ...
        try {
            ...
            const { stdout: fileCount } = await execAsync(`find "${projectPath}/src" -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\) 2>/dev/null | wc -l`, { timeout: 10000 }).catch(() => ({ stdout: '0' }));
            // Count lines of code
            const { stdout: lineCount } = await execAsync(`find "${projectPath}/src" -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\) -exec wc -l {} + 2>/dev/null | tail -1 | awk '{print $1}'`, { timeout: 10000 }).catch(() => ({ stdout: '0' }));
