...
export class PythonAdapter extends BaseLanguageAdapter {
    ...
    async runTests(options) {
        ...
        try {
            const { stdout, stderr } = await execAsync(`pytest ${coverageArg} ${patternArg} --tb=short --json-report --json-report-file=/tmp/pytest-report.json`, { cwd: this.rootPath });
    ...
    async getQualityMetrics() {
        ...
        try {
            ...
            if (ccMatch) {
                ...
                const gradeScores = { 'A': 100, 'B': 85, 'C': 70, 'D': 55, 'F': 40 };
    ...
    async findPythonFiles(patterns) {
        const files = [];
