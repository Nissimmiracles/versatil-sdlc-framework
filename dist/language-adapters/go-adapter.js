...
export class GoAdapter extends BaseLanguageAdapter {
    ...
    async runTests(options) {
        ...
        try {
            const { stdout } = await execAsync(`go test ./... -v -json ${coverageArg} ${patternArg}`, { cwd: this.rootPath });
    ...
    async lint(options) {
        ...
        try {
            ...
            const { stdout } = await execAsync(`golangci-lint run ${fixArg} --out-format=json ${files}`, { cwd: this.rootPath });
