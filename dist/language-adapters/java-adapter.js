...
export class JavaAdapter extends BaseLanguageAdapter {
    ...
    async lint(options) {
        try {
            ...
            const checkstyleReport = join(this.rootPath, this.buildTool === 'maven' ? 'target/checkstyle-result.xml' : 'build/reports/checkstyle/main.xml');
    ...
    async getQualityMetrics() {
        ...
        try {
            ...
            const { stdout: pmdOutput } = await execAsync('pmd check -d . -f text -R category/java/design.xml/CyclomaticComplexity', { cwd: this.rootPath });
            ...
            try {
                const { stdout: checkstyleOutput } = await execAsync('checkstyle -c /google_checks.xml .', { cwd: this.rootPath });
                // Parse checkstyle complexity warnings
                const complexityWarnings = checkstyleOutput.split('\n').filter(line => line.includes('CyclomaticComplexity') || line.includes('complexity'));
