...
import { FlywheelHealthMonitor } from '../agents/monitoring/flywheel-health-monitor.js';
export class FlywheelMonitoringTask extends EventEmitter {
    ...
    setupMonitorListeners() {
        this.monitor.on('health-check', (dashboard) => {
            this.emit('dashboard-updated', dashboard);
        });
        this.monitor.on('health-warning', (data) => {
    ...
    setupTaskPlanListeners() {
        ...
        this.taskPlanManager.on('plan-started', (plan) => {
     ...
     * Record flywheel execution metrics
     */
    async recordFlywheelExecution(metrics) {
        try {
     ...
     */
    async logPlanExecutionToRAG(plan) {
        try {
     ...
     */
    calculateEstimationAccuracy(plan) {
        if (!plan.actualDuration || !plan.estimatedDuration)
            return 0;
...
//# sourceMappingURL=flywheel-monitoring-task.js.map