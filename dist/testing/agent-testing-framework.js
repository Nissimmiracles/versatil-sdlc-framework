...
export class AgentTestingFramework {
    ...
    extractIssuesFromResponse(response) {
        const issues = [];
    ...
    generateTestReport() {
        ...
        // Critical failures
        const criticalFailures = this.testResults.filter(r => !r.passed && this.testScenarios.find(s => s.id === r.scenario)?.severity === 'critical');
        ...
        // Performance metrics
        const avgExecutionTime = Math.round(this.testResults.reduce((sum, r) => sum + r.executionTime, 0) / totalTests);
     ...
     */
    async runScenarioTests(scenarioId) {
...
let _instance = null;
export function getAgentTestingFramework(agentRegistry) {
    if (!_instance) {
        _instance = new AgentTestingFramework(agentRegistry);
    }
    return _instance;
}
//# sourceMappingURL=agent-testing-framework.js.map