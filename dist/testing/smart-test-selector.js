...
export class SmartTestSelector {
     ...
     */
    async selectTests(changedFiles, testTypes, options = {}) {
        const { includeIndirect = true, maxTests = 50, fullSuiteThreshold = 20 } = options;
        const reasoning = [];
        // If too many files changed, run full suite
        if (changedFiles.length >= fullSuiteThreshold) {
            reasoning.push(`Running full test suite: ${changedFiles.length} files changed (>= ${fullSuiteThreshold} threshold)`);
            return {
                direct: [],
                indirect: [],
        ...
        if (all.length > maxTests) {
            reasoning.push(`Limiting tests to ${maxTests} (found ${all.length}). Consider running full suite.`);
            all.splice(maxTests);
        }
        // Estimate duration (assume 2 seconds per test on average)
        const estimatedDuration = all.length * 2000;
        reasoning.push(`Total tests selected: ${all.length} (${direct.length} direct + ${indirect.length} indirect)`);
     ...
     */
    async getChangedFiles(sinceCommit) {
        try {
    ...
    async buildDependencyGraph() {
        try {
            ...
            const { stdout } = await execAsync('find src -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\)', { cwd: this.projectRoot });
     ...
     */
    resolveImportPath(importStr, fromFile) {
     ...
     */
    async findIndirectTests(sourceFile) {
        const indirectTests = new Set();
        const visited = new Set();
        const traverse = (filePath, depth = 0) => {
...
//# sourceMappingURL=smart-test-selector.js.map