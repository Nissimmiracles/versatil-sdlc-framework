...
import { ParallelTaskManager, ExecutionStatus } from './parallel-task-manager.js';
import { CollisionDetector, CollisionRisk } from './collision-detector.js';
...
export class WaveExecutor extends EventEmitter {
    ...
    async executeWave(wave) {
        ...
        try {
            ...
            if (wave.parallel_execution && wave.tasks.length > 1) {
                ...
                const fileDependencies = tasks.map(task => ({
                    task_id: task.id,
                    task_name: task.name,
            ...
            const failedTasks = Array.from(taskResults.values()).filter(r => r.status === ExecutionStatus.FAILED);
     ...
     * Convert wave tasks to Task objects for ParallelTaskManager
     */
    async convertWaveTasksToTasks(wave) {
     ...
     */
    calculateEstimatedParallelTime(waves) {
        return waves.reduce((total, wave) => {
     ...
     */
    calculateWaveTimeSavings(wave, actualDuration) {
        if (!wave.parallel_execution || wave.tasks.length <= 1) {
            return 0;
        }
