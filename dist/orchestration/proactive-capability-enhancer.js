...
export class ProactiveCapabilityEnhancer extends EventEmitter {
    constructor(config) {
        ...
        this.agentToolRegistry = new Map();
        this.config = {
     ...
     */
    async detectMissingCapabilities(agentType, task) {
        ...
        const missingTools = requirements.recommendedTools.filter(tool => !currentTools.has(tool));
        ...
        const gap = {
            agentType,
            missingCapabilities: Object.keys(requirements.detectedRequirements).filter(key => requirements.detectedRequirements[key]),
     ...
     */
    async enhanceAgentWithTools(agentType, tools, reason) {
        ...
        try {
            ...
            if (totalTools > this.config.maxToolsPerAgent) {
                throw new Error(`Tool limit exceeded (${totalTools} > ${this.config.maxToolsPerAgent})`);
            }
            // Add tools
            const toolsAdded = [];
     ...
     */
    async inheritToolsToSubAgent(parentAgentType, subAgentId) {
        ...
        return await this.enhanceAgentWithTools(subAgentId, tools, `Inherited from parent agent ${parentAgentType}`);
    }
    /**
     * Proactively enhance agent before task execution
     */
    async proactiveEnhancement(agentType, task) {
        ...
        return await this.enhanceAgentWithTools(agentType, gap.requiredTools, gap.reason);
    }
    /**
     * Get agent's current tools
     */
    ...
    // ========== Detection Helpers ==========
    detectBrowserAutomation(taskText, files) {
...
export function getProactiveCapabilityEnhancer(config) {
    if (!_enhancerInstance) {
        _enhancerInstance = new ProactiveCapabilityEnhancer(config);
    }
    return _enhancerInstance;
}
//# sourceMappingURL=proactive-capability-enhancer.js.map