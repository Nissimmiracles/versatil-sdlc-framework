 ...
 */
import { AgentRAGSynchronization } from './agent-rag-sync.js';
...
export class AIEraDeveloperOrchestrator {
    constructor(agentRegistry, ragStore, config = {}) {
     ...
     */
    async activateAgent(agentId, context, userFeedback) {
        ...
        const response = await this.agentRAGSync.activateAgentWithFullContext(agentId, aiEraContext, [], userFeedback);
     ...
     */
    async enrichWithFullContext(context, agentId) {
     ...
     * STEP 2: Add YOUR team's winning patterns
     */
    async addWinningPatterns(context, agentId) {
     ...
     * STEP 4: Add cross-stack expertise
     */
    async addCrossStackExpertise(context, agentId) {
     ...
     */
    async runProactiveQualityGates(response, context) {
     ...
     * STEP 7: Learn from this interaction
     */
    async learnFromInteraction(agentId, context, response, userFeedback) {
        ...
        if (userFeedback === 'positive' || (response.context?.confidence || 0) > 0.8) {
            await this.patternLearning.learnFromSuccess(context, response, {
        ...
        if (userFeedback === 'negative' || (response.context?.confidence || 0) < 0.3) {
            await this.patternLearning.learnFromFailure(context, response, response.context?.errorMessage || 'Low confidence result', response.context?.introspectionTime || 0);
        }
    }
    /**
    ...
    determineCategory(context) {
        ...
        if (content.includes('architecture') || content.includes('design pattern'))
            return 'architecture';
        if (content.includes('tool') || content.includes('library') || content.includes('framework'))
            return 'tool';
     ...
     */
    async executeFullStackWorkflow(workflowDescription, initialContext) {
     ...
     */
    determineAgentSequence(workflowDescription, context) {
 ...
 * );
 */ 
//# sourceMappingURL=ai-era-dev-orchestrator.js.map