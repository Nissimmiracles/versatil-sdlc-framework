...
export class IncrementalIntelligence {
    constructor(vectorStore) {
        ...
        this.qualityScores = [];
        this.SYNTHESIS_INTERVAL = 100; // Synthesize every 100 interactions
        this.vectorStore = vectorStore;
    }
    /**
     * Record interaction and learn from it
     */
    async recordInteraction(agentId, context, response, userFeedback) {
     ...
     */
    calculateQualityScore(response, userFeedback) {
     ...
     * Store interaction
     */
    async storeInteraction(agentId, context, response, qualityScore, userFeedback) {
     ...
     */
    async queryTopQualityInteractions(limit) {
        ...
        const highQuality = (result.documents || []).filter(doc => doc.metadata?.quality_score && doc.metadata.quality_score >= 0.7);
        return highQuality;
    }
    /**
     ...
     */
    incrementPattern(counts, pattern, confidence) {
     ...
     */
    extractPatternType(patternKey) {
        if (patternKey.includes('-handles-'))
            return 'agent-priority';
        if (patternKey.includes('-processes-'))
            return 'file-type';
        if (patternKey.includes('-feedback'))
    ...
    getMetrics() {
        ...
        const improvementTrend = learningRate > 0.05 ? 'improving' :
     ...
     */
    async queryMetaLearnings(agentId, limit = 10) {
        const result = await this.vectorStore.queryMemories({
...
//# sourceMappingURL=incremental-intelligence.js.map