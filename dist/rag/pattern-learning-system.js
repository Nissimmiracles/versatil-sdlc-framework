...
export class PatternLearningSystem {
     ...
     */
    async learnFromSuccess(context, response, actualOutcome) {
     ...
     */
    async learnFromFailure(context, response, failureReason, timeWasted) {
        console.log('[PatternLearning] Learning from failure to avoid future mistakes');
        const pattern = await this.extractPattern(context, response, { timeToComplete: timeWasted, testsPassed: false, codeReviewed: false, deployed: false }, 'failure');
     ...
     */
    async extractPattern(context, response, outcome, result) {
     ...
     */
    determinePatternType(context, response) {
        const content = context.content?.toLowerCase() || '';
        const filePath = context.filePath?.toLowerCase() || '';
        if (filePath.includes('test') || content.includes('test'))
            return 'testing';
        if (filePath.includes('deploy') || content.includes('deploy'))
            return 'deployment';
        if (content.includes('debug') || content.includes('error') || content.includes('fix'))
            return 'debugging';
        if (filePath.includes('architect') || content.includes('architect') || content.includes('design'))
            return 'architecture';
        return 'development';
    }
    /**
     ...
     */
    extractApproach(context, response) {
     ...
     */
    generatePatternDescription(type, approach, result) {
     ...
     */
    extractTags(context, response) {
        const tags = ['pattern', 'team-specific'];
        // Add agent tag
        if (response.agentId) {
     ...
     */
    async reinforcePattern(patternId, outcome) {
     ...
     */
    async storeAntiPattern(pattern, failureReason) {
        await this.vectorStore.storeMemory({
     ...
     */
    async getWinningPatternsFor(context, limit = 5) {
     ...
     */
    async getAntiPatternsToAvoid(context) {
        const query = context.content || context.userRequest || context.filePath || '';
        const result = await this.vectorStore.queryMemories({
            query,
            queryType: 'hybrid',
...
//# sourceMappingURL=pattern-learning-system.js.map