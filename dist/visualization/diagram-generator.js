...
export class DiagramGenerator {
     ...
     * - Subagent tasks
     */
    async generateTaskHierarchyTree(plan, options = {}) {
     ...
     */
    async generateTaskGanttChart(plan, options = {}) {
        const opts = { ...this.defaultOptions, ...options };
     ...
     * - Blocked tasks
     */
    async generateDependencyGraph(plan, options = {}) {
        const opts = { ...this.defaultOptions, ...options };
     ...
     */
    async generateArchitectureDiagram(scanResult, options = {}) {
        const opts = { ...this.defaultOptions, ...options };
        const title = opts.title || 'Project Architecture';
        let mermaid = `graph TB\n`;
        // Detect layers
        const hasFrontend = scanResult.techStack.frameworks.some(f => ['React', 'Vue', 'Angular', 'Next.js', 'Nuxt'].includes(f));
        const hasBackend = scanResult.techStack.frameworks.some(f => ['Express', 'NestJS', 'Django', 'Flask', 'FastAPI'].includes(f));
     ...
     * - SDLC phases
     */
    async generateWorkflowDiagram(agents = ['alex-ba', 'marcus-backend', 'james-frontend', 'maria-qa', 'sarah-pm'], options = {}) {
        const opts = { ...this.defaultOptions, ...options };
     ...
     * - Success/error paths
     */
    async generateUserFlowDiagram(flowSteps, options = {}) {
        const opts = { ...this.defaultOptions, ...options };
     ...
     */
    renderTaskTreeMermaid(tasks, parentId, depth) {
        let output = '';
     ...
     * @returns File path
     */
    async saveDiagram(diagram, outputDir = './docs', filename) {
     ...
     */
    async generateAllPlanDiagrams(plan, outputDir = './docs') {
        const results = [];
        // Task hierarchy
