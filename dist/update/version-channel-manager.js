...
export class VersionChannelManager {
     ...
     */
    async getVersionsForChannel(channel) {
        ...
        try {
            ...
            console.warn(`Failed to fetch versions for ${targetChannel}:`, error.message);
            return [];
        }
    }
    /**
     ...
     */
    async getLatestVersionForChannel(channel) {
        const targetChannel = channel || this.currentChannel;
        const channelConfig = this.channels.get(targetChannel);
        if (!channelConfig) {
            throw new Error(`Unknown channel: ${targetChannel}`);
        ...
        try {
            ...
            console.warn(`Failed to fetch latest for ${targetChannel}:`, error.message);
            return null;
        }
    }
    /**
     * Add custom channel
     */
    async addCustomChannel(name, url, description) {
     ...
     */
    async removeCustomChannel(name) {
        const customName = `custom:${name}`;
...
//# sourceMappingURL=version-channel-manager.js.map