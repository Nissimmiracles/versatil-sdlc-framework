 ...
 */
export class PagesMustHaveRoutesRule {
    constructor() {
        this.name = 'pages-must-have-routes';
        this.description = 'All page components must have corresponding route definitions';
    ...
    async check(graph) {
        ...
        for (const page of pageComponents) {
            ...
            const isRouted = routeConfigs.some(routeConfig => graph.hasEdge(routeConfig.filePath, page.filePath));
            if (!isRouted) {
    ...
    generateRouteSuggestion(page, routeConfigs) {
        ...
        const relativePath = path.relative(path.dirname(mainRouteConfig.filePath), page.filePath).replace(/\\/g, '/');
        return `
 ...
 */
export class MenusMustHaveRoutesRule {
    constructor() {
        this.name = 'menus-must-have-routes';
        this.description = 'All navigation menu items must have valid route definitions';
    ...
    async checkRouteExists(routePath, routeConfigs) {
 ...
 */
export class RoutesMustHaveComponentsRule {
    constructor() {
        this.name = 'routes-must-have-components';
        this.description = 'All route definitions must point to existing components';
        this.severity = 'blocker';
        this.enabled = true;
    }
    async check(graph) {
        ...
        for (const routeConfig of routeConfigs) {
            try {
                ...
                for (const route of routes) {
                    ...
                    const componentExists = routeConfig.imports.some(imp => imp.includes(route.component) || imp.endsWith(`/${route.component}`));
 ...
 */
export class DeliverableCompletenessRule {
    constructor() {
        this.name = 'deliverable-completeness';
        this.description = 'Multi-file deliverables must be complete before commit';
        this.severity = 'major';
        this.enabled = true;
    }
    async check(graph, changedFiles) {
        const violations = [];
        // Check if any changed files indicate a new page deliverable
        const newPages = changedFiles.filter(f => f.match(/\/(pages|views|routes)\/.*\.(tsx|jsx|vue|svelte)$/) &&
            !f.includes('.test.') &&
            !f.includes('.spec.'));
    ...
    getExpectedDeliverableFiles(pageFile) {
...
//# sourceMappingURL=architectural-validator.js.map