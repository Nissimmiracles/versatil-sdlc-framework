...
export class UserAgentMemoryStore extends EventEmitter {
     ...
     */
    async ensureMemoryDir(userId, agentId) {
     ...
     */
    async storeMemory(userId, agentId, memory) {
     ...
     * Get memory by key
     */
    async getMemory(userId, agentId, key) {
        try {
     ...
     */
    async deleteAllMemories(userId, agentId) {
        try {
            const memoryDir = join(this.usersPath, userId, 'memories', agentId);
     ...
     */
    async getMemoryCount(userId, agentId) {
        try {
            const memoryDir = join(this.usersPath, userId, 'memories', agentId);
            const files = await fs.readdir(memoryDir);
     ...
     */
    async listMemoryKeys(userId, agentId) {
        try {
            const memoryDir = join(this.usersPath, userId, 'memories', agentId);
            const files = await fs.readdir(memoryDir);
     ...
     */
    async cleanupExpired(userId, agentId) {
        let deletedCount = 0;
        try {
     ...
     */
    async cleanupExpiredForAgent(userId, agentId) {
        let deletedCount = 0;
        const now = Date.now();
        try {
            const memoryDir = join(this.usersPath, userId, 'memories', agentId);
...
//# sourceMappingURL=user-agent-memory-store.js.map