...
export class CodingStyleDetector {
     ...
     */
    async analyzeGitHistory(repoPath, userId, maxCommits = 100) {
        const result = {
            confidence: 0,
            samples: 0,
            detectedPreferences: {},
        ...
        try {
            ...
            const { stdout } = await execAsync(`cd "${repoPath}" && git log ${authorFilter} --pretty=format:"%H" --max-count=${maxCommits}`, { maxBuffer: 1024 * 1024 * 10 } // 10MB buffer
            );
            const commits = stdout.trim().split('\n').filter(Boolean);
            for (const commitHash of commits) {
                try {
                    // Get files changed in commit
                    const { stdout: filesOutput } = await execAsync(`cd "${repoPath}" && git diff-tree --no-commit-id --name-only -r ${commitHash}`);
                    const files = filesOutput.trim().split('\n').filter(f => this.isCodeFile(f));
                    for (const file of files) {
                        try {
                            // Get file diff
                            const { stdout: diffOutput } = await execAsync(`cd "${repoPath}" && git show ${commitHash}:${file}`);
     ...
     */
    async analyzeDirectory(dirPath, extensions = ['.ts', '.tsx', '.js', '.jsx']) {
        const result = {
            confidence: 0,
            samples: 0,
            detectedPreferences: {},
     ...
     */
    calculateConfidence(samples, analysis) {
        if (samples < 5)
     ...
     */
    async findCodeFiles(dir, extensions) {
        const files = [];
        try {
...
//# sourceMappingURL=coding-style-detector.js.map