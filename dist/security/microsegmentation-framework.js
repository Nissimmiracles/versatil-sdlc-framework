...
export class MicrosegmentationFramework extends EventEmitter {
    ...
    async createProjectSecurityContext(projectId, projectPath) {
    ...
    cloneSegmentForProject(templateId, newSegmentId, projectId) {
        const template = this.segments.get(templateId);
        if (!template)
            throw new Error(`Template segment ${templateId} not found`);
    ...
    checkAccessPolicy(segment, operation, permissions) {
        const operationMap = {
    ...
    logSecurityEvent(event, category, details) {
        const logEntry = {
            timestamp: new Date().toISOString(),
            event,
            category,
    ...
    getEventSeverity(event, category) {
        const severityMap = {
    ...
    updateSecurityMetrics(projectId, event) {
        const context = this.projectContexts.get(projectId);
        if (!context)
            return;
    ...
    generateSecurityFingerprint(projectId, projectPath) {
        const data = `${projectId}:${projectPath}:${Date.now()}`;
        return crypto.createHash('sha256').update(data).digest('hex');
    }
    generateProjectToken(projectId) {
        const tokenData = {
            projectId,
            timestamp: Date.now(),
    ...
    // Public API Methods
    getProjectSecurityContext(projectId) {
        return this.projectContexts.get(projectId);
    }
    getAllSecuritySegments() {
        return Array.from(this.segments.values());
    }
    getMicrosegmentationPolicy() {
        return this.policy;
    }
    ...
    async removeProjectSecurityContext(projectId) {
        const context = this.projectContexts.get(projectId);
        if (!context)
            return;
...
//# sourceMappingURL=microsegmentation-framework.js.map