...
export class CredentialAuditLogger {
     ...
     */
    async getProjectSummary(projectId, days = 30) {
    ...
    async detectAnomalies(projectId) {
        ...
        const recentHour = entries.filter(e => e.projectId === projectId &&
        ...
        const recentFailed = entries.filter(e => e.projectId === projectId &&
            !e.success &&
     ...
     */
    async exportLogs(outputPath, projectId) {
        try {
            const entries = await this.readLogEntries();
            ...
            await fs.writeFile(outputPath, JSON.stringify(filteredEntries, null, 2), { encoding: 'utf8' });
...
export function getCredentialAuditLogger() {
    if (!auditLoggerInstance) {
        auditLoggerInstance = new CredentialAuditLogger();
    }
    return auditLoggerInstance;
}
//# sourceMappingURL=credential-audit-logger.js.map