...
export class BoundaryEnforcementEngine extends EventEmitter {
    ...
    startDirectoryWatcher(boundaryId, boundary) {
        try {
    ...
    async handleFileSystemEvent(boundaryId, eventType, filePath) {
    ...
    matchesRule(rule, filePath, eventType) {
    ...
    evaluateCondition(condition, filePath, eventType) {
        switch (condition) {
            case 'always':
                return true;
    ...
    async createViolation(rule, filePath, eventType, boundaryId) {
        const violation = {
    ...
    mapRuleToViolationType(rule) {
        if (rule.rule_id.includes('traversal'))
            return 'path_traversal';
        if (rule.rule_id.includes('executable'))
            return 'executable_creation';
        if (rule.rule_id.includes('symlink'))
            return 'symlink_attack';
        if (rule.rule_id.includes('cross'))
            return 'cross_boundary_write';
        if (rule.rule_id.includes('escalation'))
    ...
    determineSeverity(rule, filePath) {
        if (rule.enforcement_level === 'quarantine')
            return 'critical';
        if (rule.priority === 1)
            return 'high';
        if (filePath.includes(this.frameworkRoot))
            return 'high';
    ...
    async enforceViolation(violation, rule) {
        let blocked = false;
        let action = 'logged';
    ...
    async quarantineResource(violation) {
        try {
            ...
            if (fs.existsSync(violation.target_path)) {
                ...
                fs.writeFileSync(`${quarantineFile}.metadata.json`, JSON.stringify(metadata, null, 2));
    ...
    async performIntegrityCheck(boundaryId) {
        const boundary = this.boundaries.get(boundaryId);
        if (!boundary || !fs.existsSync(boundary.root_path))
            return;
        try {
        ...
        try {
            ...
            for (const file of files) {
                if (file.isFile()) {
                    ...
                    const fileDir = file.parentPath || file.path || dirPath;
    ...
    getBoundaryViolations(boundaryId, severity, limit = 100) {
...
//# sourceMappingURL=boundary-enforcement-engine.js.map