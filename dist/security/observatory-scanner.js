...
export class ObservatoryScanner {
    constructor() {
        ...
        this.rateLimit = 30000; // 30 seconds between scans per domain
    ...
    async scanUrl(url, options = {}) {
        ...
        if (!options.rescan) {
            ...
            if (lastScan && Date.now() - lastScan < this.rateLimit) {
                ...
                throw new Error(`Rate limit: Please wait ${waitTime} seconds before scanning ${hostname} again`);
            }
        }
        try {
     ...
     * Initiate a new scan
     */
    async initiateScan(hostname, options) {
     ...
     */
    async pollScanResults(scanId, timeout) {
        const startTime = Date.now();
    ...
    generateRecommendations(result) {
        ...
        if (result.grade === 'F' || result.grade === 'D' || result.grade === 'D-' || result.grade === 'D+') {
            recommendations.push('🚨 CRITICAL: Security grade is below minimum threshold (C required)');
        }
        // Specific header recommendations
        const headers = result.response_headers;
        if (!headers['content-security-policy']) {
            recommendations.push('🔒 Add Content-Security-Policy (CSP) header to prevent XSS attacks');
        }
        if (!headers['strict-transport-security']) {
            recommendations.push('🔒 Enable HSTS (Strict-Transport-Security) to enforce HTTPS');
        }
        ...
        failedTests.forEach(test => {
            if (test.name === 'content-security-policy') {
                recommendations.push('📋 CSP Implementation: Start with report-only mode, then enforce');
            }
            if (test.name === 'subresource-integrity') {
                recommendations.push('🔐 Enable Subresource Integrity (SRI) for third-party scripts/styles');
            }
            if (test.name === 'cookies') {
                recommendations.push('🍪 Secure cookies with HttpOnly, Secure, and SameSite attributes');
            }
            if (test.name === 'redirection') {
                recommendations.push('🔄 Ensure all HTTP traffic redirects to HTTPS');
            }
        });
        // Score-based recommendations
        if (result.score < 50) {
            recommendations.push('⚡ Quick wins: Add basic security headers (takes < 5 minutes)');
        }
        if (result.score >= 50 && result.score < 80) {
            recommendations.push('📈 Good progress! Focus on CSP and cookie security next');
        }
        if (result.score >= 80) {
            recommendations.push('✅ Strong security posture! Consider A+ optimizations');
        }
    ...
    validateCSP(headers) {
        ...
        if (!csp) {
            ...
            validation.recommendations.push("Add: Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline'");
            return validation;
        }
        ...
        if (validation.issues.length > 0) {
            validation.recommendations.push('Review CSP directives and remove unsafe-* values', 'Use nonces or hashes instead of unsafe-inline', 'Add report-uri to monitor violations');
        }
        return validation;
    }
    /**
    ...
    validateHSTS(headers) {
        ...
        if (!hsts) {
            ...
            validation.recommendations.push('Add: Strict-Transport-Security: max-age=31536000; includeSubDomains; preload');
            return validation;
        }
        // Extract max-age
        ...
        if (!hsts.includes('preload')) {
            validation.recommendations.push('Consider adding preload directive and submitting to HSTS preload list');
        }
        validation.valid = validation.issues.length === 0;
        return validation;
    }
    ...
    validateXFrameOptions(headers) {
        ...
        if (csp?.includes('frame-ancestors')) {
            ...
            validation.recommendations.push('Using CSP frame-ancestors (modern alternative to X-Frame-Options)');
            return validation;
        }
        if (!xfo) {
            validation.issues.push('X-Frame-Options header missing');
            validation.recommendations.push('Add: X-Frame-Options: DENY or SAMEORIGIN', 'Or use CSP frame-ancestors directive (recommended)');
            return validation;
        }
    ...
    validateReferrerPolicy(headers) {
        ...
        if (!rp) {
            ...
            validation.recommendations.push('Add: Referrer-Policy: strict-origin-when-cross-origin (recommended)', 'Or: Referrer-Policy: no-referrer (most secure)');
            return validation;
        }
        if (validPolicies.includes(rp.toLowerCase())) {
            ...
            if (rp === 'unsafe-url') {
                validation.recommendations.push('unsafe-url leaks full URL in referrer - consider stricter policy');
            }
        }
        else {
    ...
    validatePermissionsPolicy(headers) {
        ...
        if (!pp) {
            validation.recommendations.push('Consider adding Permissions-Policy to control browser features', "Example: Permissions-Policy: geolocation=(), microphone=(), camera=()");
     ...
     */
    meetsMinimumGrade(grade, minimum = 'A') {
...
//# sourceMappingURL=observatory-scanner.js.map