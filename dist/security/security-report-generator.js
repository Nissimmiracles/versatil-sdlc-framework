...
export class SecurityReportGenerator {
    constructor(outputDir = '~/.versatil/security-reports') {
     ...
     */
    async generateReport(scanResult, validations) {
     ...
     */
    extractVulnerabilities(scanResult, validations) {
     ...
     */
    generateRecommendations(scanResult, validations) {
        ...
        const otherHeaders = validations.filter(v => !v.present &&
     ...
     */
    generateRemediationSteps(scanResult, validations) {
        const steps = [];
        let stepNumber = 1;
    ...
    async exportJSON(report, filename) {
        const outputPath = path.join(this.outputDir, filename || `security-report-${report.id}.json`);
     ...
     */
    async exportMarkdown(report, filename) {
        const outputPath = path.join(this.outputDir, filename || `security-report-${report.id}.md`);
...
`;
     ...
     */
    mapScoreToSeverity(scoreModifier) {
        if (scoreModifier <= -10)
            return 'critical';
        if (scoreModifier <= -5)
            return 'high';
    ...
    getHeaderSeverity(header) {
        ...
        if (criticalHeaders.includes(header))
            return 'critical';
        if (highHeaders.includes(header))
            return 'high';
        if (mediumHeaders.includes(header))
            return 'medium';
        return 'low';
    }
    /**
...
//# sourceMappingURL=security-report-generator.js.map