...
import os from 'os';
import { DailyAuditSystem, AuditStatus, IssueSeverity } from './daily-audit-system.js';
export class DailyAuditDaemon extends EventEmitter {
     ...
     */
    async runImmediateAudit(reason) {
     ...
     */
    shouldTriggerImmediateAudit(result) {
        const criticalIssues = result.issues.filter(issue => issue.severity >= this.config.immediateAuditThreshold);
        return criticalIssues.length > 0 || result.status === AuditStatus.FAILURE;
    }
    /**
     * Get next scheduled audit time
     */
    getNextScheduledAudit() {
        if (!this.cronJob)
            return undefined;
        try {
     ...
     */
    static async isRunning(pidPath) {
...
//# sourceMappingURL=daily-audit-daemon.js.map