...
export class VelocityPhaseTransitions {
     ...
     */
    async canTransitionFromPlanToAssess(context) {
     ...
     */
    async transitionPlanToAssess(context) {
     ...
     */
    async canTransitionFromAssessToDelegate(context) {
     ...
     */
    async transitionAssessToDelegate(context) {
     ...
     */
    async canTransitionFromDelegateToWork(context) {
     ...
     */
    async transitionDelegateToWork(context) {
     ...
     */
    async canTransitionFromWorkToCodify(context) {
        ...
        if (result.allowed && context.plan && context.work) {
            ...
            if (completionRate < 1.0) {
                ...
                result.warnings.push(`Only ${Math.round(completionRate * 100)}% of todos completed (${context.work.completedTodos.length}/${context.plan.todos.length})`);
            }
        }
        return result;
    }
     ...
     */
    async transitionWorkToCodify(context) {
     ...
     */
    checkUnresolvedDependencies(dependencies) {
        const unresolved = [];
     ...
     */
    async rollbackAssessToPlan(context, reason) {
     ...
     */
    async rollbackDelegateToAssess(context, reason) {
     ...
     */
    async rollbackWorkToDelegate(context, reason) {
...
//# sourceMappingURL=velocity-phase-transitions.js.map