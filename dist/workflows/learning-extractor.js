...
export class LearningExtractor {
    ...
    async extractLearnings(analysis) {
        ...
        try {
            const [codePatterns, warnings, lessons, performanceMetrics, agentInsights] = await Promise.all([
     ...
     */
    async extractCodePatterns(codeChanges) {
        const patterns = [];
     ...
     */
    identifyComponentPattern(change) {
        const content = change.content;
        // React hook pattern
     ...
     */
    extractSnippet(content, keyword, maxLength) {
     ...
     */
    async extractWarnings(analysis) {
        const warnings = [];
        // Quality warnings
     ...
     */
    async extractPerformanceMetrics(analysis) {
        const metrics = [];
        // Time saved metric
        metrics.push({
            metric: 'Time Saved',
     ...
     */
    async extractAgentInsights(performance) {
    ...
    calculateOverallEffectiveness(analysis) {
        ...
        return Math.round(timeSavedScore * weights.timeSaved +
     ...
     */
    calculateCompoundingScore(patterns, lessons, metrics) {
