...
export class UserCoherenceCheckService {
     ...
     */
    async performCoherenceCheck(quick = false) {
        const startTime = Date.now();
        // Run all checks in parallel
        const [versionCheck, installationCheck, agentCheck, mcpCheck, ragCheck, dependencyCheck, contextCheck] = await Promise.all([
            this.checkVersion(),
    ...
    async checkVersion() {
        try {
            ...
            const breaking_changes_since = [];
            // Calculate health score
            let health_score = 100;
            if (status === 'patch_available')
                health_score = 95;
            else if (status === 'minor_available')
                health_score = 85;
            else if (status === 'major_available')
                health_score = 70;
    ...
    async checkInstallation() {
        try {
            ...
            let compilation_status = 'missing';
            ...
            if (critical_missing.length > 0)
                health_score = Math.min(health_score, 30);
            if (compilation_status === 'outdated')
                health_score *= 0.95;
            if (compilation_status === 'missing')
                health_score *= 0.8;
            return {
                status,
                files_present,
    ...
    async checkDependencies() {
        try {
            ...
            const version_compatibility = 'valid'; // Placeholder
            // Determine status
            let status = 'healthy';
            ...
            if (critical_vulnerabilities > 0)
                ...
                health_score = 70;
            if (lock_file_integrity === 'invalid')
                health_score *= 0.9;
            if (!peer_dependencies_installed)
                health_score *= 0.85;
            if (version_compatibility === 'invalid')
                health_score *= 0.8;
            return {
                status,
    ...
    calculateOverallHealth(checks) {
        ...
        const weighted_sum = checks.version.health_score * weights.version +
     ...
     */
    getAutoFixConfidence(issue) {
        // High confidence for well-known fixes
        if (issue.component === 'dependencies' && issue.fix_action?.includes('npm audit'))
            return 95;
        if (issue.component === 'installation' && issue.fix_action?.includes('npm install'))
            return 90;
        if (issue.component === 'rag' && issue.fix_action?.includes('rag:start'))
            return 85;
     ...
     */
    getEstimatedDuration(issue) {
        // Estimate in seconds
        if (issue.component === 'rag')
            return 60; // 1 minute to restart
        if (issue.component === 'installation' && issue.fix_action?.includes('build'))
            return 120; // 2 minutes to rebuild
        if (issue.component === 'installation' && issue.fix_action?.includes('install'))
            return 180; // 3 minutes to reinstall
        if (issue.component === 'dependencies')
            return 90; // 1.5 minutes for audit fix
 ...
 */
export function getUserCoherenceCheckService(projectRoot) {
    return UserCoherenceCheckService.getInstance(projectRoot);
}
//# sourceMappingURL=user-coherence-check.js.map