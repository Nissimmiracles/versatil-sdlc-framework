...
export class SupabaseVectorStore extends EventEmitter {
     ...
     */
    async initializeEmbeddingProvider() {
        try {
     ...
     */
    async initializeLocalEmbeddings() {
        try {
            const { pipeline } = await import('@xenova/transformers');
            this.embeddingModel = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2', { device: 'cpu' } // Ensure CPU usage for broader compatibility
            );
    ...
    async addPattern(pattern) {
        try {
            const embedding = await this.generateEmbedding(`${pattern.type} ${pattern.language || ''} ${pattern.framework || ''} ${pattern.code}`);
    ...
    extractTags(pattern) {
        ...
        if (pattern.agent)
            tags.push(pattern.agent);
        return tags;
    }
    async notifyPatternAdded(pattern) {
        if (this.learningChannel) {
    ...
    async broadcastLearning(interaction) {
        if (this.learningChannel) {
            await this.learningChannel.send({
                type: 'broadcast',
                event: 'new-solution',
...
export function createSupabaseVectorStore(config) {
