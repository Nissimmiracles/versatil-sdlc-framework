 ...
 */
export async function detectProfile(context, profileManager, logger) {
 ...
 */
export function detectProfileRuleBased(context) {
 ...
 */
export function suggestProfileUpgrade(currentProfile, recentToolCalls) {
    // If user is in coding profile but using many ML tools, suggest upgrade
    if (currentProfile === 'coding') {
        const mlToolCalls = recentToolCalls.filter(tool => tool.includes('embedding') ||
            tool.includes('rag') ||
        ...
        const testingToolCalls = recentToolCalls.filter(tool => tool.includes('test') ||
