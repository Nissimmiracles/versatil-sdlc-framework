...
export class MCPToolRouter extends EventEmitter {
     ...
     */
    async handlePlaywrightCall(request) {
     ...
     */
    async handleChromeCall(request) {
     ...
     */
    async handleGitHubCall(request) {
        const { action, params } = request;
        switch (action) {
            case 'create_issue':
                return await this.githubMCP.createIssue(params.owner, params.repo, params.issue);
            case 'list_issues':
                return await this.githubMCP.listIssues(params.owner, params.repo, params.options);
            case 'create_pr':
                return await this.githubMCP.createPullRequest(params.owner, params.repo, params.pr);
            case 'get_file':
                return await this.githubMCP.getFile(params.owner, params.repo, params.path, params.ref);
            case 'update_file':
                return await this.githubMCP.createOrUpdateFile(params.owner, params.repo, params.file);
            case 'search_code':
                return await this.githubMCP.searchCode(params.query, params.options);
            case 'get_repo':
                return await this.githubMCP.getRepository(params.owner, params.repo);
            case 'list_branches':
                return await this.githubMCP.listBranches(params.owner, params.repo);
            default:
     ...
     */
    async handleExaSearchCall(request) {
        const { action, params } = request;
        switch (action) {
            case 'search':
            ...
            case 'search_papers':
                return await this.exaMCP.searchPapers(params.query, params.options);
            case 'search_linkedin':
                return await this.exaMCP.searchLinkedIn(params.query, params.options);
            case 'search_with_content':
                return await this.exaMCP.searchWithContent(params.query, params.options);
            case 'find_similar':
                return await this.exaMCP.findSimilar(params.url, params.numResults);
            case 'get_contents':
     ...
     * Handle GitMCP calls
     */
    async handleGitMCPCall(request) {
        const { action, params } = request;
        switch (action) {
            case 'query_repo':
     ...
     */
    async handleShadcnCall(request) {
...
export function getMCPToolRouter() {
    if (!_routerInstance) {
        _routerInstance = new MCPToolRouter();
    }
    return _routerInstance;
}
//# sourceMappingURL=mcp-tool-router.js.map