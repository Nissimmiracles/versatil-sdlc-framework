...
export class PlaywrightStealthExecutor {
    constructor() {
        ...
        this.context = null;
        this.page = null;
        this.rateLimitDelay = 2000; // 2 seconds between requests (ethical scraping)
        this.lastRequestTime = 0;
    }
    /**
     * Execute stealth action with bot detection avoidance
     */
    async executeStealthAction(action, params = {}) {
    ...
    async scrapeDesignSystem(url) {
        ...
        try {
            ...
            const designSystem = await this.page.evaluate(() => {
                ...
                const getComputedStyles = (el) => window.getComputedStyle(el);
    ...
    async analyzeComponents(url) {
        ...
        try {
            ...
            console.log(`✅ Components analyzed: ${Object.keys(components).filter(k => components[k]).length} types found`);
            return {
                success: true,
                data: {
                    components,
    ...
    async benchmarkPerformance(url) {
        ...
        try {
            ...
            const resourceSizes = { js: 0, css: 0, images: 0, total: 0 };
            let requestCount = 0;
            ...
            this.page.on('response', async (response) => {
                ...
                if (status >= 400)
                    failedRequests++;
                if (status === 304 || response.fromCache?.())
                    cachedRequests++;
                try {
     ...
     */
    async extractAccessibility(url) {
        const startTime = Date.now();
        try {
            await this.launchBrowserIfNeeded();
    ...
    calculateAccessibilityScore(summary) {
        ...
        // Landmarks (20 points)
        if (summary.hasLandmarks)
            score += 20;
        // Skip link (20 points)
        if (summary.hasSkipLink)
            score += 20;
...
//# sourceMappingURL=playwright-stealth-executor.js.map