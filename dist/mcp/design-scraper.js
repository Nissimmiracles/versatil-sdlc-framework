...
export class DesignScraper {
     ...
     */
    async researchDesign(url, options = {}) {
        console.log(`🔬 JAMES (Frontend): Starting design research for ${url}`);
        const { includeDesignSystem = true, includeComponents = true, includeAccessibility = true, includePerformance = true } = options;
        const report = {
            url,
            timestamp: new Date(),
            recommendations: [],
        ...
        if (report.components) {
            ...
            for (const [type, data] of Object.entries(report.components)) {
                if (data && data.count) {
                    ...
                    markdown += `- Count: ${data.count}\n`;
                    markdown += `- Accessible: ${data.accessibility?.hasAriaLabel ? '✅' : '⚠️'}\n\n`;
                }
            }
        }
        // Accessibility
...
//# sourceMappingURL=design-scraper.js.map