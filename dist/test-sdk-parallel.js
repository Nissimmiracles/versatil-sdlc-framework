...
async function testSDKParallelExecution() {
    ...
    try {
        ...
        const hasRAG = Array.from(sdkResults.values()).some(r => r.result && typeof r.result === 'object' && 'sdkQuery' in r.result);
        ...
        const allSDK = Array.from(sdkResults.values()).every(r => r.executionMethod === 'Claude SDK');
