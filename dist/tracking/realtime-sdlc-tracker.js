...
export class RealTimeSDLCTracker extends EventEmitter {
    constructor(sdlcOrchestrator, operaCoordinator, agentRegistry, config) {
        super();
        this.tasks = new Map();
        this.contextHistory = [];
    ...
    createContextSnapshot() {
        ...
        this.agentRegistry.getAllAgents().forEach((agent) => {
            const agentId = agent.id;
            const activeTasks = Array.from(this.tasks.values()).filter(t => t.agentId === agentId && t.status === 'in-progress');
    ...
    calculateVelocity() {
        ...
        const completedTasks = Array.from(this.tasks.values()).filter(t => t.status === 'completed' && (t.actualCompletion || 0) > oneHourAgo);
    ...
    identifyRisks() {
        ...
        const stalledTasks = Array.from(this.tasks.values()).filter(t => t.status === 'in-progress' &&
     ...
     */
    updateTaskProgress(taskId, progress, estimatedCompletion) {
     ...
     */
    getHistory(from, to) {
        return this.contextHistory.filter(snapshot => snapshot.timestamp >= from && snapshot.timestamp <= to);
    }
    /**
    ...
    getActiveTasksForPhase(phaseId) {
        return Array.from(this.tasks.values()).filter(t => t.phase === phaseId && t.status === 'in-progress').length;
    }
    getCompletedTasksForPhase(phaseId) {
        return Array.from(this.tasks.values()).filter(t => t.phase === phaseId && t.status === 'completed').length;
    }
    ...
    getAgentPerformance(agentId) {
        const rates = this.completionRates.get(agentId) || [];
        const completedTasks = Array.from(this.tasks.values()).filter(t => t.agentId === agentId && t.status === 'completed');
    ...
    calculateVelocityTrend() {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            const tasks = Array.from(this.tasks.values()).filter(t => t.status === 'completed' &&
    ...
    calculateAverageTaskDuration() {
        const completedTasks = Array.from(this.tasks.values()).filter(t => t.status === 'completed' && t.startTime && t.actualCompletion);
    ...
    calculateSuccessRate() {
        const allTasks = Array.from(this.tasks.values()).filter(t => t.status === 'completed' || t.status === 'failed');
        if (allTasks.length === 0)
            return 100;
        const successfulTasks = allTasks.filter(t => t.status === 'completed');
        return (successfulTasks.length / allTasks.length) * 100;
    }
    updatePhaseMetrics(phaseId, duration) {
    ...
    async persistProgress(snapshot) {
...
//# sourceMappingURL=realtime-sdlc-tracker.js.map