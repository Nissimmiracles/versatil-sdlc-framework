    ...
    'vertex-ai': {
        ...
        credentials: [
            ...
            },
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'Service account key path is required';
    ...
    'n8n': {
        ...
        credentials: [
            ...
            },
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'n8n API key is required';
                    return true;
                },
        ...
        credentials: [
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'Figma API token is required';
    ...
    'docker-hub': {
        ...
        credentials: [
            ...
            },
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'Docker Hub token is required';
                    return true;
                },
        ...
        credentials: [
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'Confluence API token is required';
                    if (!value.startsWith('ATATT')) {
                        return 'Invalid token format (should start with "ATATT")';
                    }
                    return true;
            ...
            },
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'Confluence domain is required';
                    if (!value.includes('.atlassian.net')) {
        ...
        credentials: [
            {
                ...
                validation: (value) => {
                    if (!value)
                        return 'W&B API key is required';
                    return true;
                },
...
export function detectNeededServices(projectAnalysis) {
    ...
    if (projectAnalysis) {
        ...
        if (projectAnalysis.detectedTechnologies?.some(t => t.toLowerCase().includes('ai') ||
            t.toLowerCase().includes('ml') ||
            t.toLowerCase().includes('python')) || projectAnalysis.hasML) {
            needed.push(SERVICE_TEMPLATES['vertex-ai']);
            needed.push(SERVICE_TEMPLATES['huggingface']);
            needed.push(SERVICE_TEMPLATES['wandb']);
        }
        // Frontend integrations
        if (projectAnalysis.hasFrontend || projectAnalysis.detectedTechnologies?.some(t => ['react', 'vue', 'angular', 'frontend'].some(fw => t.toLowerCase().includes(fw)))) {
 ...
 */
export const AGENT_CREDENTIAL_MAP = {
 ...
 */
export function getServicesForAgents(agentIds) {
 ...
 */
export function getAgentsUsingService(serviceId) {
