...
import { SERVICE_TEMPLATES } from './credential-templates';
...
function detectCloudProvider() {
    ...
    if (fs.existsSync(gcpKeyPath))
        return 'gcp';
    if (fs.existsSync(awsKeyPath))
        return 'aws';
    if (fs.existsSync(supabaseKeyPath))
        return 'supabase';
 ...
 */
async function selectCloudProvider(rl, detected) {
 ...
 */
async function collectServiceCredentials(rl, service) {
 ...
 */
async function testDatabaseConnection(provider, credentials) {
 ...
 */
function generateSetupScript(provider, credentials) {
...
//# sourceMappingURL=ml-credential-wizard.js.map