...
`);
     ...
     */
    async checkSupabaseVectorCapability() {
        // Check for migrations or SQL files
        try {
            const migrations = await fs.readdir('supabase/migrations').catch(() => []);
            for (const migration of migrations) {
                const content = await fs.readFile(path.join('supabase/migrations', migration), 'utf8');
    ...
    async executeIntegration(plan) {
        ...
        await fs.writeFile('.versatil/integration-config.json', JSON.stringify(integrationConfig, null, 2));
...
`;
     ...
     */
    async generateMigrationScripts(plan) {
...
`;
            await fs.writeFile('.versatil/migrations/01_add_rag_memory.sql', ragMigration);
...
`;
        await fs.writeFile('.versatil/migrations/enhance-agents.js', agentEnhancement);
     ...
     */
    async generateIntegrationReport(plan) {
