...
export class TechStackDetector {
     ...
     */
    static async detectFromProject(projectPath) {
        const detectedFiles = [];
        const signals = [];
     ...
     */
    static analyzeSignals(signals, detectedFiles, projectPath) {
        if (signals.length === 0) {
            return {
                language: 'unknown',
                confidence: 0,
        ...
        const primaryLanguage = Object.keys(languageScores).reduce((a, b) => languageScores[a] > languageScores[b] ? a : b);
        ...
        const primaryFramework = Object.keys(frameworkScores).length > 0
            ? Object.keys(frameworkScores).reduce((a, b) => frameworkScores[a] > frameworkScores[b] ? a : b)
            : undefined;
     ...
     */
    static detectBackendStack(language, framework, confidence) {
     ...
     */
    static detectFrontendStack(framework, signals) {
     ...
     */
    static recommendSubAgents(backend, frontend) {
        const agents = [];
        // Backend sub-agents
        if (backend) {
...
TechStackDetector.FILE_INDICATORS = {
    // Node.js
