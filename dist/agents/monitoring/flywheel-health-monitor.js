...
export class FlywheelHealthMonitor extends EventEmitter {
    constructor(vectorStore, taskPlanManager) {
        super();
        this.monitoringInterval = null;
        this.metricsHistory = new Map();
        this.MONITORING_INTERVAL = 30000; // 30 seconds
        this.HISTORY_RETENTION = 100; // Keep last 100 data points per flywheel
        this.taskLevelMetrics = new Map(); // flywheel -> task metrics
     ...
     */
    async collectFlywheelHealth(agents, flywheelName) {
        try {
     ...
     */
    async calculateMomentum(patternCount, turns, successRate) {
        if (turns === 0)
            return 0;
     ...
     */
    async getPatternCount(flywheelName) {
        try {
            const patternsQuery = {
     ...
     */
    async enrichPatternLibrary(flywheelName) {
     ...
     */
    calculateOverallHealth(metrics, issues) {
     ...
     */
    async recordTaskLevelMetrics(plan) {
        try {
     ...
     */
    calculateTaskMetrics(flywheelName) {
...
//# sourceMappingURL=flywheel-health-monitor.js.map