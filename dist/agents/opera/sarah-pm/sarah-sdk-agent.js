...
export class SarahSDKAgent extends SDKAgentAdapter {
    ...
    detectProjectType(content) {
        if (content.match(/agile|scrum|sprint/i))
            return 'agile';
        if (content.match(/kanban/i))
            return 'kanban';
        if (content.match(/waterfall|phase/i))
            return 'waterfall';
        return 'unknown';
    }
    /**
     ...
     */
    detectSprintPhase(content) {
        if (content.match(/planning|kickoff/i))
            return 'planning';
        if (content.match(/development|active|in-progress/i))
            return 'development';
        if (content.match(/review|demo/i))
            return 'review';
        if (content.match(/retrospective|retro/i))
     ...
     */
    determinePMHandoffs(context, response) {
        const handoffs = [];
        const content = context.content || '';
     ...
     */
    generateSprintReport(sprintData) {
    ...
    assessRisks(riskData) {
        ...
        return {
            ...
            highPriority: risks.filter((r) => r.severity === 'high').length,
            mediumPriority: risks.filter((r) => r.severity === 'medium').length,
            lowPriority: risks.filter((r) => r.severity === 'low').length,
     ...
     */
    async generateAndStoreProjectVision(projectId, input) {
     ...
     */
    checkVisionAlignment(content, vision) {
        // Extract key concepts from content
        const contentLower = content.toLowerCase();
        // Check alignment with strategic priorities
        if (vision.strategicPriorities.length > 0) {
            const hasAlignment = vision.strategicPriorities.some(priority => contentLower.includes(priority.toLowerCase()));
            if (!hasAlignment) {
                return {
                    aligned: false,
        ...
        if (vision.scope.outOfScope.length > 0) {
            const isOutOfScope = vision.scope.outOfScope.some(item => contentLower.includes(item.toLowerCase()));
            if (isOutOfScope) {
                return {
                    aligned: false,
        ...
        if (vision.goals.length > 0) {
            const supportsGoal = vision.goals.some(goal => contentLower.includes(goal.description.toLowerCase().slice(0, 20)));
            if (!supportsGoal) {
                return {
     ...
     */
    async trackCompletionEvent(projectId, context, response) {
        try {
     ...
     */
    async updateGoalProgress(projectId, goalId, progress) {
     ...
     */
    async getProjectVisionContext(projectId) {
        const vision = await projectVisionManager.getVision(projectId);
...
//# sourceMappingURL=sarah-sdk-agent.js.map