...
`;
     ...
     */
    async enforceRouteRegistration(context) {
        const violations = [];
        const suggestions = [];
        ...
        const isPageComponent = filePath.match(/\/(pages|views|routes|screens)\/.*\.(tsx|jsx|vue|svelte)$/) &&
        ...
        if (!routeConfigContent) {
            ...
            suggestions.push('Create a route configuration file (e.g., src/App.tsx) to register routes');
        ...
        const isImported = routeConfigContent.includes(`from './${relativePathToComponent.replace(/\\/g, '/')}`) ||
