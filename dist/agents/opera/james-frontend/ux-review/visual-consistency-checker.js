...
export class VisualConsistencyChecker {
    ...
    async check(context) {
        ...
        const [tables, buttons, forms, spacing, typography, colors] = await Promise.all([
    ...
    generateTableRecommendations(issues) {
    ...
    generateButtonRecommendations(issues) {
        const recommendations = [];
    ...
    findNearestSpacingValue(value, scale) {
    ...
    async analyzeTypography(context) {
        ...
        for (const [filePath, content] of context.fileContents) {
            ...
            for (const match of matches) {
                ...
                const value = match[1];
                const isCompliant = Object.values(typographyTokens).some(token => token === value || token === this.pxToRem(value));
                if (isCompliant) {
                    compliantDeclarations++;
                }
                else {
    ...
    // COLOR ANALYSIS
    // ==========================================================================
    async analyzeColors(context) {
        const violations = [];
        const paletteUsageMap = new Map();
        ...
        for (const [filePath, content] of context.fileContents) {
            ...
            for (const match of hexMatches) {
                ...
                const paletteEntry = Object.entries(palette).find(([_, value]) => value.toLowerCase() === color.toLowerCase());
                if (paletteEntry) {
                    compliantDeclarations++;
    ...
    // ==========================================================================
    calculateTokenCompliance(analysis) {
        ...
        const overallCompliance = Math.round((colorsCompliance + spacingCompliance + typographyCompliance) / 3);
        return {
            colorsCompliance,
            spacingCompliance,
    ...
    generateSummary(overallScore, analysis, compliance) {
    ...
    getTopIssues(analysis) {
        const issues = [];
        if (analysis.tables.score < 70)
            issues.push('tables');
        if (analysis.buttons.score < 70)
            issues.push('buttons');
        if (analysis.forms.score < 70)
            issues.push('forms');
        if (analysis.spacing.score < 70)
            issues.push('spacing');
        if (analysis.typography.score < 70)
            issues.push('typography');
        if (analysis.colors.score < 70)
            issues.push('colors');
...
//# sourceMappingURL=visual-consistency-checker.js.map