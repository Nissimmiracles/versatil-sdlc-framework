...
export class MarkdownAnalyzer {
    ...
    async analyze(context) {
        ...
        const [headingHierarchy, listFormatting, codeBlocks, tables, links, images, readability] = await Promise.all([
    ...
    // ==========================================================================
    async analyzeHeadingHierarchy(files, context) {
        const structure = [];
        const violations = [];
    ...
    generateHeadingRecommendations(violations) {
    ...
    // ==========================================================================
    async analyzeListFormatting(files, context) {
        const violations = [];
    ...
    // CODE BLOCK ANALYSIS
    // ==========================================================================
    async analyzeCodeBlocks(files, context) {
        let totalBlocks = 0;
        let withLanguage = 0;
    ...
    // TABLE ANALYSIS
    // ==========================================================================
    async analyzeTables(files, context) {
        let totalTables = 0;
        const violations = [];
        for (const file of files) {
            const content = context.fileContents.get(file) || '';
    ...
    // LINK ANALYSIS
    // ==========================================================================
    async analyzeLinks(files, context) {
        let totalLinks = 0;
        let internal = 0;
        let external = 0;
        const brokenLinks = [];
    ...
    // IMAGE ANALYSIS
    // ==========================================================================
    async analyzeImages(files, context) {
        let totalImages = 0;
        let withAltText = 0;
    ...
    // ==========================================================================
    async analyzeReadability(files, context) {
        ...
        const score = Math.round(((optimalParagraphs / Math.max(totalParagraphs, 1)) * 50) +
    ...
    generateSummary(score, results) {
        const rating = score >= 90 ? 'Excellent' :
            score >= 70 ? 'Good' :
                score >= 50 ? 'Fair' : 'Poor';
        const issues = [];
        if (results.headingHierarchy.score < 70)
            issues.push('heading hierarchy');
        if (results.codeBlocks.score < 70)
            issues.push('code block formatting');
        if (results.images.score < 70)
            issues.push('image alt text');
...
//# sourceMappingURL=markdown-analyzer.js.map