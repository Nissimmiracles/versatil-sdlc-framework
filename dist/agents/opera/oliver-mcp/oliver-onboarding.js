...
import { ReorganizationEngine } from './reorganization-engine.js';
...
export class OliverOnboardingAgent extends EventEmitter {
     ...
     */
    async generateRecommendations(scanResult, gaps) {
     ...
     */
    async createReorganizationPlan(scanResult, gaps) {
     ...
     */
    async detectMigrationNeeds(scanResult) {
        return await this.migrator.detectMigration(scanResult);
    }
    /**
     * Calculate effort estimate
     */
    calculateEffortEstimate(gaps, reorganizationPlan, migrationStrategy) {
        const breakdown = {
    ...
    async generateProjectConfig() {
        ...
        scan.recommendations
            ...
            .forEach(r => {
            if (r.title.includes('Maria'))
                agents.push('maria-qa');
            if (r.title.includes('James'))
                agents.push('james-frontend');
            if (r.title.includes('Marcus'))
                agents.push('marcus-backend');
            if (r.title.includes('Sarah'))
                agents.push('sarah-pm');
            if (r.title.includes('Alex'))
                agents.push('alex-ba');
            if (r.title.includes('Dr.AI'))
                agents.push('dr-ai-ml');
        });
        ...
        const rules = [3, 4]; // Daily audit + onboarding always recommended
...
//# sourceMappingURL=oliver-onboarding.js.map