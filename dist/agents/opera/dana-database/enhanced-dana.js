...
export class EnhancedDana extends RAGEnabledAgent {
    constructor(vectorStore) {
        super(vectorStore);
        this.name = 'EnhancedDana';
        this.id = 'enhanced-dana';
        this.specialization = 'Database Architect & Data Layer Specialist';
        this.systemPrompt = 'Database architect specializing in PostgreSQL, Supabase, schema design, migrations, RLS policies, and vector databases for RAG systems';
    }
    /**
    ...
    generateEnhancedMessage(analysis, ragContext) {
        ...
        if (ragContext) {
            ...
            if (ragContext.similarCode.length > 0)
                ragInsights.push(`${ragContext.similarCode.length} similar schema patterns`);
            if (Object.keys(ragContext.previousSolutions).length > 0)
                ragInsights.push(`solutions for ${Object.keys(ragContext.previousSolutions).length} migration types`);
...
//# sourceMappingURL=enhanced-dana.js.map