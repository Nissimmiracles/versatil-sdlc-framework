...
export class QueryOptimizer {
    constructor() {
        this.TARGET_LATENCY_MS = 50; // Default target: < 50ms
        this.SLOW_QUERY_THRESHOLD = 100; // Warn if > 100ms
    }
    /**
    ...
    normalizeScanType(scanType) {
        ...
        if (lower.includes('index only'))
            return 'index_only_scan';
        if (lower.includes('index'))
            return 'index_scan';
        if (lower.includes('bitmap'))
            return 'bitmap_scan';
        return 'seq_scan';
    }
    /**
     ...
     */
    estimateLatencyFromSQL(sql, tables, joins, whereConditions) {
     ...
     */
    detectPerformanceIssues(sql, tables, joins, whereConditions, estimatedLatency) {
        const issues = [];
        const recommendations = [];
        const suggestedIndexes = [];
     ...
     */
    calculatePerformanceScore(estimatedLatency, targetLatency, issues) {
        let score = 100;
