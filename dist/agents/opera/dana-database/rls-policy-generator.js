...
export class RLSPolicyGenerator {
    ...
    async generate(options) {
        const { tableName, tenantColumn = 'user_id', userIdColumn = 'auth.uid()', policies = ['select', 'insert', 'update', 'delete'], rolePattern = 'user-owned' } = options;
        const policySQL = [];
        // Enable RLS on table
     ...
     */
    generateUserOwnedPolicies(tableName, tenantColumn, userIdColumn, policies) {
        const sql = [];
     ...
     */
    generateOrgOwnedPolicies(tableName, policies) {
        const sql = [];
        if (policies.includes('select')) {
     ...
     */
    generatePublicReadPolicies(tableName, tenantColumn, userIdColumn, policies) {
        const sql = [];
        if (policies.includes('select')) {
     ...
     */
    generateAdminOnlyPolicies(tableName, policies) {
        const sql = [];
     ...
     */
    detectTenantColumn(sql, tableName) {
     ...
     */
    calculateCompliance(sql, tableNames) {
        let score = 0;
...
//# sourceMappingURL=rls-policy-generator.js.map