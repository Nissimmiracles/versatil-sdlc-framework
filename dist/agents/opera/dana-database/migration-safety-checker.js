...
export class MigrationSafetyChecker {
    ...
    async validate(options) {
        ...
        const safetyScore = this.calculateSafetyScore(isIdempotent, hasRollback, dataIntegrityRisk, issues);
        return {
     ...
     */
    assessDataIntegrityRisk(sql, direction) {
     ...
     */
    getDataIntegritySuggestion(risk) {
        const suggestions = {
     ...
     */
    estimateDowntime(sql, targetEnv) {
        ...
        if (sql.match(/DROP TABLE/i))
            downtime += 2;
        if (sql.match(/ALTER TABLE.*ADD COLUMN/i))
            downtime += 5;
        if (sql.match(/ALTER TABLE.*DROP COLUMN/i))
            downtime += 10;
        if (sql.match(/CREATE INDEX/i))
            downtime += 30; // Can be slow on large tables
        if (sql.match(/ALTER COLUMN.*TYPE/i))
            downtime += 60; // Requires table rewrite
     ...
     */
    calculateSafetyScore(isIdempotent, hasRollback, dataIntegrityRisk, issues) {
        let score = 100;
        // Idempotency
        if (!isIdempotent)
            score -= 20;
...
//# sourceMappingURL=migration-safety-checker.js.map