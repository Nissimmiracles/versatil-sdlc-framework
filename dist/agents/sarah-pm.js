...
import { ParallelTaskManager, Priority, CollisionRisk } from '../orchestration/parallel-task-manager.js';
/**
...
export class SarahPM {
     ...
     */
    async generateExecutionWaves(todos) {
        ...
        while (remainingTasks.length > 0) {
            ...
            // Create wave
            const agents = Array.from(new Set(readyTasks.map(t => t.agentId).filter(Boolean)));
     ...
     */
    async planSprint(stories, durationDays) {
    ...
    identifyCoordinationCheckpoints(waves, tasks) {
        return waves
            .filter(w => w.blocking)
            .map(w => ({
    ...
    identifyCollisionRisks(waves, tasks) {
        const risks = [];
...
//# sourceMappingURL=sarah-pm.js.map