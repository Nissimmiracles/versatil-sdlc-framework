...
import { MCPSelectionEngine } from './mcp-selection-engine.js';
import { AntiHallucinationDetector } from './anti-hallucination-detector.js';
import { GitMCPQueryGenerator } from './gitmcp-query-generator.js';
// ============================================================================
// MCP Registry Schema
// ============================================================================
const MCP_REGISTRY = {
...
export class OliverMCPAgent extends BaseAgent {
    constructor(logger) {
        super();
        this.logger = logger;
        this.systemPrompt = 'Oliver-MCP: MCP Intelligence Officer - Intelligent MCP selection, classification, and anti-hallucination orchestration';
        this.name = 'Oliver-MCP';
        this.id = 'oliver-mcp';
        this.specialization = 'MCP Intelligence & Orchestration';
    ...
    async routeTask(request) {
        ...
        const hallucinationRisk = await this.hallucinationDetector.detectHallucinationRisk(`${request.name} ${request.description}`);
        ...
        if (hallucinationRisk.level === 'high' &&
            ...
            const detectedFramework = frameworks.find(fw => request.description.toLowerCase().includes(fw.toLowerCase())) || 'unknown';
            const topicKeywords = request.keywords?.filter(k => !frameworks.some(fw => fw.toLowerCase() === k.toLowerCase())) || [];
     ...
     */
    buildExecutionPlan(mcpName, request, gitMCPQuery) {
        const parameters = {};
     ...
     */
    getMCPsForAgent(agentId) {
        return Array.from(this.mcpRegistry.values()).filter(mcp => mcp.recommendedFor.includes(agentId) || mcp.recommendedFor.includes('all'));
    }
    /**
     * Track MCP usage
     */
    trackMCPUsage(mcpName) {
    ...
    selectBestCandidate(candidates, task) {
    ...
    generateMCPSuggestions(context) {
        const suggestions = [];
 ...
 * ```
 */
//# sourceMappingURL=oliver-mcp-orchestrator.js.map