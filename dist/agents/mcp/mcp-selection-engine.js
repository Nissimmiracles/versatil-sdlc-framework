...
export class MCPSelectionEngine extends EventEmitter {
     ...
     */
    detectTaskType(task, keywords) {
        let bestMatch = 'general';
        let bestScore = 0;
        for (const [taskType, pattern] of Object.entries(TASK_TYPE_PATTERNS)) {
            let score = 0;
            // Score based on keyword matches
            const matchedKeywords = pattern.keywords.filter(kw => keywords.includes(kw.toLowerCase()));
            score += matchedKeywords.length * 10;
            // Score based on file pattern matches
            if (task.files) {
                const matchedFiles = task.files.filter(file => pattern.filePatterns.some(p => this.matchesPattern(file, p)));
     ...
     */
    inferRequiredCapabilities(task, keywords) {
    ...
    assessComplexity(task) {
        ...
        if (task.files && task.files.length > 5)
            complexityScore += 2;
        else if (task.files && task.files.length > 2)
            complexityScore += 1;
     ...
     */
    generateRecommendations(taskAnalysis) {
        const recommendations = [];
        // Get primary recommendation from task type
        const taskTypePattern = TASK_TYPE_PATTERNS[taskAnalysis.taskType];
        if (taskTypePattern) {
        ...
        for (const [mcpName, capability] of Object.entries(MCP_CAPABILITIES)) {
            ...
            if (recommendations.some(r => r.mcpName === mcpName))
                continue;
            // Check if MCP capabilities match required capabilities
            const matchScore = this.calculateCapabilityMatch(capability, taskAnalysis.requiredCapabilities, taskAnalysis.keywords);
            if (matchScore > 30) {
                recommendations.push({
                    mcpName,
     ...
     */
    calculateCapabilityMatch(mcp, requiredCapabilities, keywords) {
        let score = 0;
...
//# sourceMappingURL=mcp-selection-engine.js.map