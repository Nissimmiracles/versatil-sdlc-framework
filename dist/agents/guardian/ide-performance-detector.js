...
export class IDEPerformanceDetector {
    ...
    async detectCrashRisk() {
        ...
        const crashRisk = this.calculateCrashRisk(missingFiles, totalIndexableSizeGB, memoryInfo.available_ram_gb, memoryInfo.current_memory_usage_percent);
        // Step 7: Calculate confidence
        const confidence = this.calculateConfidence(ideType, missingFiles.length, largeDirectories.length, memoryInfo.available_ram_gb);
        // Step 8: Generate recommendation
        const recommendation = this.generateRecommendation(ideType, crashRisk, missingFiles, totalIndexableSizeGB);
     ...
     */
    checkMissingIgnoreFiles(ideType) {
        const missingFiles = [];
    ...
    async getSystemMemoryInfo() {
        try {
            ...
            const { stdout: memPressure } = await execAsync('ps aux | awk \'{sum+=$4} END {print sum}\'');
            const currentMemoryUsagePercent = parseFloat(memPressure.trim()) || 0;
            // Try to get IDE memory usage
            let ideMemoryUsageMB;
            try {
                const { stdout: ideMemory } = await execAsync('ps aux | grep -iE "Cursor|Code|VSCode|idea" | grep -v grep | awk \'{sum+=$6} END {print sum}\'');
     ...
     */
    calculateCrashRisk(missingFiles, totalIndexableSizeGB, availableRamGB, currentMemoryUsagePercent) {
     ...
     */
    calculateConfidence(ideType, missingFilesCount, largeDirectoriesCount, availableRamGB) {
        let confidence = 0;
     ...
     */
    generateRecommendation(ideType, crashRisk, missingFiles, totalIndexableSizeGB) {
     ...
     */
    generateSuggestedFixes(ideType, missingFiles, largeDirectories) {
        const fixes = [];
...
//# sourceMappingURL=ide-performance-detector.js.map