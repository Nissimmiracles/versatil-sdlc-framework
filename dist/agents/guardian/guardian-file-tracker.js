 ...
 */
export async function trackFileEditForGuardian(event) {
 ...
 */
export async function markAgentActivated(filePath, agent, success, duration_ms, error) {
    const versatilHome = path.join(os.homedir(), '.versatil');
    const trackingFile = path.join(versatilHome, 'logs', 'guardian', 'file-edits.jsonl');
 ...
 */
export async function checkActivationFailures() {
    const versatilHome = path.join(os.homedir(), '.versatil');
    const trackingFile = path.join(versatilHome, 'logs', 'guardian', 'file-edits.jsonl');
    if (!fs.existsSync(trackingFile)) {
        return [];
...
//# sourceMappingURL=guardian-file-tracker.js.map