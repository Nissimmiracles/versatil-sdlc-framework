...
export class VersionManager {
     ...
     */
    validateVersionBump(currentVersion, newVersion, bumpType) {
     ...
     */
    async generateReleaseInfo(version) {
        try {
            // Get commits since last tag
            const { stdout: commits } = await execAsync(`git log $(git describe --tags --abbrev=0)..HEAD --oneline`, { cwd: this.frameworkRoot });
            ...
            // Get contributors
            const { stdout: contributorsStr } = await execAsync(`git log $(git describe --tags --abbrev=0)..HEAD --format='%an' | sort -u`, { cwd: this.frameworkRoot });
