...
import { classifyIssues, getLayerStatistics, LAYER_AGENT_ROUTING } from './layer-classifier.js';
import { verifyFrameworkIssue } from './framework-verifier.js';
import { verifyProjectIssue } from './project-verifier.js';
import { verifyContextIssue } from './context-verifier.js';
/**
 ...
 */
export async function detectAndVerifyIssues(healthCheckResult, workingDir, contextIdentity, userId, teamId, projectId) {
        ...
        try {
            ...
            switch (classification.layer) {
                ...
                case 'context':
                    ...
                    verificationResult = await verifyContextIssue(issue, workingDir, userId, teamId, projectId, resolvedContext);
                    break;
            }
...
function assignAgentByLayer(issue, layer, verificationResult, contextIdentity) {
    ...
    const layerRouting = LAYER_AGENT_ROUTING[layer];
 ...
 */
export async function createVerifiedTodos(verifiedIssues, outputDir) {
...
export class VerifiedIssueDetector {
    constructor() {
        ...
        this.verificationDepth = 'thorough';
        this.autoTODOCreation = true;
     ...
     */
    async detectAndVerify(issue, options) {
     ...
     */
    async performVerificationSteps(issue) {
        return [
    ...
    validateEvidenceQuality(evidence) {
        ...
        if (evidence.contentMatches)
            score += 25;
        if (evidence.symbolFound)
            score += 25;
        if (evidence.stackTraceValid)
            score += 25;
        return score;
    }
    /**
     ...
     */
    async createVerifiedTODO(issue) {
     ...
     */
    async detectIssuesInFiles(sources) {
        return sources
     ...
     */
    async verifyBatch(issues) {
        const results = await Promise.all(issues.map(issue => this.detectAndVerify(issue)));
     ...
     */
    async generateVerificationReport(issue) {
    ...
    setAutoTODOCreation(enabled) {
        this.autoTODOCreation = enabled;
    }
    isAutoTODOCreationEnabled() {
        return this.autoTODOCreation;
    }
}
//# sourceMappingURL=verified-issue-detector.js.map