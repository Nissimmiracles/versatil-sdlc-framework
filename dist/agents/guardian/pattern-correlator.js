...
export class PatternCorrelator {
     ...
     */
    async analyzePatterns(healthHistory) {
        ...
        const predictiveAlerts = this.generatePredictiveAlerts(degradationTrends, correlations);
     ...
     */
    extractMetricsTimeSeries(healthHistory) {
     ...
     */
    detectMetricCorrelations(metricsSeries) {
        const correlations = [];
        const metricPairs = [];
        ...
        for (const [metric1, metric2] of metricPairs) {
            ...
            const correlation = this.calculatePearsonCorrelation(series1.map(s => s.value), series2.map(s => s.value));
     ...
     */
    describeCorrelation(metric1, metric2, relationship) {
     ...
     */
    detectDegradationTrends(metricsSeries) {
        const trends = [];
     ...
     */
    calculateLinearRegression(series) {
     ...
     */
    generatePredictiveAlerts(degradationTrends, correlations) {
        const alerts = [];
...
//# sourceMappingURL=pattern-correlator.js.map