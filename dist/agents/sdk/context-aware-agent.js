 ...
 */
import { MEMORY_TOOL_CONFIG } from '../../memory/memory-tool-config.js';
 ...
 * ```
 */
export function createContextAwareAgent(agentId, baseAgent) {
    return {
        ...baseAgent,
 ...
 */
function enhancePromptWithMemory(agentId, basePrompt) {
...
export class AgentMemoryAPI {
     ...
     */
    async strReplace(filename, oldStr, newStr) {
        const fullPath = `${MEMORY_TOOL_CONFIG.agentMemoryPaths[this.agentId]}${filename}`;
        const result = await memoryToolHandler.execute({
            type: 'str_replace',
            path: fullPath,
     ...
     */
    async insert(filename, line, content) {
        const fullPath = `${MEMORY_TOOL_CONFIG.agentMemoryPaths[this.agentId]}${filename}`;
        const result = await memoryToolHandler.execute({
            type: 'insert',
            path: fullPath,
    ...
    async storePattern(pattern) {
        ...
        try {
            ...
            await this.strReplace(filename, '## Notes', `${patternMarkdown}\n\n## Notes`);
        }
        catch {
 ...
 */
function formatPatternAsMarkdown(pattern) {
 ...
 */
export async function initializeAgentMemories() {
 ...
 */
export function getAgentMemoryAPI(agentId) {
    return new AgentMemoryAPI(agentId);
}
//# sourceMappingURL=context-aware-agent.js.map