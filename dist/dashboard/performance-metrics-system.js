...
export class PerformanceMetricsSystem extends EventEmitter {
     ...
     */
    recordAgentActivation(agentId, task, context) {
        this.recordEvent({
     ...
     */
    recordModelSelection(agentId, modelId, reason, confidence, alternatives) {
        this.recordEvent({
     ...
     */
    recordTaskCompletion(agentId, task, success, duration, quality) {
        this.recordEvent({
     ...
     */
    recordDecision(agentId, decision, rationale, confidence, alternatives) {
        this.recordEvent({
            type: 'decision_made',
            agentId,
            details: {
     ...
     */
    recordError(agentId, error, context, severity = 'medium') {
        this.recordEvent({
            type: 'error_occurred',
            agentId,
            details: {
    ...
    generateInsights() {
        ...
        const topAgent = agents.reduce((top, agent) => agent.successRate > (top?.successRate || 0) ? agent : top, null);
        // Find most used model
        const topModel = models.reduce((top, model) => model.selectionCount > (top?.selectionCount || 0) ? model : top, null);
     ...
     */
    analyzeActivationReason(task, context) {
        if (task.type === 'testing')
            return 'quality_assurance_needed';
        if (task.type === 'frontend')
            return 'ui_development_required';
        if (task.type === 'backend')
            return 'api_development_required';
    ...
    updateSystemMetrics(event) {
        ...
        if (event.details.ruleType) {
            const ruleType = event.details.ruleType;
        ...
        try {
            ...
            if (parsed.agentMetrics) {
                for (const [id, metrics] of Object.entries(parsed.agentMetrics)) {
                    this.agentMetrics.set(id, metrics);
                }
            }
    ...
    getMetricsSummary() {
        ...
        const firstEvent = events.reduce((earliest, event) => !earliest || event.timestamp < earliest.timestamp ? event : earliest, null);
        const lastEvent = events.reduce((latest, event) => !latest || event.timestamp > latest.timestamp ? event : latest, null);
        return {
...
//# sourceMappingURL=performance-metrics-system.js.map