...
export function createDefaultPreClearHook(config = {}) {
    ...
    const { minTokensBeforeExtraction = 80000, // 80% of 100k threshold
...
export function createAdvancedPreClearHook(config = {}) {
    const logger = new VERSATILLogger();
    const { minTokensBeforeExtraction = 80000, priorityPatterns = [
        'authentication',
        'security',
        'performance',
        'database',
...
export function createEmergencyPreClearHook(config = {}) {
    ...
    return async (inputTokens, agentId) => {
        try {
            const EMERGENCY_THRESHOLD = 170000; // 85% of 200k context window
...
**Timestamp**: ${timestamp}
**Input Tokens**: ${inputTokens.toLocaleString()} / 200,000 (${((inputTokens / 200000) * 100).toFixed(1)}%)
**Agent**: ${targetAgentId}
**Priority**: CRITICAL
...
//# sourceMappingURL=default-pre-clear-hook.js.map