...
import { MEMORY_TOOL_CONFIG, getAllAgentIds } from './memory-tool-config.js';
/**
...
export class MemoryToolIntegration {
     ...
     */
    async loadAgentPatterns(agentId) {
        await this.ensureInitialized();
     ...
     */
    async clearContext(agentId, reason = 'manual') {
        await this.ensureInitialized();
 ...
 */
export async function viewMemory(agentId, path) {
 ...
 */
export async function createMemory(agentId, path, content) {
    const result = await memoryToolIntegration.execute(agentId, {
        type: 'create',
        path: `${agentId}/${path}`,
        content
 ...
 */
export async function replaceMemory(agentId, path, oldStr, newStr) {
    const result = await memoryToolIntegration.execute(agentId, {
        type: 'str_replace',
        path: `${agentId}/${path}`,
        oldStr,
 ...
 * @param path - Path relative to agent's memory directory
 */
export async function deleteMemory(agentId, path) {
    const result = await memoryToolIntegration.execute(agentId, {
        type: 'delete',
        path: `${agentId}/${path}`
    });
 ...
 */
export async function renameMemory(agentId, oldPath, newPath) {
    const result = await memoryToolIntegration.execute(agentId, {
        type: 'rename',
 ...
 */
export async function insertMemory(agentId, path, line, content) {
    const result = await memoryToolIntegration.execute(agentId, {
        type: 'insert',
        path: `${agentId}/${path}`,
        insertLine: line,
...
//# sourceMappingURL=memory-tool-integration.js.map