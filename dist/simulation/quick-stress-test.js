...
async function runQuickStressTest() {
    ...
    try {
        ...
        if (capabilityMatrix.blockers && capabilityMatrix.blockers.length > 0) {
            console.log('\n🚨 Critical Blockers:');
            capabilityMatrix.blockers.forEach((blocker) => console.log(`  - ${blocker}`));
        }
...
function generateLightweightMatrix() {
...
function generateQuickRecommendations(matrix) {
 ...
 */
async function runAgnosticDetection() {
    const issues = [];
...
//# sourceMappingURL=quick-stress-test.js.map