...
import type { AgentActivationContext } from '../dist/agents/core/base-agent.js';
 ...
 */
const SUB_AGENT_PATTERNS: Record<string, RegExp[]> = {
 ...
 */
function detectSubAgent(filePath: string, baseAgent: string): string | null {
  const normalizedPath = filePath.replace(/\\/g, '/');
...
async function activateAgent(
  ...
  content?: string,
  trigger?: string
): Promise<{ success: boolean; agent: string; message: string; metadata?: any }> {
  try {
    // Detect agent
 ...
 */
function formatStatusMessage(agentId: string, response: any): string {
