 ...
 */
async function extractPatterns(sessionData) {
    const patterns = [];
 ...
 */
async function storeToLocalStorage(patterns) {
 ...
 */
async function storeToRAG(patterns) {
    try {
 ...
 * Update search index
 */
async function updateSearchIndex(patterns) {
    ...
    for (const pattern of patterns) {
        ...
        if (!index.byCategory)
            index.byCategory = {};
        if (!index.byCategory[pattern.category])
            index.byCategory[pattern.category] = [];
        index.byCategory[pattern.category].push(pattern.id);
        // Index by agent
        if (!index.byAgent)
            index.byAgent = {};
        if (!index.byAgent[pattern.agent])
            index.byAgent[pattern.agent] = [];
