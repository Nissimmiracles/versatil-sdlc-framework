 ...
 */
function archiveWorkflow(state) {
    ensureStateDir();
    try {
        const historyEntry = {
            ...state,
        ...
        fs.appendFileSync(WORKFLOW_HISTORY_FILE, JSON.stringify(historyEntry) + '\n', 'utf-8');
 ...
 */
function loadWorkflowHistory(limit = 10) {
    try {
...
class VelocityCLI {
     ...
     * Phase 3: Delegate
     */
    async delegate(pattern, options = {}) {
        const state = loadWorkflowState();
        if (!state) {
            console.error('❌ No active workflow. Run `velocity plan <target>` first.\n');
            process.exit(1);
    ...
    async history(limit = 10) {
        ...
        for (const workflow of history) {
            const duration = workflow.completedAt - workflow.startTime;
