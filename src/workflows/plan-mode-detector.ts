...
export class PlanModeDetector {
  ...
  private complexityWeights: Record<ComplexityType, number> = {
    'multi-agent': 25,
    'long-horizon': 20,
    'complex-refactor': 18,
    'full-stack': 22,
  ...
  private activationThreshold = 60;
   ...
   */
  async detectPlanMode(context: TaskAnalysisContext): Promise<PlanModeDetection> {
    ...
    const detection: PlanModeDetection = {
      shouldActivate,
      confidence,
      reasoning,
   ...
   */
  private detectComplexityIndicators(context: TaskAnalysisContext): ComplexityIndicator[] {
    ...
    if (
      refactorKeywords.some(kw => request.includes(kw)) &&
    ...
    if (layerCount >= 2) {
      indicators.push({
        type: 'full-stack',
        description: `Full-stack feature (${layerCount} layers: ${
          [hasFrontend && 'Frontend', hasBackend && 'Backend', hasDatabase && 'Database']
            .filter(Boolean)
            .join(', ')
        })`,
    ...
    const phaseCount = request.split(/\n|[.,;]/).filter(line =>
      phaseKeywords.some(kw => line.includes(kw))
    ).length;
   ...
   */
  private countAgentRequirements(request: string, context: TaskAnalysisContext): number {
    let count = 0;
    const agentMap = {
   ...
   */
  private extractEvidenceForAgents(request: string): string {
    const matches = [];
   ...
   */
  private calculateConfidence(indicators: ComplexityIndicator[]): number {
  ...
  private identifyRequiredAgents(
    ...
    const typeToAgent: Record<ComplexityType, string[]> = {
   ...
   */
  private assessRiskLevel(indicators: ComplexityIndicator[]): 'low' | 'medium' | 'high' {
    const highRiskTypes: ComplexityType[] = [
      'security-critical',
      'high-risk',
      'database-migration',
      'full-stack',
    ...
    const mediumRiskTypes: ComplexityType[] = [
      'complex-refactor',
      'integration',
      'multi-phase',
