...
export class SessionReportGenerator {
  ...
  async generateReport(
    ...
    codificationResult: CodificationResult
...
`;
   ...
   */
  private generateBriefReport(analysis: SessionAnalysis, learnings: ExtractedLearnings): string {
    return `
╔═══════════════════════════════════════════════════════════╗
...
`;
   ...
   */
  private generateSummary(analysis: SessionAnalysis, learnings: ExtractedLearnings): SessionReport['summary'] {
   ...
   */
  private generateRecommendations(analysis: SessionAnalysis, learnings: ExtractedLearnings): string {
    const recs: string[] = [];
  ...
  private formatCodeChanges(changes: any[]): string {
    if (changes.length === 0) return '*No code changes tracked*';
   ...
   * Save report to disk
   */
  private async saveReport(sessionId: string, markdown: string): Promise<void> {
    try {
 ...
 * Factory function
 */
export function createSessionReportGenerator(): SessionReportGenerator {
