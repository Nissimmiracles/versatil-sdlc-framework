...
export class VelocityOrchestratorImpl {
   ...
   */
  async invokeWorkCommand(context: any): Promise<any> {
    ...
    try {
      ...
      if (fs.existsSync(codifyScript)) {
        ...
        const effortAccuracy = this.calculateEffortAccuracy(
          context.plan?.estimates?.total || 0,
          context.work?.actualDuration || 0
        );
   ...
   */
  private async queryRAGForSimilarFeatures(target: string): Promise<any> {
    try {
   ...
   */
  private generateTodosFromTarget(target: string): any[] {
   ...
   */
  private calculateEffortEstimates(ragContext: any, todos: any[]): any {
    ...
    if (ragContext.documents && ragContext.documents.length > 0) {
      const avgSimilarity = ragContext.documents.reduce((sum: number, d: any) =>
        sum + (d.metadata?.relevanceScore || 0), 0) / ragContext.documents.length;
   ...
   */
  private loadRelevantTemplates(target: string): string[] {
    const templates = [];

    if (target.match(/auth/i)) templates.push('auth-system');
    if (target.match(/crud|endpoint/i)) templates.push('crud-endpoint');
    if (target.match(/dashboard|analytics/i)) templates.push('dashboard');
    if (target.match(/upload|file/i)) templates.push('file-upload');
    if (target.match(/api.*integration|third.*party/i)) templates.push('api-integration');
   ...
   */
  private async checkFrameworkHealth(): Promise<any> {
    try {
   ...
   */
  private async runQuickBuild(): Promise<any> {
    try {
   ...
   */
  private calculateHealthScore(checks: Record<string, any>): number {
   ...
   */
  private detectAgentForTodo(todo: any): string {
    if (todo.agent) return todo.agent;

    const title = todo.title?.toLowerCase() || '';

    if (title.match(/api|backend|endpoint|server/)) return 'marcus-backend';
    if (title.match(/database|schema|migration|table/)) return 'dana-database';
    if (title.match(/ui|component|frontend|page|form/)) return 'james-frontend';
    if (title.match(/test|qa|coverage/)) return 'maria-qa';
    if (title.match(/requirements|story|acceptance/)) return 'alex-ba';
    if (title.match(/plan|milestone|release/)) return 'sarah-pm';
   ...
   */
  private calculateEffortAccuracy(estimatedHours: number, actualMs: number): number {
