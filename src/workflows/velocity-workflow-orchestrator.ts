...
export interface WorkflowContext {
  ...
  plan?: {
    todos: any[];
    estimates: { total: number; byPhase: Record<string, number> };
    templates: string[];
  ...
  delegation?: {
    assignments: Map<string, string[]>; // agent -> task IDs
...
export class VelocityWorkflowOrchestrator extends EventEmitter {
  ...
  private stateMachine: VelocityWorkflowStateMachine;
  private transitions: VelocityPhaseTransitions;
   ...
   */
  async executePlan(workflowId: string, context: WorkflowContext): Promise<PhaseExecutionResult> {
   ...
   */
  async executeAssess(workflowId: string, context: WorkflowContext): Promise<PhaseExecutionResult> {
    const startTime = Date.now();
   ...
   */
  async executeDelegate(workflowId: string, context: WorkflowContext): Promise<PhaseExecutionResult> {
    const startTime = Date.now();
   ...
   */
  async executeWork(workflowId: string, context: WorkflowContext): Promise<PhaseExecutionResult> {
    const startTime = Date.now();
  ...
  async executeCodify(workflowId: string, context: WorkflowContext): Promise<PhaseExecutionResult> {
    ...
    try {
      ...
      await this.stateMachine.transition(workflowId, 'Completed' as WorkflowPhase);
  ...
  private async autoTransitionToAssess(workflowId: string, context: WorkflowContext): Promise<void> {
  ...
  private async autoTransitionToDelegate(workflowId: string, context: WorkflowContext): Promise<void> {
  ...
  private async autoTransitionToWork(workflowId: string, context: WorkflowContext): Promise<void> {
  ...
  private async autoTransitionToCodify(workflowId: string, context: WorkflowContext): Promise<void> {
  ...
  private async invokePlanCommand(target: string): Promise<any> {
  ...
  private async invokeAssessCommand(target: string): Promise<any> {
  ...
  private async invokeDelegateCommand(todos: any[]): Promise<any> {
  ...
  private async invokeWorkCommand(target: string, delegation: any): Promise<any> {
  ...
  private async invokeLearnCommand(context: WorkflowContext): Promise<any> {
  ...
  private updateMetrics(workflowId: string, phase: WorkflowPhase, duration: number): void {
  ...
  private calculateCompoundingEffect(workflowId: string, context: WorkflowContext): void {
    const metrics = this.workflowMetrics.get(workflowId);
    if (!metrics || !context.plan || !context.work) return;
