...
export class VelocityPhaseDetector {
  ...
  private currentWorkflowFile: string;
  ...
  private detectManual(
    ...
    const phase = context.metadata?.phase as WorkflowPhase;
   ...
   */
  private isPlanningFile(filePath: string): boolean {
  ...
  getNextPhase(currentPhase: WorkflowPhase): WorkflowPhase | null {
    const phaseSequence: WorkflowPhase[] = ['Plan', 'Assess', 'Delegate', 'Work', 'Codify'];
 ...
 */
export function detectPhaseFromFileEdit(filePath: string): PhaseDetectionResult {
 ...
 */
export function detectPhaseFromCommand(commandName: string): PhaseDetectionResult {
  const detector = new VelocityPhaseDetector();
  return detector.detect({
    trigger: 'command',
    commandName,
 ...
 */
export function detectPhaseFromBuild(): PhaseDetectionResult {
  const detector = new VelocityPhaseDetector();
  return detector.detect({
    trigger: 'build',
  });
 ...
 */
export function detectPhaseFromSessionEnd(): PhaseDetectionResult {
  const detector = new VelocityPhaseDetector();
  return detector.detect({
    trigger: 'sessionEnd',
  });
