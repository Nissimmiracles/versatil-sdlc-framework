...
export class BrowserMonitor extends EventEmitter {
  ...
  private messageHistory: BrowserMessage[] = [];
  ...
  private handleMessage(data: string): void {
    try {
      const message: BrowserMessage = JSON.parse(data);
...
export class TerminalDashboard {
  ...
  private grid: any;
  private consoleLog: blessed.Widgets.Log;
  private networkTable: contrib.widget.Table;
  private errorLog: blessed.Widgets.Log;
  private statsBox: blessed.Widgets.BoxElement;

  constructor(private monitor: BrowserMonitor) {
    // Create screen
  ...
  private setupEventListeners(): void {
    ...
    this.monitor.on('message', (message: BrowserMessage) => {
      switch (message.type) {
        case 'console': {
          const data = message.data as ConsoleData;
        ...
        case 'network': {
          const data = message.data as NetworkData;
 ...
 */
export function startBrowserMonitor(): BrowserMonitor {
