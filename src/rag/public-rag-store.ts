...
export interface PatternInput {
  ...
  description?: string;
  code?: string;
  examples?: string[];  // For pattern examples/usage scenarios
  agent: string;
  category: string;
  effectiveness?: number;
  timeSaved?: number;
  tags?: string[];
  skipSanitization?: boolean;  // NEW: For internal use only
}
...
export class PublicRAGStore extends GraphRAGStore {
  private static instance: PublicRAGStore;
   ...
   */
  async addPattern(pattern: PatternInput): Promise<string> {
    ...
    switch (policyDecision.classification) {
      ...
      case PatternClassification.UNSANITIZABLE:
        throw new Error(
          `❌ REJECTED: Pattern is too project-specific\n` +
          `   Pattern: "${pattern.pattern}"\n` +
          `   Reason: ${policyDecision.reasoning.join('\n   ')}\n` +
          `   Action: Store in Private RAG only (NOT Public RAG)\n` +
      ...
      case PatternClassification.REQUIRES_SANITIZATION:
        ...
        if (!policyDecision.sanitizationResult || !policyDecision.sanitizationResult.sanitized) {
          throw new Error(
            `❌ Sanitization failed for pattern: "${pattern.pattern}"\n` +
   ...
   */
  private isPublicPattern(pattern: PatternInput): boolean {
  ...
  async query(query: GraphRAGQuery): Promise<GraphRAGResult[]> {
    ...
    const publicQuery: GraphRAGQuery = {
      ...query,
      includePublic: true,
  ...
  async getStats(): Promise<{
    ...
    const stats = {
      ...
      mostUsed: [] as PatternNode[],
      recentlyAdded: [] as PatternNode[]
    };
   ...
   */
  private setProjectConfig(config: { projectId: string; databaseId: string }) {
   ...
   */
  private async getAllPatterns(): Promise<PatternNode[]> {
