...
import { ProjectFingerprint, detectProjectFingerprint } from './project-detector.js';
...
export enum SanitizationLevel {
  ...
  REJECT = 'reject'                 // Cannot be sanitized, private-only
}

export enum SanitizationDecision {
  ...
  REJECT_CREDENTIALS = 'reject_credentials'              // Contains secrets/credentials
}
...
export class PatternSanitizer {
  private projectFingerprint: ProjectFingerprint | null = null;
  ...
  async sanitize(input: string): Promise<SanitizationResult> {
    ...
    const redactions: Array<{ type: string; original: string; redacted: string; reason: string }> = [];
    ...
    let decision: SanitizationDecision;
    let level: SanitizationLevel;
   ...
   */
  private checkSensitiveKeywords(input: string): {
   ...
   */
  private getPlaceholderForIdentifier(identifier: string): string {
    // GCP Project ID
   ...
   */
  async sanitizeBatch(inputs: string[]): Promise<SanitizationResult[]> {
    await this.initialize();
  ...
  getStats(results: SanitizationResult[]): {
    ...
    const rejected = results.filter(r =>
      r.decision === SanitizationDecision.REJECT_UNSANITIZABLE ||
      r.decision === SanitizationDecision.REJECT_BUSINESS_LOGIC ||
      r.decision === SanitizationDecision.REJECT_CREDENTIALS
    ).length;
...
let sanitizerInstance: PatternSanitizer | null = null;

export function getPatternSanitizer(): PatternSanitizer {
