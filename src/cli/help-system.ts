 ...
 */
export function parseHelpQuery(query: string): {
  topic: string;
  confidence: number;
  suggestions: string[];
} {
 ...
 */
export function getHelpContent(topic: string): HelpContent | null {
 ...
 */
function getHelpDatabase(): Record<string, HelpContent> {
  return {
    'quick-start': {
      topic: 'quick-start',
