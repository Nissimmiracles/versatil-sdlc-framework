...
export class VectorStoreMigration {
  ...
  private sourceStore: EnhancedVectorMemoryStore;
  private targetStore: SupabaseVectorStore;
   ...
   * Phase 1: Pre-migration validation
   */
  private async phase1PreValidation(options: MigrationOptions): Promise<void> {
   ...
   */
  private async phase2CreateBackup(options: MigrationOptions): Promise<void> {
   ...
   * Phase 3: Initialize target store
   */
  private async phase3InitializeTarget(options: MigrationOptions): Promise<void> {
   ...
   * Phase 4: Migrate memories and patterns
   */
  private async phase4MigrateMemories(options: MigrationOptions): Promise<void> {
    ...
    try {
      ...
      if (options.agentFilter?.length) {
        filteredMemories = memories.filter(m =>
          options.agentFilter!.includes(m.metadata.agentId)
        );
      }
   ...
   * Phase 5: Migrate agent solutions and interactions
   */
  private async phase5MigrateSolutions(options: MigrationOptions): Promise<void> {
    ...
    try {
      ...
      const memories = await this.sourceStore.getAllMemories();
      const solutionMemories = memories.filter(m =>
        m.metadata.tags?.includes('solution') ||
   ...
   * Phase 6: Post-migration validation
   */
  private async phase6PostValidation(options: MigrationOptions): Promise<void> {
   ...
   */
  private async convertMemoryToPattern(
   ...
   */
  private convertMemoryToSolution(memory: MemoryDocument): AgentSolution {
    return {
      agent: memory.metadata.agentId,
   ...
   */
  private convertMemoryToInteraction(memory: MemoryDocument): AgentInteraction {
    return {
      agent: memory.metadata.agentId,
      problemType: memory.metadata.tags?.find(t => t.includes('problem')) || 'general',
      problem: this.extractProblemFromContent(memory.content),
  ...
  private updateProgress(phase: string, step: number, processed: number): void {
  ...
  private getAgentBreakdown(memories: MemoryDocument[]): Record<string, number> {
  ...
  private getContentTypeBreakdown(memories: MemoryDocument[]): Record<string, number> {
    const breakdown: Record<string, number> = {};
    for (const memory of memories) {
  ...
  private inferPatternType(memory: MemoryDocument): string {
  ...
  private calculateQualityScore(memory: MemoryDocument): number {
    let score = 70; // Base score

    if (memory.metadata.tags?.length) score += 10;
    if (memory.metadata.language) score += 5;
    if (memory.metadata.framework) score += 5;
    if (memory.content.length > 100) score += 10;

    return Math.min(100, score);
  }

  private calculateEffectivenessScore(memory: MemoryDocument): number {
  ...
  private extractProblemFromContent(content: string): string {
  ...
  private async isDuplicatePattern(pattern: CodePattern): Promise<boolean> {
    try {
      const existing = await this.targetStore.retrieveSimilarPatterns(
  ...
  private generateRecommendations(): void {
    ...
    if (stats.duplicatesSkipped > 0) {
      this.report.recommendations.push(
        `${stats.duplicatesSkipped} duplicates were skipped. Consider cleaning up redundant patterns.`
      );
    }

    if (stats.preservedEmbeddings < stats.migratedPatterns * 0.8) {
      this.report.recommendations.push(
        'Many embeddings were not preserved. Consider regenerating embeddings for better search performance.'
      );
    }

    this.report.recommendations.push(
      'Test agent RAG functionality to ensure migration was successful.'
    );

    this.report.recommendations.push(
      'Consider updating agent configurations to use the new Supabase store.'
    );
  }
...
export function createMigration(): VectorStoreMigration {
