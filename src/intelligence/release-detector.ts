...
export class ReleaseDetector {
   ...
   */
  public async checkReleaseReadiness(): Promise<ReleaseReadinessCheck> {
   ...
   */
  public async detectFeatures(): Promise<FeatureDetection[]> {
    ...
    try {
      ...
      this.logger.debug('Failed to detect features', { error: (error as Error).message });
    }
   ...
   */
  private detectVersionBump(features: FeatureDetection[], stats: any): VersionBumpType {
   ...
   */
  private calculateNextVersion(current: string, bumpType: VersionBumpType): string {
  ...
  private getCurrentVersion(): string {
    try {
      const packageJson = JSON.parse(
        fs.readFileSync(path.join(this.cwd, 'package.json'), 'utf-8')
      );
  ...
  private async gatherStats() {
    ...
    try {
      ...
      this.logger.debug('Failed to gather stats', { error: (error as Error).message });
    }
   ...
   * Group files by feature (heuristic)
   */
  private groupFilesByFeature(files: string[]): Record<string, string[]> {
    const groups: Record<string, string[]> = { 'Miscellaneous': [] };
   ...
   */
  private detectFeatureType(files: string[]): FeatureDetection['type'] {
   ...
   */
  private calculateFeatureConfidence(files: string[]): number {
    let confidence = 50;

    // More files = higher confidence
    if (files.length >= 5) confidence += 20;
    else if (files.length >= 3) confidence += 10;
   ...
   */
  private calculateConfidence(reasons: number, blockers: number, features: FeatureDetection[]): number {
    let confidence = 50;
