...
import { UserInteractionLearner, type AnswerFormatPreferences, type DetailLevelPreferences } from './user-interaction-learner.js';
...
export class ContextResponseFormatter {
   ...
   */
  public formatResponse(rawAnswer: string, context: ResponseContext): string {
  ...
  private buildSection(
    ...
    prefs: AnswerFormatPreferences,
    detailPrefs: DetailLevelPreferences
  ): string | null {
    switch (type) {
      case 'direct_answer':
   ...
   */
  private buildDirectAnswer(context: ResponseContext): string {
   ...
   */
  private buildEvidence(context: ResponseContext, prefs: AnswerFormatPreferences): string | null {
    if (!context.verification_evidence) return null;

    const sections: string[] = ['🔍 **Verification Evidence**:'];
  ...
  private buildDetails(
    ...
    if (!context.implementation_details) return null;
   ...
   * Build table
   */
  private buildTable(context: ResponseContext): string {
    const sections: string[] = [];
   ...
   */
  private buildFilePaths(context: ResponseContext, prefs: AnswerFormatPreferences): string {
    if (!context.implementation_details) return '';
   ...
   */
  private buildActionItems(context: ResponseContext): string | null {
