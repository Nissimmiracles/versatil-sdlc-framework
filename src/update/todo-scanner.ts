...
export class TodoScanner {
   ...
   */
  async scanTodos(includeResolved: boolean = false): Promise<TodoSummary> {
    try {
      // Get all todo files
      ...
      console.warn('⚠️  Todo scan failed:', (error as Error).message);
   ...
   */
  private async parseTodoFile(filename: string): Promise<TodoItem | null> {
    try {
   ...
   */
  private extractCreatedDate(content: string): Date | null {
   ...
   */
  private extractAssignedAgent(content: string): string | null {
   ...
   */
  private extractEstimatedEffort(content: string): string | null {
   ...
   */
  private generateSummary(todos: TodoItem[]): TodoSummary {
   ...
   */
  private getEmptySummary(): TodoSummary {
    return {
      total: 0,
      pending: 0,
      resolved: 0,
   ...
   */
  async getTodoDetails(number: string): Promise<TodoItem | null> {
    try {
      const files = await fs.readdir(this.todosDir);
   ...
   */
  async getTodosByPriority(priority: 'p1' | 'p2' | 'p3' | 'p4'): Promise<TodoItem[]> {
    const summary = await this.scanTodos(false);
    const todos: TodoItem[] = [...summary.stale, ...summary.recent];
   ...
   * Get todos by agent
   */
  async getTodosByAgent(agentName: string): Promise<TodoItem[]> {
    try {
      const files = await fs.readdir(this.todosDir);
  ...
  formatRecommendations(summary: TodoSummary): string[] {
    ...
    if (summary.stale.length > 0) {
      recommendations.push(
        `Review ${summary.stale.length} stale todos (>${this.staleThresholdDays} days old): /resolve "${summary.stale.map(t => t.number).join('|')}"`
      );
    }

    if (summary.byPriority.p1 > 0) {
      recommendations.push(
        `Work on ${summary.byPriority.p1} critical (P1) todos: Start with ${summary.oldestPending?.filename || 'oldest item'}`
      );
    }

    if (summary.byPriority.p2 > 5) {
      recommendations.push(
        `High backlog: ${summary.byPriority.p2} P2 todos pending. Consider prioritizing or delegating.`
      );
    }
   ...
   */
  async exportSummaryToJSON(outputPath?: string): Promise<string> {
