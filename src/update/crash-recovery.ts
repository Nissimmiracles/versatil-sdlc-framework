...
export class CrashRecoveryManager {
   ...
   */
  async hasInterruptedUpdate(): Promise<boolean> {
    try {
   ...
   */
  async getInterruptedUpdateState(): Promise<UpdateState | null> {
   ...
   */
  async startUpdate(fromVersion: string, toVersion: string, steps: UpdateStep[]): Promise<string> {
   ...
   * Mark step as failed
   */
  async failStep(stepId: string, error: string, criticalFailure: boolean = false): Promise<void> {
    const state = await this.loadState();
    if (!state) return;
   ...
   * Abort update
   */
  async abortUpdate(reason: string): Promise<void> {
    const state = await this.loadState();
    if (!state) return;
   ...
   */
  async resumeUpdate(): Promise<RecoveryResult> {
    const state = await this.loadState();
   ...
   * Get remaining steps
   */
  async getRemainingSteps(): Promise<UpdateStep[]> {
    const state = await this.loadState();
    if (!state) return [];
   ...
   */
  async forceRemoveLock(): Promise<void> {
    await this.removeLock();
   ...
   */
  async validateCanProceed(): Promise<{ canProceed: boolean; reason?: string }> {
   ...
   * Get update progress
   */
  async getProgress(): Promise<{ percentage: number; currentStep: string; completedSteps: number }> {
    const state = await this.loadState();
   ...
   */
  async getUpdateSummary(): Promise<string> {
    const state = await this.loadState();
   ...
   * Load update state
   */
  private async loadState(): Promise<UpdateState | null> {
    try {
   ...
   * Create lock file
   */
  private async createLock(updateId: string): Promise<void> {
    await fs.mkdir(this.versatilHome, { recursive: true });
    await fs.writeFile(
      this.lockFile,
      JSON.stringify(
