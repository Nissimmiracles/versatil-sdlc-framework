...
export class VersionDiffGenerator {
   ...
   */
  async generateDiff(fromVersion: string, toVersion: string): Promise<VersionDiff> {
   ...
   */
  async generateCumulativeDiff(currentVersion: string, targetVersion: string): Promise<VersionDiff> {
  ...
  private parseChangelog(changelog: string): {
    ...
    let currentCategory: keyof typeof result = 'other';
   ...
   */
  private determineUpdateType(fromVersion: string, toVersion: string): 'major' | 'minor' | 'patch' | 'prerelease' {
   ...
   */
  private generateCumulativeReleaseNotes(releases: ReleaseInfo[]): string {
    const lines: string[] = [];
