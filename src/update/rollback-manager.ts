...
import { compareVersions } from './semantic-version';
...
export class RollbackManager {
  ...
  private rollbackHistoryFile: string;
  private maxRollbackPoints: number;

  constructor(maxRollbackPoints: number = 5) {
   ...
   */
  async createRollbackPoint(version: string, reason?: string): Promise<RollbackPoint> {
   ...
   */
  private async rollbackToPoint(point: RollbackPoint): Promise<boolean> {
    try {
   ...
   */
  async rollbackChain(count: number): Promise<boolean> {
  ...
  async emergencyRollback(): Promise<boolean> {
    ...
    try {
      ...
      console.error(`❌ Emergency rollback failed: ${(error as Error).message}`);
  ...
  async listRollbackPoints(): Promise<RollbackPoint[]> {
    try {
      ...
      const points: RollbackPoint[] = JSON.parse(data);
   ...
   */
  async validateUpdateHealth(): Promise<HealthCheckResult> {
   ...
   */
  async autoRollbackOnFailure<T>(updateFn: () => Promise<T>): Promise<T> {
    const rollbackPoints = await this.listRollbackPoints();
   ...
   */
  private async checkFrameworkFiles(): Promise<HealthCheck> {
    try {
   ...
   */
  private async checkTests(): Promise<HealthCheck> {
   ...
   * Record rollback in history
   */
  private async recordRollback(point: RollbackPoint): Promise<void> {
   ...
   */
  async getRollbackPointSize(point: RollbackPoint): Promise<number> {
    try {
   ...
   */
  async getTotalRollbackStorage(): Promise<number> {
    const points = await this.listRollbackPoints();
    let total = 0;
