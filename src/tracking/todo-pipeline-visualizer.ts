...
export interface TodoPipeline {
  ...
  criticalPath: string[]; // Task IDs on critical path (longest dependency chain)
...
export class TodoPipelineVisualizer {
   ...
   */
  private parseTodoFile(filename: string, content: string): TodoTask[] {
    const tasks: TodoTask[] = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (match) {
        ...
        tasks.push({
          ...
          priority: filePriority as TodoTask['priority'],
          assignedAgent,
          estimatedMinutes,
          dependsOn,
  ...
  private extractTags(filename: string, description: string): string[] {
    ...
    // From filename
    if (filename.includes('backend')) tags.push('backend');
    if (filename.includes('frontend')) tags.push('frontend');
    if (filename.includes('database')) tags.push('database');
    if (filename.includes('api')) tags.push('api');
    if (filename.includes('ui')) tags.push('ui');

    // From description (lowercase)
    const lowerDesc = description.toLowerCase();
    if (lowerDesc.includes('test')) tags.push('testing');
    if (lowerDesc.includes('security')) tags.push('security');
    if (lowerDesc.includes('performance')) tags.push('performance');
    if (lowerDesc.includes('accessibility')) tags.push('accessibility');
   ...
   */
  async analyzePipeline(): Promise<TodoPipeline> {
   ...
   * Build dependency graph
   */
  private buildDependencyGraph(tasks: TodoTask[]): Map<string, Set<string>> {
  ...
  private findCriticalPath(
    ...
    const findPath = (taskId: string, currentPath: string[] = []): void => {
      ...
      const duration = newPath.reduce(
        (sum, id) => sum + (taskMap.get(id)?.estimatedMinutes || 0),
        0
      );
   ...
   */
  async generateGanttChart(width: number = 80): Promise<GanttChart> {
    ...
    const timeScale: GanttChart['timeScale'] =
      maxMinutes > 1440 ? 'days' : maxMinutes > 120 ? 'hours' : 'minutes';
    ...
    const timeHeader = this.generateTimelineHeader(
      pipeline.totalEstimatedMinutes,
      timelineWidth,
      timeScale
    );
   ...
   */
  private generateTimelineBar(task: TodoTask, width: number, totalMinutes: number): string {
   ...
   * Group tasks by dependency level
   */
  private groupTasksByLevel(tasks: TodoTask[]): TodoTask[][] {
    const levels: TodoTask[][] = [];

    const getLevel = (task: TodoTask, visited = new Set<string>()): number => {
   ...
   * Get status icon
   */
  private getStatusIcon(status: TodoTask['status']): string {
    const icons = {
      completed: '✅',
      in_progress: '🔄',
      pending: '⏸️',
   ...
   * Get priority icon
   */
  private getPriorityIcon(priority: TodoTask['priority']): string {
    const icons = {
      high: '🔴',
      medium: '🟡',
      low: '🟢'
...
let _visualizerInstance: TodoPipelineVisualizer | null = null;

export function getTodoPipelineVisualizer(projectRoot?: string): TodoPipelineVisualizer {
