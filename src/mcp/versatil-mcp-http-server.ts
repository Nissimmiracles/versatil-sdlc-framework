...
import { VERSATILMCPServerV2, VERSATILMCPConfig } from './versatil-mcp-server-v2.js';
...
export class VERSATILMCPHTTPServer {
  ...
  private mcpServer: VERSATILMCPServerV2;
    ...
    return new Promise((resolve, reject) => {
      try {
        this.server = this.app.listen(this.config.port, this.config.host, () => {
          this.logger.info(
            `[MCP HTTP] VERSATIL MCP Server listening on http://${this.config.host}:${this.config.port}`
          );
...
export async function createHTTPServer(
  mcpConfig: VERSATILMCPConfig,
  httpConfig?: Partial<HTTPServerConfig>
): Promise<VERSATILMCPHTTPServer> {
