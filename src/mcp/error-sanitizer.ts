 ...
 */
function extractSafeDetails(error: any): Record<string, unknown> | undefined {
 ...
 */
export function sanitizeError(error: unknown, customCode?: ErrorCode): SanitizedError {
 ...
 */
export function logSanitizedError(error: unknown, context?: string): void {
  const sanitized = sanitizeError(error);
