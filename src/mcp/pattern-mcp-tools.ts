  ...
  for (const file of patternFiles) {
    ...
    try {
      ...
      queryTokens.forEach(token => {
        if (allText.includes(token)) matchScore += 10;
      });

      // Boost for keyword exact matches
      pattern.keywords.forEach(kw => {
        if (queryTokens.includes(kw.toLowerCase())) matchScore += 30;
      });
...
export async function applyPattern(patternName: string, options?: {
  dryRun?: boolean;
  outputDir?: string;
}): Promise<{
  success: boolean;
  ...
  const pattern = JSON.parse(readFileSync(filePath, 'utf-8')) as Pattern;
...
export async function setupWebSocket(options?: {
  ...
  auth?: boolean;
  rooms?: boolean;
}): Promise<{ success: boolean; message: string; next_steps: string[] }> {
...
export async function generateTelemetryReport(format: 'console' | 'json' | 'markdown' = 'console'): Promise<{
  success: boolean;
