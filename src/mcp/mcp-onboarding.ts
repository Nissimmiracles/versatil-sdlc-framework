...
export class MCPOnboarding {
  ...
  async checkOnboardingStatus(): Promise<OnboardingStatus> {
    ...
    if (!hasFrameworkHome) missingComponents.push('Framework directory (~/.versatil/)');
    if (!hasPreferences) missingComponents.push('Preferences file');
    if (!hasEnvFile) missingComponents.push('Environment configuration');
   ...
   */
  async runAutoOnboarding(): Promise<OnboardingResult> {
 ...
 */
let mcpOnboardingInstance: MCPOnboarding | null = null;

export function getMCPOnboarding(): MCPOnboarding {
