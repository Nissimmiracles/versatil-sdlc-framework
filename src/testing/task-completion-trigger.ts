...
export class TaskCompletionTrigger extends EventEmitter {
  ...
  private testSelector: SmartTestSelector;
  private activeTasks = new Map<string, TaskCompletionEvent>();
  private testQueue: TaskCompletionEvent[] = [];
   ...
   */
  private async handleTodoStatusChange(todo: Todo, previousStatus: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[TaskCompletionTrigger] Queued tests for task ${todo.id}: ` +
   ...
   */
  private async getChangedFilesForTask(todo: Todo): Promise<string[]> {
   ...
   */
  private getTestTypesForFiles(files: string[]): TestType[] {
   ...
   */
  private getResponsibleAgentForFiles(files: string[]): string {
   ...
   */
  private estimateTestDuration(files: string[]): number {
   ...
   */
  private async processNextTest(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[TaskCompletionTrigger] Processing test for task ${event.todo.id}...`
      );
   ...
   */
  private async executeTests(event: TaskCompletionEvent): Promise<TestExecutionResult> {
    const startTime = Date.now();

    try {
      // Get smart test selection
      const testSelection = await this.testSelector.selectTests(
        event.changedFiles,
        event.testTypes
      );

      console.log(
        `[TaskCompletionTrigger] Running ${testSelection.all.length} tests ` +
   ...
   */
  private buildTestCommand(testFiles: string[], testTypes: TestType[]): string {
    // Base command
