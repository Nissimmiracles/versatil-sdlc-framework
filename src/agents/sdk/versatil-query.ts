...
import { query, AgentDefinition, type Query } from '@anthropic-ai/claude-agent-sdk';
import { Task, TaskType, TaskExecution, ExecutionStatus } from '../../orchestration/parallel-task-manager.js';
...
export async function executeWithSDK(
...
): Promise<Map<string, SDKExecutionResult>> {
  ...
  try {
    ...
    const errorResults = new Map<string, SDKExecutionResult>();
 ...
 */
async function buildAgentDefinitions(
  tasks: Task[],
  config: SDKExecutionConfig
 ...
 */
function buildAgentDescription(task: Task): string {
...
async function buildAgentPrompt(
  ...
  if (config.ragContext) {
    ...
    try {
      const ragContext = await config.vectorStore.searchMemories(
        `${task.name} ${task.type} ${task.metadata?.component || ''}`,
        5
      );
 ...
 */
async function buildMasterPrompt(config: SDKExecutionConfig): Promise<string> {
 ...
 */
function getSDLCPhaseInstructions(phase: string): string {
...
function getToolsForTaskType(taskType: TaskType): string[] {
  const toolMap: Record<TaskType, string[]> = {
    ...
    [TaskType.BUILD]: ['Read', 'Bash', 'Glob'],
    [TaskType.DEPLOYMENT]: ['Read', 'Bash', 'WebFetch'],
    [TaskType.QUALITY_ASSURANCE]: ['Read', 'Grep', 'Glob', 'Bash'],
    [TaskType.ANALYSIS]: ['Read', 'Grep', 'Glob'],
    [TaskType.MONITORING]: ['Read', 'Bash', 'WebFetch'],
    [TaskType.SECURITY]: ['Read', 'Bash', 'Grep', 'WebFetch']
  };
 ...
 */
function getDefaultMCPTools(): string[] {
  return [
 ...
 */
function getVersatilSystemPrompt(): string {
  return `
 ...
 */
async function processSDKResults(
  sdkQuery: Query,
  tasks: Task[],
  startTime: number
): Promise<Map<string, SDKExecutionResult>> {

  const results = new Map<string, SDKExecutionResult>();
...
export async function executeBatchTasks(
  tasks: Task[],
  config?: Partial<SDKExecutionConfig>
): Promise<Map<string, SDKExecutionResult>> {
