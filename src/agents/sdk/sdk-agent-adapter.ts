...
import type { ToolCallRequest, ToolCallResponse } from '../../mcp/mcp-tool-router.js';
...
export interface SDKAgentConfig {
  ...
  enableMCPRouting?: boolean; // NEW v6.1: Enable MCP tool routing
}
...
export class SDKAgentAdapter {
  ...
  protected enableMCPRouting: boolean; // NEW v6.1
   ...
   */
  protected async getRAGContext(context: AgentActivationContext): Promise<any> {
    if (!this.vectorStore) {
      return {
        similarCode: [],
        previousSolutions: {},
  ...
  protected buildEnhancedPrompt(context: AgentActivationContext, ragContext: any): string {
    ...
    if (ragContext.totalDocuments > 0) {
      ...
      if (ragContext.similarCode.length > 0) {
        ...
        ragContext.similarCode.forEach((code: any, idx: number) => {
      ...
      if (Object.keys(ragContext.previousSolutions).length > 0) {
        ...
        Object.entries(ragContext.previousSolutions).forEach(([issueType, solution]: [string, any]) => {
      ...
      if (ragContext.projectStandards.length > 0) {
        ...
        ragContext.projectStandards.forEach((standard: any) => {
  ...
  protected extractKeywords(content: string): string[] {
    ...
    // Common patterns
    if (content.includes('test') || content.includes('describe')) keywords.push('testing');
    if (content.includes('component')) keywords.push('component');
    if (content.includes('api') || content.includes('route')) keywords.push('api');
    if (content.includes('security') || content.includes('auth')) keywords.push('security');
    if (content.includes('performance') || content.includes('optimize')) keywords.push('performance');
   ...
   */
  protected async storeInteraction(context: AgentActivationContext, response: AgentResponse): Promise<void> {
   ...
   */
  protected async handleToolCall(toolCall: any): Promise<any> {
