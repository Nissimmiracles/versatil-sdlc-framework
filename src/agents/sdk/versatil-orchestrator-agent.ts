...
import { FlywheelHealth, HealthDashboard } from '../monitoring/flywheel-health-monitor.js';
...
export interface OrchestratorConfig {
  vectorStore: EnhancedVectorMemoryStore;  // Required for monitoring agents
  monitoring?: {
  ...
  ragIntegration?: {
    ...
    retrievalThreshold: number;  // 0-1 similarity score
  };
}
...
export interface PhaseContext {
  ...
  triggers: string[];  // What indicated this phase
  timestamp: number;
}
...
export class VersatilOrchestratorAgent extends EventEmitter {
  ...
  private flywheelMonitor: FlywheelMonitoringTask | null = null;
  private contextMonitor: ContextMonitoringTask | null = null;
  private evolutionTask: EvolutionBackgroundTask | null = null;

  // Flywheel management
  private flywheels: Map<SDLCPhase, FlywheelState> = new Map();
   ...
   */
  private initializeFlywheels(): void {
    const phases: SDLCPhase[] = ['requirements', 'design', 'development', 'testing', 'deployment', 'evolution'];
   ...
   */
  private setupFlywheelMonitorListeners(): void {
    if (!this.flywheelMonitor) return;

    this.flywheelMonitor.on('dashboard-updated', (dashboard: HealthDashboard) => {
      // Update flywheel momentum from health monitor
      for (const [flywheel, metrics] of Object.entries(dashboard.flywheels)) {
        const flywheelState = this.flywheels.get(flywheel as SDLCPhase);
        if (flywheelState) {
          flywheelState.momentum = (metrics as FlywheelHealth).momentum;
        }
      }
      this.updateMetrics();
    });
   ...
   */
  private setupContextMonitorListeners(): void {
    if (!this.contextMonitor) return;
   ...
   */
  private setupEvolutionTaskListeners(): void {
    if (!this.evolutionTask) return;
  ...
  async detectPhase(context: {
    ...
    const scores: Map<SDLCPhase, number> = new Map();
    ...
    let maxScore = 0;
    let detectedPhase: SDLCPhase = 'development';  // Default
   ...
   */
  async routeTask(task: Omit<AgentTask, 'id' | 'status'>): Promise<AgentTask> {
    // Generate task ID
  ...
  private updateTaskMetrics(task: AgentTask, success: boolean): void {
    const flywheel = this.flywheels.get(task.phase);
    if (!flywheel || !task.startTime || !task.endTime) return;
  ...
  private updateMetrics(): void {
    ...
    this.status.metrics.totalTasksCompleted = Array.from(this.flywheels.values()).reduce(
      (sum, fw) => sum + fw.metrics.tasksCompleted,
      0
    );
  ...
  private notifyUser(notification: {
    ...
    if (!this.config.userCommunication.notificationsEnabled) return;
   ...
   */
  async activateFlywheel(phase: SDLCPhase): Promise<void> {
   ...
   */
  async deactivateFlywheel(phase: SDLCPhase): Promise<void> {
    const flywheel = this.flywheels.get(phase);
    if (!flywheel) {
      throw new Error(`Unknown phase: ${phase}`);
    }
