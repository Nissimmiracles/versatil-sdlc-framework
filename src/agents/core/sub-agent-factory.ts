...
export interface SubAgentConfig {
  ...
  epicId: string;
  priority: number;
  maxConcurrency?: number; // Max parallel tasks this sub-agent can handle
}
...
export class SubAgentFactory extends EventEmitter {
  ...
  private mcpExecutor: MCPTaskExecutor;
  private agentPool: AgentPool;

  private subAgents: Map<string, SubAgentInstance> = new Map();
  private activeAgentInstances: Map<string, BaseAgent> = new Map(); // Track borrowed agent instances
  private taskQueue: Map<SubAgent['type'], Task[]> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    for (const type of Object.keys(this.MAX_SUBAGENTS_PER_TYPE) as SubAgent['type'][]) {
   ...
   */
  async createSubAgent(config: SubAgentConfig): Promise<SubAgentInstance> {
   ...
   */
  async executeTask(subAgentId: string): Promise<void> {
   ...
   */
  private async performAgentWork(subAgent: SubAgentInstance, mcpResult: any): Promise<void> {
   ...
   */
  private mapPriorityToUrgency(priority: number): 'low' | 'medium' | 'high' {
   ...
   */
  private updateAverageTaskDuration(subAgent: SubAgentInstance, duration: number): void {
   ...
   */
  private updateOverallAverageTaskDuration(duration: number): void {
   ...
   */
  private queueTask(config: SubAgentConfig): void {
   ...
   */
  private async processQueuedTasks(type: SubAgent['type']): Promise<void> {
   ...
   */
  private getActiveSubAgentCount(type: SubAgent['type']): number {
    return Array.from(this.subAgents.values()).filter(sa =>
      sa.type === type && (sa.status === 'pending' || sa.status === 'running')
    ).length;
  }
