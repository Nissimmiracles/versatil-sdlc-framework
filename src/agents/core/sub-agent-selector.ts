...
export class SubAgentSelector {
  private static techStackCache: Map<string, { result: TechStackResult; timestamp: number }> = new Map();
  private static readonly CACHE_TTL = 5 * 60 * 1000; // 5 minutes

  /**
   * Select optimal sub-agent based on file and context
   */
  static async selectSubAgent(
    filePath: string,
    content: string,
    projectPath: string
  ): Promise<SubAgentSelection> {
   ...
   */
  static async selectBackendSubAgent(
    filePath: string,
    content: string,
    projectPath: string
  ): Promise<SubAgentSelection> {
   ...
   */
  static async selectFrontendSubAgent(
    filePath: string,
    content: string,
    projectPath: string
  ): Promise<SubAgentSelection> {
   ...
   */
  private static async getProjectStack(projectPath: string): Promise<TechStackResult> {
   ...
   */
  private static getBaseAgentId(subAgentId: string): 'marcus-backend' | 'james-frontend' {
   ...
   */
  private static isBackendAgent(subAgentId: string): boolean {
   ...
   */
  private static isFrontendAgent(subAgentId: string): boolean {
   ...
   */
  private static fallbackBackendSelection(filePath: string, content: string): SubAgentSelection {
   ...
   */
  private static fallbackFrontendSelection(filePath: string, content: string): SubAgentSelection {
