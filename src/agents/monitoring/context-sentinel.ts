...
export class ContextSentinel extends EventEmitter {
  ...
  private readonly MAX_TOKENS = 200000; // Claude Sonnet 4
  private readonly USABLE_TOKENS = 180000; // 20K buffer
  private readonly EMERGENCY_THRESHOLD = 0.85; // 85% triggers emergency
  private readonly MONITORING_INTERVAL = 5000; // 5 seconds (real-time)
  private readonly HISTORY_RETENTION = 60; // Keep last 5 minutes (60 * 5s)
   ...
   */
  async runContextCheck(): Promise<ContextDashboard> {
    try {
      ...
      // 7. Create dashboard
      const dashboard: ContextDashboard = {
        usage,
        risks,
        integrityStatus,
        recommendations,
   ...
   */
  private async measureContextUsage(): Promise<ContextUsage> {
   ...
   */
  private estimateWastePercentage(): number {
   ...
   */
  private storeUsageHistory(usage: ContextUsage): void {
   ...
   */
  private detectContextRisks(usage: ContextUsage): ContextRisk[] {
   ...
   */
  private async emergencyCompaction(usage: ContextUsage): Promise<CompactionResult> {
    ...
    let pausedFlywheel: string | null = null;
...
Tokens Reclaimed: ${tokensReclaimed.toLocaleString()}
   ...
   */
  private async observationMasking(): Promise<number> {
   ...
   */
  private async hierarchicalSummarization(): Promise<number> {
   ...
   */
  private async pauseLowestPriorityFlywheel(): Promise<string> {
   ...
   */
  private async validateContextIntegrity(): Promise<IntegrityCheck> {
    const lostInformation: string[] = [];
   ...
   */
  private generateRecommendations(usage: ContextUsage, risks: ContextRisk[]): string[] {
   ...
   */
  async getCurrentContext(): Promise<ContextDashboard> {
   ...
   */
  getUsageTrend(): { increasing: boolean; rate: number } {
