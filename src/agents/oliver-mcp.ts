...
import { MCPToolRouter, type ToolCallRequest, type ToolCallResponse } from '../mcp/mcp-tool-router.js';
import { MCPHealthMonitor, type MCPHealth, type MCPExecutionResult } from '../mcp/mcp-health-monitor.js';
...
export interface RoutingResult {
  ...
  reasoning: string;
  alternatives: Array<{ server: string; score: number }>;
}

export interface EnvironmentValidation {
  nodejs: { installed: boolean; version?: string; meetsRequirements: boolean };
  npm: { installed: boolean; version?: string; meetsRequirements: boolean };
  git: { installed: boolean; version?: string; meetsRequirements: boolean };
  mcpServers: Array<{
    name: string;
    installed: boolean;
    healthy: boolean;
...
export class OliverMCP {
  private static instance: OliverMCP;
  ...
  async routeQuery(query: string): Promise<RoutingResult> {
    ...
    if (
      lowerQuery.includes('orchestration') ||
   ...
   */
  async executeToolCall(toolCall: ToolCallRequest): Promise<MCPToolCallResult> {
  ...
  async validateEnvironment(): Promise<EnvironmentValidation> {
    ...
    if (unhealthyServers.length > 0) {
      warnings.push(
        `${unhealthyServers.length} MCP server(s) unhealthy: ${unhealthyServers.map(s => s.name).join(', ')}`
      );
    }
   ...
   */
  async getAllHealthStatuses(): Promise<MCPHealth[]> {
    const statuses: MCPHealth[] = [];
    const mcpIds = [
      'chrome_mcp',
      'playwright_mcp',
      'github_mcp',
  ...
  private checkNodeJS(): { installed: boolean; version?: string; meetsRequirements: boolean } {
    try {
  ...
  private checkNPM(): { installed: boolean; version?: string; meetsRequirements: boolean } {
    try {
  ...
  private checkGit(): { installed: boolean; version?: string; meetsRequirements: boolean } {
    try {
  ...
  private async checkMCPServers(): Promise<
    Array<{
      name: string;
      installed: boolean;
      healthy: boolean;
