...
export interface SchemaValidationOptions {
  ...
  strictMode?: boolean; // Enforce strict validation (3NF, all constraints)
}
...
export class SchemaValidator {
   ...
   */
  async validate(options: SchemaValidationOptions): Promise<SchemaValidationResult> {
    const issues: SchemaValidationResult['issues'] = [];
    const recommendations: string[] = [];
    const tables: SchemaValidationResult['tables'] = [];
   ...
   */
  private parseSQLTables(sql: string): SchemaValidationResult['tables'] {
    const tables: SchemaValidationResult['tables'] = [];
   ...
   */
  private parseColumns(columnsDef: string): SchemaValidationResult['tables'][0]['columns'] {
    const columns: SchemaValidationResult['tables'][0]['columns'] = [];
   ...
   */
  private parseConstraints(columnsDef: string): SchemaValidationResult['tables'][0]['constraints'] {
    const constraints: SchemaValidationResult['tables'][0]['constraints'] = [];
    const lines = columnsDef.split(',').map(l => l.trim());
   ...
   */
  private parseIndexes(sql: string, tableName: string): SchemaValidationResult['tables'][0]['indexes'] {
    const indexes: SchemaValidationResult['tables'][0]['indexes'] = [];
   ...
   */
  private validateNormalization(tables: SchemaValidationResult['tables']): {
    level: SchemaValidationResult['normalizationLevel'];
    issues: SchemaValidationResult['issues'];
    recommendations: string[];
  } {
    const issues: SchemaValidationResult['issues'] = [];
    const recommendations: string[] = [];
    let level: SchemaValidationResult['normalizationLevel'] = '3NF';
   ...
   */
  private detectRepeatingGroups(table: SchemaValidationResult['tables'][0]): string[] {
   ...
   */
  private detectPartialDependencies(table: SchemaValidationResult['tables'][0]): string[] {
   ...
   */
  private detectTransitiveDependencies(table: SchemaValidationResult['tables'][0]): string[] {
   ...
   */
  private validateConstraints(tables: SchemaValidationResult['tables']): {
    issues: SchemaValidationResult['issues'];
    recommendations: string[];
  } {
    const issues: SchemaValidationResult['issues'] = [];
   ...
   */
  private checkMissingIndexes(tables: SchemaValidationResult['tables']): {
    issues: SchemaValidationResult['issues'];
    recommendations: string[];
  } {
    const issues: SchemaValidationResult['issues'] = [];
   ...
   */
  private detectAntiPatterns(tables: SchemaValidationResult['tables'], sql: string): {
    issues: SchemaValidationResult['issues'];
    recommendations: string[];
  } {
    const issues: SchemaValidationResult['issues'] = [];
    ...
    for (const table of tables) {
      ...
      const enumColumns = table.columns.filter(c =>
        c.name.includes('status') || c.name.includes('type') || c.name.includes('state')
      );
   ...
   */
  private calculateSchemaScore(issues: SchemaValidationResult['issues']): number {
    let score = 100;
