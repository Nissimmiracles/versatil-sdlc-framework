...
export interface ScanResult {
  projectType: string;  // 'web', 'mobile', 'backend', 'fullstack', 'ml', 'library'
...
export interface TestingAnalysis {
  ...
  testQuality: number;  // 0-100 based on assertion density, test structure
}
...
export interface DocumentationAnalysis {
  ...
  missingDocs: string[];  // Functions/classes without documentation
}
...
export interface SecurityAnalysis {
  ...
  exposedSecrets: string[];  // Files with potential secrets
  insecurePatterns: string[];  // SQL injection, XSS, etc.
}
...
export interface StructureAnalysis {
  ...
  issues: string[];  // Flat structure, inconsistent naming, etc.
}

export interface PerformanceAnalysis {
  score: number;  // 0-100
  slowFiles: string[];  // Large files, complex components
...
export class ProjectScanner {
  ...
  private async analyzeTechStack(): Promise<TechStackAnalysis> {
    ...
    try {
      ...
      // Frontend Frameworks
      if (allDeps.react) { detected.push('React'); frameworks.push('React'); }
      if (allDeps.next) { detected.push('Next.js'); frameworks.push('Next.js'); }
      if (allDeps.vue) { detected.push('Vue'); frameworks.push('Vue'); }
      if (allDeps.nuxt) { detected.push('Nuxt'); frameworks.push('Nuxt'); }
      if (allDeps['@angular/core']) { detected.push('Angular'); frameworks.push('Angular'); }
      if (allDeps.svelte) { detected.push('Svelte'); frameworks.push('Svelte'); }

      // Backend Frameworks
      if (allDeps.express) { detected.push('Express'); frameworks.push('Express'); }
      if (allDeps['@nestjs/core']) { detected.push('NestJS'); frameworks.push('NestJS'); }
      if (allDeps.fastify) { detected.push('Fastify'); frameworks.push('Fastify'); }
      if (allDeps.koa) { detected.push('Koa'); frameworks.push('Koa'); }

      // Languages
      if (allDeps.typescript) { detected.push('TypeScript'); languages.push('TypeScript'); }
      else { languages.push('JavaScript'); detected.push('JavaScript'); }

      // Build Tools
      if (allDeps.webpack) { detected.push('Webpack'); buildTools.push('Webpack'); }
      if (allDeps.vite) { detected.push('Vite'); buildTools.push('Vite'); }
      if (allDeps.rollup) { detected.push('Rollup'); buildTools.push('Rollup'); }
      if (allDeps.esbuild) { detected.push('esbuild'); buildTools.push('esbuild'); }
      if (allDeps.turbopack) { detected.push('Turbopack'); buildTools.push('Turbopack'); }

      // Databases
      if (allDeps.pg || allDeps.postgres) { detected.push('PostgreSQL'); databases.push('PostgreSQL'); }
      if (allDeps.mysql || allDeps.mysql2) { detected.push('MySQL'); databases.push('MySQL'); }
      if (allDeps.mongodb || allDeps.mongoose) { detected.push('MongoDB'); databases.push('MongoDB'); }
      if (allDeps.redis || allDeps.ioredis) { detected.push('Redis'); databases.push('Redis'); }
      if (allDeps['@prisma/client']) { detected.push('Prisma'); databases.push('Prisma'); }
      if (allDeps['drizzle-orm']) { detected.push('Drizzle'); databases.push('Drizzle'); }
      ...
      // Testing Frameworks
      if (allDeps.jest) { detected.push('Jest'); testing.push('Jest'); }
      if (allDeps.mocha) { detected.push('Mocha'); testing.push('Mocha'); }
      if (allDeps.vitest) { detected.push('Vitest'); testing.push('Vitest'); }
      if (allDeps['@playwright/test']) { detected.push('Playwright'); testing.push('Playwright'); }
      if (allDeps.cypress) { detected.push('Cypress'); testing.push('Cypress'); }
    } catch (error) {
    ...
    if (pythonFiles.length > 0) {
      ...
      try {
        ...
        if (requirements.includes('django')) { detected.push('Django'); frameworks.push('Django'); }
        if (requirements.includes('flask')) { detected.push('Flask'); frameworks.push('Flask'); }
        if (requirements.includes('fastapi')) { detected.push('FastAPI'); frameworks.push('FastAPI'); }
        if (requirements.includes('pytest')) { testing.push('Pytest'); }
      } catch (error) {
        // No requirements.txt
      }
    }
  ...
  private async analyzeStructure(): Promise<StructureAnalysis> {
    ...
    const issues: string[] = [];
    if (!hasSourceDir) issues.push('No src/ directory - flat structure detected');
    if (!hasTestDir) issues.push('No dedicated test directory');
    if (!hasDocsDir) issues.push('No docs/ directory');
   ...
   */
  private async analyzeAccessibility(): Promise<AccessibilityAnalysis | undefined> {
   ...
   * Analyze complexity
   */
  private async analyzeComplexity(): Promise<ComplexityAnalysis> {
    ...
    if (allFiles.length > 500) complexityScore += 40;
    else if (allFiles.length > 100) complexityScore += 20;
    else complexityScore += 10;

    if (totalLines > 50000) complexityScore += 40;
    else if (totalLines > 10000) complexityScore += 20;
    else complexityScore += 10;

    if (avgLinesPerFile > 300) complexityScore += 20;
    else if (avgLinesPerFile > 150) complexityScore += 10;
