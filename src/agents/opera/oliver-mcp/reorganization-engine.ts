...
export class ReorganizationEngine {
  ...
  async createPlan(scanResult: ScanResult): Promise<ReorganizationPlan> {
   ...
   */
  async applyPlan(plan: ReorganizationPlan): Promise<void> {
  ...
  async applyPlanSafe(
    ...
    const opts: Required<ApplyPlanOptions> = {
   ...
   */
  private async applyAction(action: ReorganizationAction): Promise<{ success: boolean; error?: Error }> {
    try {
   ...
   */
  private async validateAction(action: ReorganizationAction): Promise<boolean> {
    try {
