...
export class JamesReact extends EnhancedJames {
  name = 'James-React';
  id = 'james-react';
  specialization = 'React 18+ Frontend Specialist';
  systemPrompt = `You are James-React, a specialized React frontend expert with deep knowledge of:
...
- Testing with React Testing Library`;
  ...
  public async analyzeReactPatterns(context: AgentActivationContext): Promise<{
    ...
    const bestPractices: ReactBestPractices = {
      hooksPatterns: [],
      componentStructure: [],
   ...
   */
  private hasInlineObjectsInJSX(content: string): boolean {
   ...
   */
  private hasProperPropTypes(content: string): boolean {
   ...
   */
  private hasMissingKeys(content: string): boolean {
   ...
   */
  private hasComplexLocalState(content: string): boolean {
   ...
   */
  private usesTanStackQuery(content: string): boolean {
   ...
   */
  private hasProperQueryKeys(content: string): boolean {
   ...
   */
  private hasLoadingErrorStates(content: string): boolean {
  ...
  detectReactVersion(content: string): string {
    if (content.includes('createRoot')) return 'React 18+';
    if (content.includes('ReactDOM.render')) return 'React 17 or earlier';
    if (content.includes('Suspense') && content.includes('Concurrent')) return 'React 18+ (Concurrent Mode)';
  ...
  detectStateManagement(content: string): string {
    if (content.includes('useContext') && content.includes('Provider')) return 'Context API';
    if (content.includes('zustand')) return 'Zustand';
    if (content.includes('jotai')) return 'Jotai';
    if (content.includes('redux')) return 'Redux Toolkit';
    if (content.includes('recoil')) return 'Recoil';
