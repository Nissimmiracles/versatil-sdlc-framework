...
export class UXReportGenerator {
  ...
  generatePriorityRoadmap(
    ...
    const criticalItems = this.createRoadmapItems(
      issues.filter(i => i.severity === 'critical'),
      recommendations.filter(r => r.type === 'immediate')
    );

    // Priority 2: High priority issues
    const highItems = this.createRoadmapItems(
      issues.filter(i => i.severity === 'high'),
      recommendations.filter(r => r.type === 'systematic' && r.expectedImpact === 'high')
    );

    // Priority 3: Medium priority improvements
    const mediumItems = this.createRoadmapItems(
      issues.filter(i => i.severity === 'medium'),
      recommendations.filter(r => r.type === 'systematic' && r.expectedImpact === 'medium')
    );

    // Priority 4: Low priority enhancements
    const lowItems = this.createRoadmapItems(
      issues.filter(i => i.severity === 'low'),
      recommendations.filter(r => r.type === 'enhancement')
    );
  ...
  private generateMarkdownReport(data: UXReportData, options: ExportOptions): string {
  ...
  private generateMarkdownHeader(data: UXReportData): string {
...
${this.getScoreDescription(data.overallScore)}`;
  ...
  private generateExecutiveSummary(data: UXReportData): string {
...
${this.generateKeyFindings(data)}`;
  ...
  private generateKeyFindings(data: UXReportData): string {
  ...
  private generateStrengthsSection(strengths: string[]): string {
    return `## ✅ What's Working Well

${strengths.map(s => `- ✅ ${s}`).join('\n')}`;
  }

  private generateCriticalIssuesSection(issues: UXIssue[], options: ExportOptions): string {
  ...
  private generateRecommendationsSection(recommendations: UXRecommendation[], options: ExportOptions): string {
  ...
  private formatRecommendations(recommendations: UXRecommendation[], options: ExportOptions): string {
  ...
  private generateRoadmapSection(roadmap: PriorityRoadmap): string {
  ...
  private generateDetailedAnalysis(data: UXReportData): string {
  ...
  private generateMarkdownFooter(data: UXReportData): string {
    return `## ✅ Next Steps
...
*Generation Time: ${data.timestamp.toISOString()}*`;
  ...
  private generateJSONReport(data: UXReportData): string {
    const roadmap = this.generatePriorityRoadmap(data.criticalIssues, data.recommendations);
  ...
  private generateHTMLReport(data: UXReportData, options: ExportOptions): string {
...
<body>
  ...
  private createRoadmapItems(issues: UXIssue[], recommendations: UXRecommendation[]): RoadmapItem[] {
  ...
  private calculateTotalTime(items: RoadmapItem[]): string {
  ...
  private estimateTimeFromEffort(effort: string): string {
    ...
    return estimates[effort as keyof typeof estimates] || '1 hour';
  }

  private estimateTimeFromSeverity(severity: string): string {
    const estimates = {
      'critical': '2-4 hours',
      'high': '1-2 hours',
      'medium': '1 hour',
    ...
    return estimates[severity as keyof typeof estimates] || '1 hour';
  }
  ...
  private formatEffort(effort: string): string {
    ...
    return formatted[effort as keyof typeof formatted] || effort;
  }
  ...
  private countBySeverity(issues: UXIssue[]): Record<string, number> {
  ...
  private countByType(recommendations: UXRecommendation[]): Record<string, number> {
