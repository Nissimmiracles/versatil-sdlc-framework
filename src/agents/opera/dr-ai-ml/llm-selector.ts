...
export class LLMSelector extends EventEmitter {
   ...
   */
  async recommendLLM(requirements: TaskRequirements): Promise<ModelRecommendation> {
    console.log(`[LLMSelector] 🔍 Finding best model for ${requirements.taskType} task...`);

    const candidates = Array.from(this.models.values());
    const scores: Array<{ model: LLMModel; score: number; reasons: string[] }> = [];
    ...
    let vertexAIConfig: VertexAIConfig | undefined;
  ...
  private scoreModel(
  ...
  ): { model: LLMModel; score: number; reasons: string[] } {
    let score = 0;
    ...
    if (requirements.budgetConstraint) {
      ...
      if (model.performance.cost === 'low') reasons.push('cost-effective');
    }
    ...
    if (requirements.qualityRequirement) {
      ...
      if (model.performance.quality >= 95) reasons.push('top-tier quality');
    }
    ...
    if (requirements.speedRequirement) {
      ...
      if (model.performance.speed === 'fast') reasons.push('fast inference');
    }
   ...
   */
  private getVertexAIConfig(model: LLMModel): VertexAIConfig {
    const projectId = process.env.VERTEX_AI_PROJECT_ID || 'versatil-framework';
    const location = process.env.VERTEX_AI_LOCATION || 'us-central1';

    const multimodalCapabilities: string[] = [];
    if (model.capabilities.vision) multimodalCapabilities.push('vision');
    if (model.capabilities.audio) multimodalCapabilities.push('audio');
    if (model.capabilities.codeGeneration) multimodalCapabilities.push('code');
  ...
  compareModels(modelIds: string[]): {
    ...
    const models = modelIds.map(id => this.models.get(id)).filter(Boolean) as LLMModel[];
    ...
    const comparison = {
      ...
      capabilities: models.map(m => ({
        name: m.name,
        capabilities: Object.keys(m.capabilities).filter(k => m.capabilities[k as keyof typeof m.capabilities])
      }))
    };
...
// Export singleton
let _selectorInstance: LLMSelector | null = null;

export function getLLMSelector(): LLMSelector {
