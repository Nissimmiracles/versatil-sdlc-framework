...
export interface FeedbackAgentConfig {
  enabled: boolean;
  sentimentThreshold: number; // Only process feedback with sentiment below this (e.g., -0.3)
  minFeedbackForPR: number; // Minimum feedback items to cluster into PR (default: 3)
  autoSubmitPR: boolean; // If false, creates draft PRs for manual review
  collectionInterval: number; // How often to check for feedback (ms)
...
export class GitPRFeedbackAgent extends EventEmitter {
  ...
  private submittedPRs: Map<string, string> = new Map(); // prId -> githubUrl
  private collectionTimer?: NodeJS.Timeout;
  private feedbackClusters: Map<string, FeedbackCluster> = new Map();
   ...
   */
  async collectFeedback(feedback: Omit<UserFeedback, 'id' | 'timestamp' | 'sentiment' | 'sentimentScore'>): Promise<UserFeedback> {
    // Analyze sentiment
   ...
   */
  private async analyzeSentiment(feedback: Partial<UserFeedback>): Promise<{ sentiment: UserFeedback['sentiment']; score: number }> {
    let score = 0;
    let negativeSignals = 0;
    let positiveSignals = 0;

    // Analyze context for negative indicators
    if (!feedback.context?.success) negativeSignals += 2;
    if (feedback.context?.errorMessage) negativeSignals += 2;
    if ((feedback.context?.retryCount || 0) > 1) negativeSignals += 1;
    if ((feedback.context?.duration || 0) > 30000) negativeSignals += 1; // >30s
    ...
    if (score <= -0.7) sentiment = 'critical';
   ...
   */
  private async clusterFeedback(feedback: UserFeedback[]): Promise<FeedbackCluster[]> {
   ...
   */
  private async createPRForCluster(cluster: FeedbackCluster): Promise<void> {
   ...
   */
  private generateProposalFile(cluster: FeedbackCluster): string {
...
`;
   ...
   * Generate PR body
   */
  private generatePRBody(cluster: FeedbackCluster): string {
...
`;
   ...
   */
  private generateSentimentBreakdown(cluster: FeedbackCluster): string {
    const counts = {
      critical: 0,
      negative: 0,
      neutral: 0,
   ...
   */
  private async storeFeedbackInRAG(feedback: UserFeedback): Promise<void> {
  ...
  private mapSentimentToPriority(sentiment: UserFeedback['sentiment']): FeedbackCluster['priority'] {
    switch (sentiment) {
  ...
  private inferComponent(feedback: UserFeedback): string {
    // Infer component from agent, file path, or keywords
    if (feedback.agentId) return feedback.agentId;
  ...
  private generateClusterTitle(feedback: UserFeedback): string {
  ...
  private async generateClusterDescription(cluster: FeedbackCluster): Promise<string> {
    const uniqueErrors = new Set(
      cluster.feedbackItems
...
let _feedbackAgentInstance: GitPRFeedbackAgent | null = null;

export function getGitPRFeedbackAgent(config?: FeedbackAgentConfig): GitPRFeedbackAgent {
