...
export class ActivationValidator {
  ...
  static validateFilePattern(agentId: string, filePath: string): boolean {
    const patterns = this.FILE_PATTERN_TRIGGERS[agentId as keyof typeof this.FILE_PATTERN_TRIGGERS];
  ...
  static validateCodePattern(agentId: string, content: string): boolean {
    const patterns = this.CODE_PATTERN_TRIGGERS[agentId as keyof typeof this.CODE_PATTERN_TRIGGERS];
   ...
   */
  static async validateActivation(test: TriggerTest): Promise<TriggerValidationResult> {
   ...
   */
  static async validateSubAgentRouting(test: TriggerTest, projectPath: string): Promise<TriggerValidationResult> {
    const startTime = Date.now();

    let actualSubAgent: string | undefined;
   ...
   */
  static async runBatchTests(tests: TriggerTest[], projectPath?: string): Promise<{
    totalTests: number;
    passedTests: number;
    failedTests: number;
    accuracy: number;
    ...
    const results: TriggerValidationResult[] = [];
  ...
  static getAgentTriggers(agentId: string): {
    ...
    return {
      filePatterns: this.FILE_PATTERN_TRIGGERS[agentId as keyof typeof this.FILE_PATTERN_TRIGGERS] || [],
      codePatterns: this.CODE_PATTERN_TRIGGERS[agentId as keyof typeof this.CODE_PATTERN_TRIGGERS] || []
    };
  }
}

// Export sample tests for each agent
export const SAMPLE_VALIDATION_TESTS: Record<string, TriggerTest[]> = {
  'maria-qa': [
    {
      testId: 'maria-test-1',
      agentId: 'maria-qa',
