...
export interface FrameworkInfo {
  ...
  docsPath?: string; // e.g., "docs/tutorial"
  lastKnownVersion?: string;
  releaseFrequency: 'high' | 'medium' | 'low'; // How often framework updates
  knowledgeCutoffRisk: 'high' | 'medium' | 'low'; // Risk of outdated knowledge
}
 ...
 */
export const FRAMEWORK_KNOWLEDGE_BASE: Record<string, FrameworkInfo> = {
  // Python Frameworks
  'fastapi': {
    name: 'FastAPI',
...
export class AntiHallucinationDetector extends EventEmitter {
  ...
  private readonly KNOWLEDGE_CUTOFF_DATE = new Date('2025-01-01');
   ...
   */
  async detectHallucinationRisk(query: string, context?: any): Promise<HallucinationRisk> {
   ...
   */
  private extractFrameworkMentions(query: string): FrameworkInfo[] {
    const queryLower = query.toLowerCase();
    const mentioned: FrameworkInfo[] = [];
   ...
   */
  private calculateFrameworkRisk(framework: FrameworkInfo): HallucinationRisk {
    let score = 0;
    let reasoning = '';
   ...
   */
  private inferDocumentationPath(query: string, framework: FrameworkInfo): string | undefined {
    const queryLower = query.toLowerCase();
   ...
   */
  private formatGitMCPQuery(framework: FrameworkInfo, specificPath?: string): string {
    if (specificPath) {
   ...
   */
  private getDaysSinceCutoff(): number {
    const now = new Date();
   ...
   */
  private getRiskEmoji(level: HallucinationRisk['level']): string {
    switch (level) {
      case 'high': return '🔴';
      case 'medium': return '🟡';
      case 'low': return '🟢';
  ...
  hasFrameworkInfo(frameworkName: string): boolean {
    ...
    return Object.keys(FRAMEWORK_KNOWLEDGE_BASE).some(key =>
  ...
  getFrameworkInfo(frameworkName: string): FrameworkInfo | undefined {
    const lowerName = frameworkName.toLowerCase();
    const entry = Object.entries(FRAMEWORK_KNOWLEDGE_BASE).find(([key, fw]) =>
      key.toLowerCase() === lowerName ||
