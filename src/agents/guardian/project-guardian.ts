...
export interface ProjectHealthCheck {
  ...
  checks: {
    ...
    ide_performance: HealthCheckResult; // NEW v7.15.0
  };
  issues: ProjectIssue[];
...
export class ProjectGuardian {
   ...
   */
  public async performHealthCheck(): Promise<ProjectHealthCheck> {
    ...
    const result: ProjectHealthCheck = {
      overall_health,
      status,
      checks,
      issues,
   ...
   */
  private async checkFrameworkConfig(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    const issues: string[] = [];
   ...
   */
  private async checkAgentActivation(): Promise<HealthCheckResult> {
    const startTime = Date.now();
   ...
   */
  private async checkProjectBuild(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    const issues: string[] = [];
   ...
   * Check project tests
   */
  private async checkProjectTests(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    const issues: string[] = [];
   ...
   */
  private async checkFrameworkVersion(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    const issues: string[] = [];
   ...
   * Check RAG usage
   */
  private async checkRAGUsage(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    const issues: string[] = [];
   ...
   */
  private async checkIDEPerformance(): Promise<HealthCheckResult> {
    const startTime = Date.now();
    const issues: string[] = [];
    ...
    try {
      ...
      this.logger.info(
        `IDE crash risk detected: ${crashRisk.crash_risk} (confidence: ${crashRisk.confidence}%)`,
        {
      ...
      if (crashRisk.auto_fixable && crashRisk.evidence.missing_ignore_files.length > 0) {
        ...
        const result = await generator.generateOptimalConfigs(
          crashRisk.ide_type,
          crashRisk.evidence.large_directories.map(d => ({
            path: d.path,
        ...
        if (result.success) {
          this.logger.info(
            `✅ IDE config files generated: ${[...result.files_created, ...result.files_updated].join(', ')}`
          );
          // Improve score since we fixed the issue
          score = 95;
          issues.push(
            `Auto-generated IDE config files: ${[...result.files_created, ...result.files_updated].join(', ')}`
          );
        } else {
      ...
      this.logger.error(`IDE performance check failed: ${(error as Error).message}`);
      return {
  ...
  private getFixConfidence(component: string, issue: string): number {
    if (issue.includes('Missing .versatil-project.json')) return 95;
    if (issue.includes('outdated')) return 90;
    if (issue.includes('No agents configured')) return 85;
    if (issue.includes('Build failed')) return 70;
    if (issue.includes('test')) return 65;
    return 50;
  }
  ...
  private generateRecommendations(checks: any, issues: ProjectIssue[]): string[] {
