...
export class EnhancementApprovalService {
   ...
   */
  public async promptForApproval(
   ...
   */
  private async getApprovalDecision(
    suggestion: EnhancementSuggestion,
   ...
   */
  private showFullDetails(suggestion: EnhancementSuggestion): void {
    console.log('\n' + '='.repeat(80));
   ...
   */
  private async getRejectReason(): Promise<string> {
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    });
    ...
    return new Promise((resolve) => {
      rl.question('Rejection reason (optional, press Enter to skip): ', (reason: string) => {
        rl.close();
   ...
   */
  private storeDecision(result: ApprovalResult): void {
    try {
   ...
   */
  public getDecision(enhancementId: string): ApprovalResult | null {
    ...
    try {
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        const decision = JSON.parse(lines[i]) as ApprovalResult;
   ...
   */
  public getPendingApprovals(): ApprovalResult[] {
    if (!fs.existsSync(this.decisionsFile)) {
      return [];
    }
    ...
    try {
      ...
      const decisionsMap = new Map<string, ApprovalResult>();

      for (const line of lines) {
        const decision = JSON.parse(line) as ApprovalResult;
   ...
   */
  public getApprovalMode(): 'interactive' | 'auto' | 'manual' {
   ...
   */
  public shouldAutoApply(suggestion: EnhancementSuggestion): boolean {
   ...
   */
  public async executeEnhancement(
    suggestion: EnhancementSuggestion
    ...
    try {
      ...
      const issue = {
        ...
        severity: suggestion.priority as 'low' | 'medium' | 'high' | 'critical',
      ...
      return {
        success: false,
        message: `Enhancement execution error: ${(error as Error).message}. Creating TODO for manual implementation.`
      };
    }
  }
}
