 ...
 */
export async function checkGuardianHealth(
  workingDir: string,
  role: 'framework-developer' | 'user',
  runFullCheck: boolean = false
): Promise<QuickHealthStatus> {
  ...
  if (runFullCheck) {
    try {
      const { IrisGuardian } = await import('./iris-guardian.js');
      ...
      let mappedStatus: 'healthy' | 'degraded' | 'critical' = 'critical';
