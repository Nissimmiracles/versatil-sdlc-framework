...
export class IDEConfigGenerator {
   ...
   */
  public async generateOptimalConfigs(
    ideType: IDEType,
    largeDirectories: DirectorySize[] = []
  ): Promise<IDEConfigGenerationResult> {
   ...
   */
  private detectProjectType(): ProjectDetectionResult {
    ...
    let type: ProjectDetectionResult['type'] = 'unknown';
    if (technologies.length === 1) {
      type = technologies[0] as ProjectDetectionResult['type'];
   ...
   */
  private async generateCursorIgnore(
    ...
    try {
      ...
      return { success: false, error: `Failed to create .cursorignore: ${(error as Error).message}` };
    }
  }
  ...
  private async generateVSCodeSettings(
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to ${fs.existsSync(targetPath) ? 'update' : 'create'} .vscode/settings.json: ${(error as Error).message}`
      };
    }
  }
   ...
   */
  private async generateIdeaGitignore(
    projectInfo: ProjectDetectionResult
    ...
    try {
      ...
      return { success: false, error: `Failed to create .idea/.gitignore: ${(error as Error).message}` };
    }
  }
   ...
   */
  private buildIdeaGitignoreContent(projectInfo: ProjectDetectionResult): string {
    const lines: string[] = [
