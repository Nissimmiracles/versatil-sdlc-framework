...
// import { analyzeNamingConventions, type NamingConventionPreferences } from './ast-naming-analyzer.js';
type NamingConventionPreferences = any;
...
export interface PriorityViolation {
  field: string; // e.g., 'indentation', 'quotes', 'commitFormat'
  expected_value: string; // User preference value
...
export interface ContextVerificationResult {
  ...
  responsible_agent?: string; // Agent who wrote the code (via git blame)
  priority_violation?: PriorityViolation; // NEW: Track priority hierarchy violations (v7.8.0)
}
 ...
 */
export async function verifyContextIssue(
  issue: HealthIssue,
  workingDir: string,
  userId?: string,
  teamId?: string,
  ...
  const verifications: ContextVerification[] = [];
  ...
  let priorityViolation: PriorityViolation | undefined;
 ...
 */
function loadUserPreferences(userId?: string): any | null {
 ...
 */
function loadTeamConventions(teamId?: string): any | null {
 ...
 */
function loadProjectVision(projectId?: string): any | null {
 ...
 */
async function verifyIndentationStyle(
 ...
 */
async function verifyQuoteStyle(
  claim: { type: string; description: string; file?: string },
  workingDir: string,
  userId?: string,
  resolvedContext?: any
  ...
  const actualQuoteStyle =
    backticks > singleQuotes && backticks > doubleQuotes
      ? 'backticks'
...
async function verifyNamingConvention(
  ...
  try {
    ...
    const namingPreferences: NamingConventionPreferences = {
 ...
 */
async function verifyCommitFormat(
 ...
 */
async function verifyTestPolicy(
  claim: { type: string; description: string },
  workingDir: string,
  teamId?: string,
  resolvedContext?: any
...
async function verifyVisionAlignment(
  ...
  const visionGoals = projectVision.goals?.map((g: any) => g.description) || [];
  ...
  try {
    ...
    const hasMatch = visionGoals.some((goal: string) =>
      featureDesc.toLowerCase().includes(goal.toLowerCase()) ||
 ...
 */
function detectPriorityViolation(
  verification: ContextVerification,
  resolvedContext?: any
): PriorityViolation | undefined {
 ...
 */
function extractFieldFromClaim(claim: string): string {
  if (claim.includes('indentation')) return 'indentation';
  if (claim.includes('quote')) return 'quotes';
  if (claim.includes('naming')) return 'naming';
  if (claim.includes('Commits')) return 'commitFormat';
  if (claim.includes('Coverage')) return 'testCoverage';
  if (claim.includes('vision')) return 'visionAlignment';
  return 'unknown';
}
 ...
 */
function determineExpectedPriority(
  verification: ContextVerification,
  resolvedContext?: any
): 'User' | 'Team' | 'Project' | 'Framework' {
  ...
  if (resolvedContext?.resolution) {
    ...
    if (resolvedContext.resolution.userOverrides?.some((o: string) => claim.includes(o.split(' ')[0]))) {
      return 'User';
    }

    // Check if field has team override
    if (resolvedContext.resolution.teamOverrides?.some((o: string) => claim.includes(o.split(' ')[0]))) {
      return 'Team';
    }

    // Check if field has project override
    if (resolvedContext.resolution.projectOverrides?.some((o: string) => claim.includes(o.split(' ')[0]))) {
      return 'Project';
    }
  }
 ...
 */
function determineActualPriority(
  verification: ContextVerification,
  resolvedContext?: any
): 'User' | 'Team' | 'Project' | 'Framework' {
  ...
  if (verification.evidence?.user_preference && verification.evidence?.priority_violation) {
    ...
    if (actualPriorityHint.includes('Framework')) return 'Framework';
    if (actualPriorityHint.includes('Team')) return 'Team';
    if (actualPriorityHint.includes('Project')) return 'Project';
  }
...
function calculateViolationSeverity(
  ...
  if (gap >= 3) return 'critical'; // User overridden by Framework
  if (gap === 2) return 'high';    // User overridden by Project, or Team by Framework
  if (gap === 1) return 'medium';  // User overridden by Team
  return 'low';
}
 ...
 */
async function getResponsibleAgent(file: string, workingDir: string): Promise<string | undefined> {
  try {
