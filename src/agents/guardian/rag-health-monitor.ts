...
import { graphRAGStore, GraphRAGQuery } from '../../lib/graphrag-store.js';
import { EnhancedVectorMemoryStore } from '../../rag/enhanced-vector-memory-store.js';
import { getRAGRouter } from '../../rag/rag-router.js';
...
export class RAGHealthMonitor {
  private static instance: RAGHealthMonitor | null = null;
  private logger: VERSATILLogger;
  private vectorStore: EnhancedVectorMemoryStore | null = null;
  private ragRouter: any;
  private lastHealthCheck: RAGHealthReport | null = null;

  private constructor() {
    this.logger = new VERSATILLogger('RAG-Health-Monitor');
    this.ragRouter = getRAGRouter();
  }

  public static getInstance(): RAGHealthMonitor {
   ...
   */
  public async performHealthCheck(): Promise<RAGHealthReport> {
    ...
    try {
      ...
      if (overall_health >= 80) status = 'healthy';
      else if (overall_health >= 60) status = 'degraded';
      ...
      this.logger.error('RAG health check failed', { error: (error as Error).message });
        ...
        issues: [
          {
            component: 'router',
            severity: 'critical',
            description: `RAG health check failed: ${(error as Error).message}`,
            confidence: 100,
   ...
   */
  private async checkGraphRAGHealth(): Promise<RAGComponentHealth> {
    const startTime = Date.now();

    try {
      // Test query to GraphRAG
      const testQuery: GraphRAGQuery = {
        query: 'test health check',
        limit: 1,
      };

      const result = await graphRAGStore.query(testQuery);
   ...
   */
  private async checkVectorStoreHealth(): Promise<RAGComponentHealth> {
    const startTime = Date.now();
   ...
   */
  private async checkRAGRouterHealth(): Promise<RAGComponentHealth> {
    const startTime = Date.now();
   ...
   */
  private async checkPatternSearchHealth(): Promise<RAGComponentHealth> {
    const startTime = Date.now();
   ...
   */
  public async remediateIssue(issue: RAGIssue): Promise<RAGRemediationResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      success = false;
      action_taken += ` (FAILED: ${(error as Error).message})`;
   ...
   */
  private createIssueFromComponent(component: RAGComponentHealth): RAGIssue {
    let description = '';
    let root_cause = '';
    let confidence = 0;
   ...
   */
  private generateRecommendations(issues: RAGIssue[]): string[] {
   ...
   */
  private createUnknownComponent(component: 'graphrag' | 'vector' | 'router' | 'pattern_search'): RAGComponentHealth {
    return {
      component,
      healthy: false,
      latency_ms: 0,
   ...
   */
  public getLastHealthCheck(): RAGHealthReport | null {
 ...
 * Singleton instance
 */
let ragHealthMonitorInstance: RAGHealthMonitor | null = null;

/**
 * Get RAG Health Monitor instance
 */
export function getRAGHealthMonitor(): RAGHealthMonitor {
