...
export class VersionManager {
  ...
  private constructor(frameworkRoot: string) {
    this.logger = GuardianLogger.getInstance();
    this.frameworkRoot = frameworkRoot;
  }

  public static getInstance(frameworkRoot: string): VersionManager {
   ...
   */
  public async bumpVersion(type: VersionBumpType, skipGit = false): Promise<string> {
   ...
   * Create release
   */
  public async createRelease(version?: string): Promise<ReleaseInfo> {
   ...
   */
  public async getReleaseInfo(version: string): Promise<ReleaseInfo> {
    ...
    // Get contributors
    const { stdout: contributorList } = await execAsync(
      `git log v${currentVersion}..HEAD --format='%an' | sort -u`,
      { cwd: this.frameworkRoot }
    );
   ...
   */
  public async getRoadmapProgress(): Promise<RoadmapProgress> {
    ...
    const completed_features: string[] = [];
   ...
   */
  private async updateChangelog(release: ReleaseInfo): Promise<void> {
   ...
   */
  public async isOutdated(installedVersion: string): Promise<boolean> {
    try {
   ...
   */
  public async getBreakingChangesSince(version: string): Promise<string[]> {
    const changelogPath = path.join(this.frameworkRoot, 'CHANGELOG.md');
   ...
   * Generate release notes
   */
  public async generateReleaseNotes(version: string): Promise<string> {
