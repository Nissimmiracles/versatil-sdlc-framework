 ...
 */
export async function trackFileEditForGuardian(event: FileEditEvent): Promise<void> {
  ...
  const tracking: FileEditTracking = {
 ...
 */
export async function markAgentActivated(
  filePath: string,
  agent: string,
  success: boolean,
  duration_ms: number,
  ...
  const entries = lines.map(line => JSON.parse(line)) as FileEditTracking[];
 ...
 */
export async function checkActivationFailures(): Promise<Array<{
  filePath: string;
  expected_agent: string;
  ...
  const now = Date.now();
  const failures: Array<{ filePath: string; expected_agent: string; time_since_edit_ms: number }> = [];
 ...
 */
function determineExpectedAgent(filePath: string): string | undefined {
  // Test files → Maria-QA
