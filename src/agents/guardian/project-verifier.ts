...
function extractClaims(issue: HealthIssue): Array<{
  ...
  const claims: Array<{ type: string; description: string; threshold?: number; file?: string }> = [];
...
async function verifyTestCoverage(
  claim: { type: string; description: string; threshold?: number },
  workingDir: string
 ...
 */
async function verifyTestFailure(
 ...
 */
async function verifySecurityVulnerability(
 ...
 */
async function verifyOutdatedDependencies(
  claim: { type: string; description: string },
  workingDir: string
): Promise<ProjectVerification> {
  try {
...
async function verifyAccessibility(
  ...
  try {
    execSync('which lighthouse', { encoding: 'utf-8' });
  } catch {
    return {
