...
import { getUserCoherenceCheckService, CoherenceCheckResult } from '../../coherence/user-coherence-check.js';
import { VERSATILLogger } from '../../utils/logger.js';

export interface UserCoherenceMonitorConfig {
  check_interval_hours: number; // Default: 24 (daily)
  notify_on_updates: boolean; // Default: true
  notify_on_issues: boolean; // Default: true
  auto_fix_threshold: number; // Default: 90 (90%+ confidence)
...
export interface CoherenceTrend {
  ...
  issues_fixed: number;
  version_behind_by: number; // Total versions behind (major*100 + minor*10 + patch)
}
...
export class UserCoherenceMonitor {
  private static instance: UserCoherenceMonitor;
  ...
  async performMonitoring(): Promise<UserCoherenceReport> {
    ...
    try {
      ...
      if (healthCheck.auto_fixes_available.length > 0) {
        auto_remediations_applied = await this.applyAutoRemediations(
          healthCheck,
          coherenceService
        );
      }
      ...
      const nextCheckTime = new Date(
        Date.now() + this.config.check_interval_hours * 60 * 60 * 1000
      ).toISOString();
   ...
   */
  private async generateNotifications(healthCheck: CoherenceCheckResult): Promise<string[]> {
    ...
    if (this.config.notify_on_updates && healthCheck.checks.version.status !== 'up_to_date') {
      ...
      if (versionCheck.status === 'major_available') {
        notifications.push(
          `🔔 Major update available: v${versionCheck.latest_version} (you have v${versionCheck.installed_version})`
        );
        notifications.push('   Review breaking changes before updating');
      } else if (versionCheck.status === 'minor_available') {
        notifications.push(
          `🔔 Minor update available: v${versionCheck.latest_version} (you have v${versionCheck.installed_version})`
        );
        notifications.push('   Update recommended for new features and bug fixes');
      } else if (versionCheck.status === 'patch_available') {
        notifications.push(
          `🔔 Patch update available: v${versionCheck.latest_version} (you have v${versionCheck.installed_version})`
        );
      }
    }
    ...
    if (healthCheck.overall_health < 75) {
      notifications.push(
        `⚠️ Framework health degraded: ${healthCheck.overall_health}/100 (${healthCheck.status})`
      );
   ...
   */
  private async applyAutoRemediations(
    healthCheck: CoherenceCheckResult,
    coherenceService: any
  ): Promise<number> {
    // Filter fixes by confidence threshold
    const highConfidenceFixes = healthCheck.auto_fixes_available.filter(
      (fix) => fix.confidence >= this.config.auto_fix_threshold
    );
    ...
    try {
      ...
      const successCount = result.results.filter((r: string) => r.startsWith('✅')).length;
   ...
   * Update trends
   */
  private async updateTrends(healthCheck: CoherenceCheckResult): Promise<void> {
    try {
      const trends = await this.loadTrends();

      // Calculate version behind metric
      const versionCheck = healthCheck.checks.version;
      const version_behind_by =
        versionCheck.behind_by.major * 100 +
        versionCheck.behind_by.minor * 10 +
        versionCheck.behind_by.patch;

      // Add new trend data
      const newTrend: CoherenceTrend = {
   ...
   * Store check result
   */
  private async storeCheck(healthCheck: CoherenceCheckResult): Promise<void> {
    try {
   ...
   */
  private async loadLastCheck(): Promise<CoherenceCheckResult | null> {
    try {
   ...
   * Store trends
   */
  private async storeTrends(trends: CoherenceTrend[]): Promise<void> {
    try {
      await fs.mkdir(join(this.projectRoot, '.versatil', 'state'), { recursive: true });
   ...
   * Load trends
   */
  private async loadTrends(): Promise<CoherenceTrend[]> {
    try {
   ...
   */
  private async createReportFromCache(healthCheck: CoherenceCheckResult): Promise<UserCoherenceReport> {
    const trends = await this.loadTrends();

    const nextCheckTime = new Date(
      new Date(healthCheck.timestamp).getTime() + this.config.check_interval_hours * 60 * 60 * 1000
    ).toISOString();
