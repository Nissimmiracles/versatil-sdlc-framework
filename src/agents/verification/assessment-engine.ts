...
export interface AssessmentPlan {
  ...
  estimatedDuration: string; // e.g., "30s", "2m"
}
...
export interface Assessment {
  ...
  command: string;
  threshold?: number; // e.g., 80 for 80% coverage
  mandatory: boolean; // If true, failure blocks merge
  reason: string;
}
...
export interface AssessmentResult {
  ...
  executed: boolean;
  passed: boolean;
  score?: number; // e.g., 85 for 85% coverage
...
export class AssessmentEngine {
  ...
  needsAssessment(claim: Claim): boolean {
    ...
    if (this.matchesPattern(claim, 'security')) return true;

    // Always assess API changes
    if (this.matchesPattern(claim, 'api')) return true;

    // Always assess UI changes for accessibility/performance
    if (this.matchesPattern(claim, 'ui')) return true;

    // Assess test files for coverage
    if (this.matchesPattern(claim, 'test')) return true;

    // Assess database changes for security
    if (this.matchesPattern(claim, 'database')) return true;
  ...
  planAssessment(claim: Claim): AssessmentPlan {
    ...
    if (this.matchesPattern(claim, 'api')) {
      ...
      if (priority !== 'critical') priority = apiRule.priority;
    ...
    if (this.matchesPattern(claim, 'database')) {
      ...
      if (priority !== 'critical') priority = dbRule.priority;
   ...
   */
  async executeAssessment(plan: AssessmentPlan): Promise<AssessmentResult[]> {
  ...
  private matchesPattern(claim: Claim, patternType: string): boolean {
  ...
  private deduplicateAssessments(assessments: Assessment[]): Assessment[] {
  ...
  private estimateDuration(assessments: Assessment[]): string {
    let totalSeconds = 0;
