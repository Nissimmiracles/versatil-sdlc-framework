...
export class ContextStatsVisualizer {
   ...
   */
  formatMemoryOperations(operations: MemoryOperation[], limit: number = 10): string[] {
    const lines: string[] = [];
 ...
 */
export function createDashboard(stats: ContextStatistics): string {
...
export function createMarkdownReport(
  ...
  recentEvents: ContextClearEvent[]
): string {
  const visualizer = new ContextStatsVisualizer();
