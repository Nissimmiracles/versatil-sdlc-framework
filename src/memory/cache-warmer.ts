...
import { CacheAccessTracker, type AccessPattern } from './cache-access-tracker.js';
...
export interface WarmingResult {
  filesWarmed: number;
  totalTokens: number;
  cacheHitRateImprovement: number; // Expected improvement percentage
...
export interface CacheWarmingConfig {
  enabled: boolean;
  maxFilesToWarm: number; // Maximum files to pre-load per session
  maxTokensPerWarm: number; // Maximum tokens to use for warming
  strategies: WarmingStrategy[];
  warmOnSessionStart: boolean;
  warmOnAgentActivation: boolean;
  intelligentPrefetch: boolean; // Predict what will be needed next
}
...
export class CacheWarmer {
  private accessTracker: CacheAccessTracker;
  private config: CacheWarmingConfig;
  private statsDir: string;
  private lastWarmingTime: Date | null = null;
  private cachedContent: Map<string, { content: string; tokens: number; timestamp: Date }> = new Map();
  ...
  constructor(
    ...
    statsDir: string = path.join(os.homedir(), '.versatil', 'cache-warming')
  ) {
    ...
    this.config = {
      enabled: true,
      maxFilesToWarm: 10,
      maxTokensPerWarm: 10_000, // 10k tokens for warming (5% of cache)
    ...
    try {
      ...
      if (await this.fileExists(cachePath)) {
        ...
        for (const [path, entry] of Object.entries(parsed)) {
          const cacheEntry = entry as { content: string; tokens: number; timestamp: Date };
   ...
   */
  async warm(agentId?: string): Promise<WarmingResult> {
    ...
    console.log(
      `[CacheWarmer] Warming complete: ${filesWarmed} files, ${totalTokens} tokens, ${timeSpent}ms, expected +${cacheHitRateImprovement}% cache hit rate`
    );
   ...
   */
  async warmForAgent(agentId: string): Promise<WarmingResult> {
   ...
   */
  private selectFilesToWarm(patterns: AccessPattern[], agentId?: string): AccessPattern[] {
   ...
   * Warm specific files
   */
  private async warmSpecificFiles(patterns: AccessPattern[]): Promise<WarmingResult> {
    const startTime = Date.now();
    const errors: string[] = [];
    ...
    for (const pattern of patterns) {
      try {
        if (totalTokens >= this.config.maxTokensPerWarm) break;

        const content = await this.loadMemoryFile(pattern.path);
        if (!content) continue;

        const tokens = Math.ceil(content.length / 4);
        if (totalTokens + tokens > this.config.maxTokensPerWarm) continue;
   ...
   */
  private async predictNeededFiles(context: {
    agentId?: string;
    taskType?: string;
    recentFiles?: string[];
    ...
    return patterns.filter(pattern => {
      // Same agent
      if (context.agentId && pattern.agentId === context.agentId) return true;
      ...
      if (context.recentFiles) {
        for (const recent of context.recentFiles) {
          if (pattern.path.includes(path.dirname(recent))) return true;
        }
      }

      // Task-specific patterns
      if (context.taskType && pattern.path.includes(context.taskType)) return true;
   ...
   */
  private async loadMemoryFile(memoryPath: string): Promise<string | null> {
    try {
   ...
   * Empty result helper
   */
  private emptyResult(reason: string): WarmingResult {
    return {
      filesWarmed: 0,
      totalTokens: 0,
      cacheHitRateImprovement: 0,
   ...
   */
  getWarmedContent(): Map<string, { content: string; tokens: number; timestamp: Date }> {
...
export function setGlobalCacheWarmer(warmer: CacheWarmer): void {
