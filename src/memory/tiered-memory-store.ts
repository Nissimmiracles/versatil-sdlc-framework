...
export interface TierStatistics {
  hot: { count: number; totalSize: number; avgAccessTime: number };
  warm: { count: number; totalSize: number; avgAccessTime: number };
  cold: { count: number; totalSize: number; avgAccessTime: number };
  promotions: { hotToWarm: number; warmToCold: number; coldToHot: number };
}
...
export class TieredMemoryStore {
  private hotTier: Map<string, MemoryEntry> = new Map(); // In-memory
  private warmTierIndex: Map<string, MemoryEntry['metadata']> = new Map(); // Index only
  private coldTierIndex: Map<string, MemoryEntry['metadata']> = new Map(); // Index only
  private baseDir: string;
  private statistics: TierStatistics;

  private readonly HOT_TIER_MAX_DAYS = 7;
  private readonly WARM_TIER_MAX_DAYS = 30;
  private readonly HOT_TIER_MAX_SIZE_MB = 50; // 50MB max for hot tier
  private readonly HOT_TIER_MAX_SIZE_BYTES = this.HOT_TIER_MAX_SIZE_MB * 1024 * 1024;

  constructor(baseDir: string = path.join(os.homedir(), '.versatil', 'memories-tiered')) {
   ...
   */
  async store(memoryPath: string, content: string, agentId?: string): Promise<void> {
   ...
   */
  async retrieve(memoryPath: string): Promise<string | null> {
   ...
   */
  async delete(memoryPath: string): Promise<boolean> {
    let deleted = false;
   ...
   */
  private async promoteToHot(memoryPath: string, content: string, metadata: MemoryEntry['metadata']): Promise<void> {
   ...
   */
  private async demoteToWarm(memoryPath: string, entry: MemoryEntry): Promise<void> {
   ...
   */
  private async demoteToCold(memoryPath: string, metadata: MemoryEntry['metadata']): Promise<void> {
   ...
   */
  private shouldPromoteToHot(metadata: MemoryEntry['metadata']): boolean {
   ...
   */
  private shouldDemoteToWarm(metadata: MemoryEntry['metadata']): boolean {
   ...
   */
  private shouldDemoteToCold(metadata: MemoryEntry['metadata']): boolean {
   ...
   */
  private async evictFromHotTier(): Promise<void> {
    let oldest: { path: string; entry: MemoryEntry } | null = null;
   ...
   * Write to tier
   */
  private async writeToTier(tier: 'hot' | 'warm' | 'cold', memoryPath: string, content: string, compress: boolean = false): Promise<void> {
   ...
   * Read from tier
   */
  private async readFromTier(tier: 'hot' | 'warm' | 'cold', memoryPath: string, decompress: boolean = false): Promise<string | null> {
    const tierDir = path.join(this.baseDir, tier);
   ...
   * Delete from tier
   */
  private async deleteFromTier(tier: 'hot' | 'warm' | 'cold', memoryPath: string): Promise<void> {
    const tierDir = path.join(this.baseDir, tier);
   ...
   */
  private updateAvgAccessTime(currentAvg: number, newTime: number, count: number): number {
 ...
 * Singleton instance
 */
let globalTieredStore: TieredMemoryStore | null = null;

export function getGlobalTieredStore(): TieredMemoryStore {
