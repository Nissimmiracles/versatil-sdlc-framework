...
import { MEMORY_TOOL_CONFIG, AgentId } from './memory-tool-config.js';
...
export class ContextEditingManager {
  ...
  private readonly keepToolUses: number;
  private readonly minTokensToClear: number;
   ...
   */
  async trackOperation(agentId: AgentId, operation: MemoryToolOperation): Promise<void> {
   ...
   */
  async clearContext(agentId: AgentId, reason: 'auto' | 'manual' = 'auto'): Promise<void> {
    try {
   ...
   */
  private async storeContextToMemory(agentId: AgentId): Promise<string[]> {
...
`;
   ...
   */
  private estimateOperationTokens(operation: MemoryToolOperation): number {
   ...
   */
  private async loadClearEvents(): Promise<void> {
    try {
   ...
   */
  private async saveClearEvents(): Promise<void> {
    try {
      const statsDir = path.join(MEMORY_TOOL_CONFIG.memoryDirectory, '.stats');
