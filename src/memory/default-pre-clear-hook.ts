 ...
 */
export function createDefaultPreClearHook(
  config: PatternExtractionConfig = {}
): PreClearHook {
  const logger = new VERSATILLogger();

  const {
    minTokensBeforeExtraction = 80_000, // 80% of 100k threshold
    maxPatterns = 10
  } = config;

  return async (inputTokens: number, agentId?: string): Promise<number> => {
    try {
...
`;
      ...
      await memory.storePattern(
        'project-knowledge' as any,
        `context-preserved-${Date.now()}.md`,
        clearEventPattern
      );
 ...
 * @returns Pre-clear hook function
 */
export function createAdvancedPreClearHook(
  config: PatternExtractionConfig = {}
): PreClearHook {
  const logger = new VERSATILLogger();
...
`;
      ...
      await memory.storePattern(
        'project-knowledge' as any,
        `advanced-preservation-${Date.now()}.md`,
        preservationContent
      );
 ...
 * @returns Pre-clear hook function
 */
export function createEmergencyPreClearHook(
  config: PatternExtractionConfig = {}
): PreClearHook {
  const logger = new VERSATILLogger();
...
`;
      ...
      await memory.storePattern(
        'project-knowledge' as any,
        `emergency-preservation-${Date.now()}.md`,
        emergencyContent
      );
