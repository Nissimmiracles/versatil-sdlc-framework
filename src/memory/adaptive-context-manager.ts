...
export interface ConversationMetrics {
  cacheHitRate: number;           // 0.0 - 1.0 (70%+ is good)
  tokensPerMessage: number;       // Average tokens per user message
  conversationDepth: number;      // Total messages in conversation
  clearEvents: number;            // Number of clears so far
  totalTokensProcessed: number;   // Cumulative tokens
  avgTokensSavedPerClear: number; // Efficiency of clears
  memoryOperationsRate: number;   // Memory ops per 1000 tokens
}
...
export interface AdaptiveConfig {
  minThreshold: number;      // Absolute minimum (15k)
  maxThreshold: number;      // Absolute maximum (100k)
  targetCacheRate: number;   // Target cache hit rate (0.7 = 70%)
  aggressiveFactor: number;  // Multiplier for long conversations (0.85)
  conservativeFactor: number; // Multiplier for high cache rate (1.15)
}
...
export class AdaptiveContextManager {
  ...
  private readonly DEFAULT_THRESHOLD = 30_000;     // Claude recommended
  private readonly MIN_THRESHOLD = 15_000;         // Too low = excessive clears
  private readonly MAX_THRESHOLD = 100_000;        // Too high = cache invalidation
  private readonly TARGET_CACHE_RATE = 0.70;       // 70%+ is excellent
  private readonly AGGRESSIVE_FACTOR = 0.85;       // 15% reduction for long convos
  private readonly CONSERVATIVE_FACTOR = 1.15;     // 15% increase for high cache rate
  ...
  constructor(
    ...
    this.historyFile = path.join(
      ...
      '.versatil',
      'stats',
      'adaptive-threshold-history.json'
    );
  ...
  async adjustThreshold(metrics: ConversationMetrics): Promise<number> {
    ...
    if (metrics.cacheHitRate < this.config.targetCacheRate) {
      ...
      newThreshold = Math.max(
        ...
        this.currentThreshold - reduction
      );
   ...
   */
  async calculateMetrics(): Promise<ConversationMetrics> {
  ...
  async getRecommendations(): Promise<string[]> {
    ...
    if (metrics.cacheHitRate < 0.5) {
      recommendations.push(
        `⚠️ Low cache efficiency (${(metrics.cacheHitRate * 100).toFixed(1)}%). ` +
    ...
    if (metrics.avgTokensSavedPerClear < 2000 && metrics.clearEvents > 3) {
      recommendations.push(
        `⚠️ Inefficient clears (${metrics.avgTokensSavedPerClear} tokens/clear). ` +
    ...
    if (metrics.conversationDepth > 100) {
      recommendations.push(
        `⚠️ Long conversation (${metrics.conversationDepth} messages). ` +
    ...
    if (metrics.tokensPerMessage > 5000) {
      recommendations.push(
        `⚠️ High tokens per message (${metrics.tokensPerMessage}). ` +
    ...
    if (metrics.cacheHitRate > 0.7 && this.currentThreshold < this.config.maxThreshold) {
      recommendations.push(
        `✅ Excellent cache efficiency (${(metrics.cacheHitRate * 100).toFixed(1)}%). ` +
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        `✅ Threshold optimized (${this.currentThreshold.toLocaleString()} tokens). ` +
  ...
  private async loadHistory(): Promise<void> {
    try {
      ...
      this.adjustmentHistory = parsed.map((adj: any) => ({
        ...adj,
  ...
  private async saveHistory(): Promise<void> {
    try {
      ...
      await fs.writeFile(
        this.historyFile,
        JSON.stringify(this.adjustmentHistory, null, 2),
        'utf-8'
      );
    } catch (error) {
 ...
 */
export function createAdaptiveContextManager(
  statsTracker: ContextStatsTracker,
  config?: Partial<AdaptiveConfig>
