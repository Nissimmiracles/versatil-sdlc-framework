...
import { ArchitecturalValidator, ValidationResult } from './architectural-validator.js';
...
export class ArchitecturalWatcher {
  private validator: ArchitecturalValidator;
  ...
  constructor(projectRoot: string = process.cwd(), config: WatcherConfig = {}) {
   ...
   */
  private onFileChange(filePath: string, event: 'added' | 'modified' | 'deleted'): void {
   ...
   */
  private async validateFile(filePath: string, event: string): Promise<void> {
