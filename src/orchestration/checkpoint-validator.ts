...
export class CheckpointValidator {
   ...
   */
  async validate(checkpoint: Checkpoint): Promise<CheckpointValidationResult> {
    ...
    const handoffResults: HandoffValidationResult[] = [];
   ...
   */
  private async validateAgentHandoff(handoff: {
    from: string;
    to: string;
    context: string;
   ...
   */
  private findMatchingValidationStep(gate: string, steps: string[]): string | null {
   ...
   */
  private executeValidationStep(step: string): string {
   ...
   */
  private parseValidationOutput(gate: string, output: string): boolean {
    const gateLower = gate.toLowerCase();
   ...
   */
  private extractRequiredCoverage(gate: string): number {
