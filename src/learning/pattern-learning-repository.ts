...
export class PatternLearningRepository extends EventEmitter {
  ...
  private technologyIndex: Map<string, Set<string>> = new Map();
  ...
  async addPattern(pattern: Omit<Pattern, 'id' | 'metadata'>): Promise<string> {
    try {
  ...
  async searchPatterns(query: PatternQuery): Promise<Pattern[]> {
    try {
      let candidateIds = new Set<string>(this.patterns.keys());
  ...
  async recordPatternUsage(
    patternId: string,
    feedback: Omit<PatternFeedback, 'timestamp'>
  ): Promise<void> {
    try {
  ...
  async validatePattern(patternId: string): Promise<{
    valid: boolean;
    errors: string[];
    warnings: string[];
  }> {
  ...
  async getLearningInsights(): Promise<LearningInsights> {
    try {
      ...
      const categoryDistribution = {} as Record<PatternCategory, number>;
      const typeDistribution = {} as Record<PatternType, number>;
      const technologyAdoption = {} as Record<string, any>;

      // Initialize distributions
      Object.values(PatternCategory).forEach(cat => {
        categoryDistribution[cat as PatternCategory] = 0;
      });
      Object.values(PatternType).forEach(type => {
        typeDistribution[type as PatternType] = 0;
      });
  ...
  async exportRepository(outputPath: string): Promise<void> {
    try {
      const exportData = {
        version: '1.0.0',
        timestamp: Date.now(),
  ...
  async importRepository(inputPath: string): Promise<void> {
    try {
  ...
  private generatePatternId(pattern: Omit<Pattern, 'id' | 'metadata'>): string {
    const data = {
      name: pattern.name,
      category: pattern.category,
      type: pattern.type,
  ...
  private async updateIndexes(pattern: Pattern): Promise<void> {
    // Category index
  ...
  private async saveIndexes(): Promise<void> {
    try {
      const indexes = {
        search: Array.from(this.searchIndex.entries()).map(([key, ids]) =>
          [key, Array.from(ids)]
        ),
        category: Array.from(this.categoryIndex.entries()).map(([key, ids]) =>
          [key, Array.from(ids)]
        ),
        technology: Array.from(this.technologyIndex.entries()).map(([key, ids]) =>
          [key, Array.from(ids)]
        )
      };

      await fs.writeFile(
        join(this.indexPath, 'search.json'),
  ...
  private calculateRelevance(pattern: Pattern, context: Partial<PatternContext>): number {
    let score = 0;
    let factors = 0;

    // Technology match
    if (context.technologies?.length && pattern.context.technologies.length) {
      const overlap = context.technologies.filter(tech =>
        pattern.context.technologies.includes(tech)
      ).length;
    ...
    if (context.frameworks?.length && pattern.context.frameworks.length) {
      const overlap = context.frameworks.filter(fw =>
        pattern.context.frameworks.includes(fw)
      ).length;
    ...
    if (context.projectTypes?.length && pattern.context.projectTypes.length) {
      const overlap = context.projectTypes.filter(type =>
        pattern.context.projectTypes.includes(type)
      ).length;
  ...
  private generateRelevanceReasons(pattern: Pattern, context: Partial<PatternContext>): string[] {
    const reasons = [];
  ...
  private suggestAdaptations(pattern: Pattern, context: Partial<PatternContext>): string[] {
    const adaptations = [];

    // Technology adaptations
    if (context.technologies?.length) {
      const missingTechs = context.technologies.filter(tech =>
        !pattern.context.technologies.includes(tech)
      );
  ...
  private identifyRisks(pattern: Pattern, context: Partial<PatternContext>): string[] {
    const risks = [];
  ...
  private estimateImpact(pattern: Pattern, context: Partial<PatternContext>): PatternRecommendation['estimatedImpact'] {
    return {
  ...
  private recalculatePatternMetrics(pattern: Pattern): void {
    if (pattern.metrics.feedback.length === 0) return;
  ...
  private updateMaturityLevel(pattern: Pattern): void {
  ...
  private async executeValidationRule(pattern: Pattern, rule: ValidationRule): Promise<{
    failed: boolean;
    message: string;
  }> {
