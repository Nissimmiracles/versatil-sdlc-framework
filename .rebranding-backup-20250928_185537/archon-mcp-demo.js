/**
 * VERSATIL SDLC Framework - Archon MCP Live Demo
 * Interactive demonstration of all Archon MCP features
 */

const chalk = require('chalk');
const inquirer = require('inquirer');
const { callMCPTool, readMCPResource } = require('./test-archon-mcp');
 ...
 * Main demo flow
 */
async function runArchonMCPDemo() {
  demo.title('🚀 VERSATIL v1.2.0 - Archon MCP Live Demo');
  
  console.log('Welcome to the interactive Archon MCP demonstration!');
  ...
  // Demo 1: Archon Status
  demo.section('1. Checking Archon MCP Status');
  demo.info('Retrieving current Archon orchestrator status...');
  
  const status = await callMCPTool('archon_get_status');
  if (status) {
    demo.success('Archon MCP is running!');
  ...
  demo.info('Creating goal with Archon...');
  demo.code(JSON.stringify(goalConfig, null, 2));
  
  const goal = await callMCPTool('archon_create_goal', goalConfig);
  if (goal) {
  ...
  const analysis = await callMCPTool('archon_analyze_project', { depth: analysisDepth });
  if (analysis) {
  ...
  demo.info('Checking for Archon MCP updates...');
  ...
  demo.section('5. Accessing Archon Resources');
  
  const resources = [
    { uri: 'archon://goals', name: 'Active Goals' },
    { uri: 'archon://metrics', name: 'Performance Metrics' },
    { uri: 'archon://context', name: 'Environmental Context' },
    { uri: 'archon://updates', name: 'Update History' }
  ];
  ...
  for (const resource of resources) {
    ...
    if (data) {
      ...
      // Show sample data
      if (resource.uri === 'archon://metrics') {
  ...
  demo.section('6. Interactive Archon Control');
  ...
  while (continueInteractive) {
    ...
    switch (action) {
      case 'create_goal':
        ...
        const quickGoal = await callMCPTool('archon_create_goal', {
          type: 'feature',
      ...
      case 'view_plans':
        const plans = await readMCPResource('archon://plans');
        if (plans) {
      ...
      case 'update_context':
        await callMCPTool('archon_update_context', {
          context: {
            lastDemo: Date.now(),
            demoUser: 'Interactive User',
            features: ['archon-mcp', 'auto-update']
          }
        });
      ...
      case 'check_performance':
        ...
        if (metrics) {
          ...
          console.log('\n📊 Performance Metrics:');
          console.log(`  Success Rate: ${metricsData.archonState?.performance?.successRate || 0}%`);
          console.log(`  Goal Completion: ${metricsData.archonState?.performance?.goalCompletionRate || 0}%`);
          console.log(`  Avg Execution Time: ${metricsData.archonState?.performance?.averageExecutionTime || 0}ms`);
        }
        break;
  ...
  console.log('You\'ve experienced the power of VERSATIL Archon MCP:');
  ...
  console.log('\n🚀 Ready to integrate Archon MCP into your workflow?');
  console.log('   Run: npm run archon:start');
  
  console.log('\n📚 Learn more:');
  console.log('   Documentation: ./ARCHON_MCP_DOCUMENTATION.md');
  console.log('   Examples: ./examples/archon-mcp/');
  console.log('   Support: support@versatil-framework.com');
}

// Run demo if called directly
if (require.main === module) {
  console.log(chalk.bold.green('\n🎬 Starting Archon MCP Demo...\n'));
...
module.exports = { runArchonMCPDemo };
